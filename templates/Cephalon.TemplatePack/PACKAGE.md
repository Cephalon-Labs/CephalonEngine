...
For the SaaS tenant governance and audit replay, use `pwsh ./scripts/validate-saas-tenant-governance-audit-adoption.ps1`. That CLI-owned replay is intentionally outside the lightweight `dotnet new` starter path, but it keeps the same package-feed and generated-app adoption discipline while proving tenant governance, invitation delivery observation, audit, REST, JSON-RPC, gRPC, dependency-health, and `/engine/*` truth from a generated modular-monolith tenant host.
