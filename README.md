...
If you are adopting Cephalon from a clean machine or package source, start with [docs/getting-started.md](docs/getting-started.md). That guide walks through installing the CLI, running `cephalon doctor`, checking the current deployment-mode support contract from that same command path, scaffolding a host, rerunning `cephalon doctor --app-root ./Acme.Store` to validate generated-app bootstrap plus target-framework, generated self-hosted and hosted deployment assets, generated container deployment-asset and Dockerfile baseline, generated local orchestration assets plus `compose.yaml` and `otel-collector-config.yaml` baseline posture, publish-claim posture, and validating the runtime introspection endpoints.
