...
- engine-level OpenTelemetry semantic-convention adapter (`Cephalon.Diagnostics`) publishing canonical `ActivitySource` and `Meter` names plus stable `cephalon.*` attribute keys, the `IRedactionFilter` + `RedactionPipeline` engine-boundary redaction surface wired at three M1 emission sites (HTTP middleware, engine runtime module-phase tags, Wolverine dispatch tags), and the central diagnostic-id registry that catalogs every allocated `EventId` range across packages
