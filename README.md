...
- curated consumer-facing analyzer meta-package (`Cephalon.Analyzers`) bundling `Microsoft.CodeAnalysis.BannedApiAnalyzers`, opt-in public-surface tracking through `Microsoft.CodeAnalysis.PublicApiAnalyzers`, `Roslynator.Analyzers`, `Meziantou.Analyzer`, and `Microsoft.VisualStudio.Threading.Analyzers` plus a curated `BannedSymbols.txt` (auto-wired through `buildTransitive/Cephalon.Analyzers.props`) and a curated `cephalon-analyzers.editorconfig`
...
- `src/Cephalon.MultiTenancy.Governance`: tenant-membership, tenant-invitation, declared tenant-domain ownership, approval/remediation action decision, in-process action workflow, and opt-in durable membership/invitation/domain/action-store governance companion package
...
If you are adopting Cephalon from a clean machine or package source, start with [docs/getting-started.md](docs/getting-started.md). That guide walks through installing the CLI, running `cephalon doctor`, checking the current deployment-mode support contract from that same command path, scaffolding a host, rerunning `cephalon doctor --app-root ./Acme.Store` to validate generated-app bootstrap plus the scaffolded `Program.cs` host bootstrap source with explicit `AddCephalonProjectConfigurations` and `MapCephalon` wiring, the generated host-project `PackageReference` set including `Cephalon.Analyzers` and `Cephalon.Engine.SourceGen` as private analyzer packages, generated REST behavior modules including `Cephalon.Behaviors.SourceGen` as a private analyzer package, plus `Configurations/**/*.json` copy/publish baseline, the generated starter test project plus `Architecture/CompositionSmokeTests.cs` and `Features/*BehaviorSpecifications.cs` Given/When/Then test-harness placeholders, target-framework posture, generated split-config assets under `Configurations/AddEngine.*.json` plus `Configurations/Observability/Development.json`, generated root, local package-feed, and deployment guidance docs (`README.md`, `.cephalon/packages/README.md`, `Configurations/README.md`, and `deploy/*/README.md`), generated OpenAPI and hosted reference-doc config assets, generated self-hosted and hosted deployment assets plus the teardown and environment baselines in `deploy/windows-service/remove-service.ps1`, `deploy/iis/remove-site.ps1`, and `deploy/linux/systemd/<App>.env`, generated container deployment assets plus the executable container deployment-script baselines in `deploy/container-image/publish-image.ps1`, `deploy/azure-container-apps/deploy-up.ps1`, and `deploy/kubernetes/apply.ps1`, the generated Kubernetes manifest baselines in `deploy/kubernetes/kustomization.yaml`, `deploy/kubernetes/namespace.yaml`, `deploy/kubernetes/deployment.yaml`, and `deploy/kubernetes/service.yaml`, generated Dockerfile baseline posture, generated local orchestration assets plus `compose.yaml` and `otel-collector-config.yaml` baseline posture, publish-claim posture, and validating the runtime introspection endpoints.
...
For the vertical-slice Eventing/outbox replay, use `pwsh ./scripts/validate-vertical-slice-eventing-adoption.ps1`. It publishes the temporary feed including `Cephalon.Eventing`, `Cephalon.Eventing.Behaviors`, `Cephalon.Data`, `Cephalon.Ids.Sfid`, and behavior packages, installs `Cephalon.Cli`, scaffolds `ModularVerticalSlice` with `CQRS`, `Outbox`, `RestApi`, and `EventDrivenIntegration`, validates generated Eventing startup through `engine.AddEventingFromConfiguration(builder.Configuration)` plus executable `Engine:Messaging` channel/routing settings and the generated process-local `IOutbox` starter descriptor, runs the ASP.NET Core host, publishes an event through `/engine/event-publications`, validates accepted outbox handoff through `/engine/event-publications/runtime/{publicationId}`, `/engine/event-dispatches/terminal-failures`, `/engine/event-dispatch-remediation-commands/summary`, `/engine/diagnostics`, `/engine/snapshot`, and `/scalar`, and writes `artifacts/adoption-smoke/vertical-slice-eventing-adoption.json` unless `-ReportPath` overrides it.

For the microservice multi-transport replay, use `pwsh ./scripts/validate-microservice-multi-transport-adoption.ps1`. It publishes the temporary feed including `Cephalon.AspNetCore.JsonRpc`, `Cephalon.AspNetCore.Grpc`, `Cephalon.Observability.HttpDependencies`, and `Cephalon.Observability.DependencyHealth.Core`, installs `Cephalon.Cli`, scaffolds `Microservice` with `RestApi`, `JsonRpc`, and `Grpc`, runs the generated host over HTTP/1.1 and HTTP/2, validates `/api/v1/platform/status`, `/json-rpc/platform`, `DiscoveryService.SayHello` through `/grpc`, `/engine/transports`, `/engine/snapshot`, `/engine/dependencies`, `/engine/diagnostics`, `/engine/runtime-story`, and `/scalar`, and writes `artifacts/adoption-smoke/microservice-multi-transport-adoption.json` unless `-ReportPath` overrides it.

For the SaaS tenant governance and audit replay, use `pwsh ./scripts/validate-saas-tenant-governance-audit-adoption.ps1`. It publishes the temporary feed including `Cephalon.MultiTenancy.Governance`, `Cephalon.MultiTenancy.Governance.AspNetCore`, `Cephalon.Audit`, `Cephalon.AspNetCore.JsonRpc`, `Cephalon.AspNetCore.Grpc`, `Cephalon.Observability.HttpDependencies`, and `Cephalon.Observability.DependencyHealth.Core`, installs `Cephalon.Cli`, scaffolds a modular-monolith tenant app with REST, JSON-RPC, and gRPC, wires a smoke invitation sender, maps tenant-administration, invitation-dispatch, delivery-status, observation, and audit proof endpoints, runs the generated host, validates `/engine/tenant-administration/commands`, `/engine/tenant-invitations/delivery-dispatches`, `/engine/tenant-invitations/delivery-status`, `/engine/tenant-invitations/delivery-status/observations`, `/engine/audit-stores`, `/engine/technology-surfaces/multi-tenancy`, `/engine/dependencies`, `/engine/transports`, `/engine/snapshot`, `/scalar`, and writes `artifacts/adoption-smoke/saas-tenant-governance-audit.json` unless `-ReportPath` overrides it.
...
Generated app roots from `cephalon new` and the app-focused `dotnet new` starters now also emit `README.md`, `NuGet.config`, `./.cephalon/packages/README.md`, `Properties/PublishProfiles/CephalonFolder.pubxml`, `Configurations/README.md`, `Configurations/AddEngine.*.json`, `Configurations/Observability/Development.json`, `Configurations/AddOpenApi.json`, `Configurations/AddReferenceDocs.json`, `tests/<App>.Tests/Architecture/CompositionSmokeTests.cs`, `tests/<App>.Tests/Features/*BehaviorSpecifications.cs`, `deploy/windows-service/README.md`, `deploy/windows-service/install-service.ps1`, `deploy/windows-service/remove-service.ps1`, `deploy/iis/README.md`, `deploy/iis/install-site.ps1`, `deploy/iis/remove-site.ps1`, `deploy/azure-app-service/README.md`, `deploy/azure-app-service/deploy-zip.ps1`, `deploy/container-image/README.md`, `deploy/container-image/publish-image.ps1`, `deploy/azure-container-apps/README.md`, `deploy/azure-container-apps/deploy-up.ps1`, `deploy/kubernetes/README.md`, `deploy/kubernetes/apply.ps1`, `deploy/kubernetes/kustomization.yaml`, `deploy/kubernetes/namespace.yaml`, `deploy/kubernetes/deployment.yaml`, `deploy/kubernetes/service.yaml`, `deploy/linux/systemd/README.md`, `deploy/linux/systemd/<App>.service`, `deploy/linux/systemd/<App>.env`, `.dockerignore`, `Dockerfile`, `compose.yaml`, and `otel-collector-config.yaml`, so teams can seed a repo-local package feed, validate a fresh scaffold with deterministic published output, keep app-model, observability, localization, `/scalar`, optional hosted reference-doc config, generated guidance docs, and the generated Given/When/Then test harness explicit from the same doctor path, build and publish a provider-neutral container image, install a self-hosted Windows or Linux service shape, preview a hosted IIS site/app-pool path, take either an Azure App Service ZIP deploy path, an Azure Container Apps source-deploy path, or a platform-neutral Kubernetes manifest path, and rerun `cephalon doctor --app-root ./Acme.Store` before the first `docker compose up --build` so generated local orchestration assets plus compose and collector baselines stay truthful before platform-specific deployment packaging begins.
