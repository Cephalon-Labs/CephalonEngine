...
public sealed class EventDispatchHostingTests
{
    ...
    public async Task MapCephalonExposesEventDispatchRuntimeAndStateEndpoints()
    {
        ...
        var missingRuntimeResponse = await client.GetAsync("/engine/event-dispatch-runtimes/missing-runtime");
        ...
        Assert.Equal(HttpStatusCode.NotFound, missingRuntimeResponse.StatusCode);
    ...
    public async Task MapCephalonRoutesCoreInProcessEventPublicationFromConfigurationWithoutWolverine()
    {
        ...
        var inboundConsumptionEntry = Assert.Single(
            eventingSurfaces.Single(surface => surface.SurfaceId == "eventing-superiority-profile").Entries,
            entry => entry.Id == "broker-inbound-consumption-ownership");
        ...
        Assert.Equal("not-claimed", inboundConsumptionEntry.Metadata["status"]);
        Assert.Contains("inProcessExecution=active", inboundConsumptionEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("brokerInboundConsumption=not-claimed", inboundConsumptionEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("providerOwnedConsumer=not-present", inboundConsumptionEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("inboundAcknowledgement=not-claimed", inboundConsumptionEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("consumerOffsetCheckpoint=not-claimed", inboundConsumptionEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("wolverineRequired=false", inboundConsumptionEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Equal("not-claimed", serializationVersioningEntry.Metadata["status"]);
        Assert.Contains("publicationPath=active", serializationVersioningEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("publicationRouting=configured", serializationVersioningEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("serializerSelection=not-claimed", serializationVersioningEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("schemaRegistry=not-present", serializationVersioningEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        ...
        Assert.Equal("not-claimed", subscriptionConcurrencyEntry.Metadata["status"]);
        Assert.Contains("inProcessExecution=active", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("subscriptionExecutionPipeline=none", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("subscriptionExecutionMiddlewareCount=0", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("subscriptionConcurrency=not-claimed", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("perSubscriptionConcurrencyLimit=not-claimed", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("parallelHandlerExecution=not-claimed", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("consumerPrefetch=not-claimed", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("backpressure=not-claimed", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("providerConcurrency=not-present", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
        ...
        var snapshotDeliveryCompletionEntry = Assert.Single(
            snapshot.TechnologySurfaces.Single(surface => surface.SurfaceId == "eventing-superiority-profile").Entries,
            entry => entry.Id == "downstream-delivery-completion-ownership");
        var snapshotInboundConsumptionEntry = Assert.Single(
            snapshot.TechnologySurfaces.Single(surface => surface.SurfaceId == "eventing-superiority-profile").Entries,
            entry => entry.Id == "broker-inbound-consumption-ownership");
        ...
        Assert.Equal("not-claimed", snapshotInboundConsumptionEntry.Metadata["status"]);
        ...
        Assert.Equal("not-claimed", snapshotSubscriptionConcurrencyEntry.Metadata["status"]);
        Assert.Contains("subscriptionConcurrency=not-claimed", snapshotSubscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
    ...
    public async Task MapCephalonRunsCodeFirstSubscriptionExecutionMiddlewareWithoutWolverine()
    {
        ...
        var subscriptionConcurrencyEntry = Assert.Single(
            ...
            entry => entry.Id == "subscription-concurrency-ownership");
        Assert.Equal("not-claimed", subscriptionConcurrencyEntry.Metadata["status"]);
        Assert.Contains("subscriptionExecutionPipeline=code-first", subscriptionConcurrencyEntry.Metadata["runtimeEvidence"], StringComparison.Ordinal);
