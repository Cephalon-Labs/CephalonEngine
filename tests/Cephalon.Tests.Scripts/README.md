...
| `scripts/validate-contract-compatibility.ps1` + `scripts/compatibility-process.ps1` | `validate-contract-compatibility.Tests.ps1` | 9 cases: package identity/hashes, declared framework, traversal/ambiguous metadata/DTD rejection, subprocess exits and timeout; AST extraction loads the package guard, and the shared process helper is loaded without running the harness |
