...
| --- | --- | --- |
| `scripts/publish-engine-completion-scorecard.ps1` | `publish-engine-completion-scorecard.Tests.ps1` | scorecard JSON/Markdown artifact shape + evidence-source reference validation + per-package GA readiness rows + SRE guardrail-reference validation + unsupported status guard + release-validation wiring |
