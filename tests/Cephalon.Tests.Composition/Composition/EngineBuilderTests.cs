...
public sealed class EngineBuilderTests
{
    ...
    public async Task AddEventingProjectsSuperiorityProfileWithoutWolverine()
    {
        ...
        var benchmarkProofEntry = Assert.Single(
            benchmarkProofSurface.Entries,
            entry => entry.Id == "eventing.remediation.filter-operator-dashboard-selectors");
        var aspNetCoreColdStartProofEntry = Assert.Single(
            benchmarkProofSurface.Entries,
            entry => entry.Id == "eventing.host-cold-start.aspnetcore-core-operator");
        ...
        Assert.Equal("false", benchmarkProofEntry.Metadata["wolverineRequired"]);
        Assert.Equal("BuildStartHandleFirstRequestAspNetCore", aspNetCoreColdStartProofEntry.Metadata["benchmark"]);
        Assert.Equal("Cephalon.Benchmarks.Runtime.ColdStartBenchmarks-report.csv", aspNetCoreColdStartProofEntry.Metadata["benchmarkReport"]);
        Assert.Equal("cold-start", aspNetCoreColdStartProofEntry.Metadata["guardrailFamily"]);
        Assert.Equal("800000000", aspNetCoreColdStartProofEntry.Metadata["maxMeanNanoseconds"]);
        Assert.Equal("30000000", aspNetCoreColdStartProofEntry.Metadata["maxAllocatedBytes"]);
        Assert.Equal("false", aspNetCoreColdStartProofEntry.Metadata["wolverineRequired"]);
        ...
        Assert.Contains("guardrailCount=11", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("guardrailIds=eventing.remediation.filter-summary-by-message-id,eventing.remediation.filter-retention-by-message-id,eventing.remediation.filter-latest-by-correlation-id,eventing.remediation.filter-oldest-by-dispatch-outcome,eventing.remediation.filter-operator-dashboard-selectors,eventing.host-cold-start.aspnetcore-core-operator,eventing.host-cold-start.worker,eventing.broker-dispatch.report-projected-headers,eventing.durable-journal.record-replay-cursor,eventing.provider-managed.report-runtime-proofs,eventing.provider-operated.report-aggregate-runtime-proofs", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("benchmarks=FilterSummaryByMessageId,FilterRetentionByMessageId,FilterLatestByCorrelationId,FilterOldestByDispatchOutcome,FilterOperatorDashboardSelectors,BuildStartHandleFirstRequestAspNetCore,BuildStartWorkerHost,ReportProjectedBrokerDispatches,RecordAndReadDurableJournal,ReportProviderManagedEventingProofs,ReportProviderOperatedEventingProofs", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("Cephalon.Benchmarks.Runtime.ColdStartBenchmarks-report.csv", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("Cephalon.Benchmarks.HotPath.EventDispatchBrokerDispatchBenchmarks-report.csv", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("Cephalon.Benchmarks.HotPath.EventDispatchDurableJournalBenchmarks-report.csv", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("Cephalon.Benchmarks.HotPath.EventProviderManagedEventingBenchmarks-report.csv", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("Cephalon.Benchmarks.HotPath.EventProviderOperatedEventingBenchmarks-report.csv", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("requiredGuardrailFamilyCount=6", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("coveredGuardrailFamilies=broker-dispatch,cold-start,durable-journal,provider-managed-eventing,provider-operated-eventing,remediation-filtered-reads", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("missingGuardrailFamilies=none", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        ...
        Assert.Contains("operationalSuperiorityReadiness=blocked", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionGate=blocked", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionAllowed=false", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionPolicy=all-operational-superiority-dimensions-claimed", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionEvidenceContract=cephalon-eventing-operational-superiority-promotion-v1", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionEvidenceContractVersion=1.0.0", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionTarget=eventing-operational-superiority", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionRequiredStatus=claimed", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionDecisionCode=missing-required-dimensions", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionBlockers=provider-operated-runtime-proof-coverage,choreography-handoff-ownership,durable-remediation-command-audit,durable-command-journal-replay-cursor", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionBlockerCount=4", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("blockingOperationalSuperiorityDimensions=provider-operated-runtime-proof-coverage,choreography-handoff-ownership,durable-remediation-command-audit,durable-command-journal-replay-cursor", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("blockingOperationalSuperiorityDimensionStatuses=provider-operated-runtime-proof-coverage:not-claimed,choreography-handoff-ownership:not-claimed,durable-remediation-command-audit:not-claimed,durable-command-journal-replay-cursor:not-claimed", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("nextOperationalSuperiorityRequirements=provider-operated-runtime-proof-coverage:complete-provider-operated-runtime-proof-coverage,choreography-handoff-ownership:activate-eventing-behavior-bridge-with-outbox-handoff,durable-remediation-command-audit:activate-durable-cross-node-remediation-command-journal,durable-command-journal-replay-cursor:provide-durable-remediation-command-replay-cursor", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("nextOperationalSuperiorityRequirementCount=4", dimensions["operational-superiority-coverage"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
    ...
    {
        var olderBrokerDeadLetterReport = EventDispatchBrokerDeadLetterReplayMetadata.CreateReport(
            new EventDispatchExecutionReport(
                outboxId: "alpha-outbox",
                channelId: "contracts",
                outcome: EventDispatchExecutionOutcomes.Failed,
                observedAtUtc: new DateTimeOffset(2026, 05, 13, 5, 0, 0, TimeSpan.Zero),
                messageId: "evt-alpha-dlq-001",
                attempt: 3,
                error: "alpha broker failure"),
            source: "alpha-dlq-runtime",
            brokerDeadLetterQueueId: "alpha-broker-dlq",
            brokerReplayActionCatalogId: "alpha-replay-actions",
            brokerReplayCursorId: "alpha-replay-cursor",
            brokerPurgeQuarantineId: "alpha-purge-quarantine",
            providerProofId: "alpha-dlq-replay-proof");
        var newerBrokerDeadLetterReport = EventDispatchBrokerDeadLetterReplayMetadata.CreateReport(
            new EventDispatchExecutionReport(
                outboxId: "beta-outbox",
                channelId: "contracts",
                outcome: EventDispatchExecutionOutcomes.Failed,
                observedAtUtc: new DateTimeOffset(2026, 05, 13, 5, 30, 0, TimeSpan.Zero),
                messageId: "evt-beta-dlq-001",
                attempt: 3,
                error: "beta broker failure"),
            source: "beta-dlq-runtime",
            brokerDeadLetterQueueId: "beta-broker-dlq",
            brokerReplayActionCatalogId: "beta-replay-actions",
            brokerReplayCursorId: "beta-replay-cursor",
            brokerPurgeQuarantineId: "beta-purge-quarantine",
            providerProofId: "beta-dlq-replay-proof");
        var newerPartialBrokerDeadLetterReport = new EventDispatchExecutionReport(
            outboxId: "gamma-outbox",
            channelId: "contracts",
            outcome: EventDispatchExecutionOutcomes.Failed,
            observedAtUtc: new DateTimeOffset(2026, 05, 13, 5, 45, 0, TimeSpan.Zero),
            messageId: "evt-gamma-dlq-001",
            attempt: 3,
            error: "gamma broker failure",
            metadata: new Dictionary<string, string>
            {
                [EventDispatchRuntimeMetadataKeys.BrokerDeadLetterReplayOwnership] = "provider-reported",
                [EventDispatchRuntimeMetadataKeys.BrokerDeadLetterReplayOwnershipSource] = "gamma-dlq-runtime",
        ...
        services.AddSingleton<IEventDispatchRuntimeCatalog>(new TestEventDispatchRuntimeCatalog(
            CreateDispatchRuntimeState(olderBrokerDeadLetterReport),
            CreateDispatchRuntimeState(newerBrokerDeadLetterReport),
            CreateDispatchRuntimeState(newerPartialBrokerDeadLetterReport)));
        services.AddCephalon(engine =>
        {
            engine.UseSettings(new EngineSettings(
                blueprint: "Microservice",
        ...
        Assert.Contains("brokerDeadLetterReplayOwnership=provider-reported", evidence, StringComparison.Ordinal);
        Assert.Contains("brokerDeadLetterReplayOwnershipSource=beta-dlq-runtime", evidence, StringComparison.Ordinal);
        ...
        Assert.Contains("brokerReplayActionCatalogId=beta-replay-actions", evidence, StringComparison.Ordinal);
        Assert.Contains("brokerReplayCursor=reported", evidence, StringComparison.Ordinal);
        Assert.Contains("brokerReplayCursorId=beta-replay-cursor", evidence, StringComparison.Ordinal);
        Assert.Contains("brokerPurgeQuarantine=reported", evidence, StringComparison.Ordinal);
        Assert.Contains("brokerPurgeQuarantineId=beta-purge-quarantine", evidence, StringComparison.Ordinal);
        Assert.Contains("brokerDeadLetterReplayProofId=beta-dlq-replay-proof", evidence, StringComparison.Ordinal);
        Assert.Contains("deadLetterOutcome=provider-broker-dead-letter", evidence, StringComparison.Ordinal);
        ...
        Assert.Contains("lastObservedAtUtc=2026-05-13T05:30:00.0000000+00:00", evidence, StringComparison.Ordinal);
        Assert.Contains("providerOwnedBrokerPath=reported", evidence, StringComparison.Ordinal);
        Assert.Contains("wolverineRequired=false", evidence, StringComparison.Ordinal);
        Assert.DoesNotContain("brokerDeadLetterReplayProofId=alpha-dlq-replay-proof", evidence, StringComparison.Ordinal);
        Assert.DoesNotContain("brokerDeadLetterReplayOwnershipSource=gamma-dlq-runtime", evidence, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void AddEventingSelectsLatestProvenDownstreamDeliveryEvidenceAcrossOutboxesWithoutWolverine()
    {
        var olderDownstreamDeliveryReport = EventDispatchExactlyOnceDeliveryProofMetadata.CreateReport(
            new EventDispatchExecutionReport(
                outboxId: "alpha-outbox",
                channelId: "contracts",
                outcome: EventDispatchExecutionOutcomes.Succeeded,
            ...
            strategy: "alpha-transactional-outbox");
        var newerDownstreamDeliveryReport = EventDispatchExactlyOnceDeliveryProofMetadata.CreateReport(
            new EventDispatchExecutionReport(
                outboxId: "beta-outbox",
                channelId: "contracts",
                outcome: EventDispatchExecutionOutcomes.Succeeded,
                observedAtUtc: new DateTimeOffset(2026, 05, 13, 7, 30, 0, TimeSpan.Zero),
            ...
            strategy: "beta-transactional-outbox");
        var newerPartialDownstreamDeliveryReport = EventDispatchDeliveryCompletionMetadata.CreateReport(
            new EventDispatchExecutionReport(
                outboxId: "gamma-outbox",
                channelId: "contracts",
                outcome: EventDispatchExecutionOutcomes.Succeeded,
                observedAtUtc: new DateTimeOffset(2026, 05, 13, 7, 45, 0, TimeSpan.Zero),
                messageId: "evt-gamma-delivery-001",
                attempt: 1),
            source: "gamma-delivery-runtime",
            providerReceiptId: "gamma-provider-receipt");
        var services = new ServiceCollection();
        services.AddSingleton<IOutbox>(new EventingProofSelectionOutbox("alpha-outbox"));
        services.AddSingleton<IEventDispatchRuntimeCatalog>(new TestEventDispatchRuntimeCatalog(
            CreateDispatchRuntimeState(olderDownstreamDeliveryReport),
            CreateDispatchRuntimeState(newerDownstreamDeliveryReport),
            CreateDispatchRuntimeState(newerPartialDownstreamDeliveryReport)));
        services.AddCephalon(engine =>
        {
            engine.UseSettings(new EngineSettings(
                blueprint: "Microservice",
            ...
            engine.AddEventing(options =>
            {
                options.Channels.Add(new EventChannelDescriptor(
                    id: "contracts",
                    displayName: "Contracts",
                    description: "Downstream delivery proof events."));
            });
        });
        ...
        Assert.Contains("downstreamDeliveryStateCount=3", evidence, StringComparison.Ordinal);
        Assert.Contains("downstreamDeliveryProvenCount=2", evidence, StringComparison.Ordinal);
        Assert.Contains("downstreamDeliveryCompletionSource=beta-delivery-runtime", evidence, StringComparison.Ordinal);
        Assert.Contains("providerDeliveryReceipt=reported", evidence, StringComparison.Ordinal);
        Assert.Contains("providerDeliveryReceiptId=beta-provider-receipt", evidence, StringComparison.Ordinal);
        Assert.Contains("subscriberAcknowledgement=reported", evidence, StringComparison.Ordinal);
        Assert.Contains("subscriberAcknowledgementId=beta-subscriber-ack", evidence, StringComparison.Ordinal);
        Assert.Contains("destinationCommit=reported", evidence, StringComparison.Ordinal);
        Assert.Contains("destinationCommitId=beta-destination-commit", evidence, StringComparison.Ordinal);
        Assert.Contains("exactlyOnceDelivery=provider-proven", evidence, StringComparison.Ordinal);
        Assert.Contains("exactlyOnceDeliverySource=beta-delivery-runtime", evidence, StringComparison.Ordinal);
        Assert.Contains("exactlyOnceDeliveryProofId=beta-exactly-once-proof", evidence, StringComparison.Ordinal);
        Assert.Contains("exactlyOnceDeliveryStrategy=beta-transactional-outbox", evidence, StringComparison.Ordinal);
        Assert.Contains("outboxId=beta-outbox", evidence, StringComparison.Ordinal);
        Assert.Contains("lastOutcome=succeeded", evidence, StringComparison.Ordinal);
        Assert.Contains("lastObservedAtUtc=2026-05-13T07:30:00.0000000+00:00", evidence, StringComparison.Ordinal);
        Assert.Contains("wolverineRequired=false", evidence, StringComparison.Ordinal);
        Assert.DoesNotContain("exactlyOnceDeliveryProofId=alpha-exactly-once-proof", evidence, StringComparison.Ordinal);
        Assert.DoesNotContain("downstreamDeliveryCompletionSource=gamma-delivery-runtime", evidence, StringComparison.Ordinal);
    }
    ...
    public void AddEventingClaimsProviderOperatedRuntimeProofCoverageWhenOperationalProofDimensionsArePresentWithoutWolverine()
    {
        ...
        Assert.Contains("benchmarkFamily=provider-operated-eventing", evidence, StringComparison.Ordinal);
        ...
        Assert.Contains("operationalSuperiorityReadiness=blocked", operationalEvidence, StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionGate=blocked", operationalEvidence, StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionAllowed=false", operationalEvidence, StringComparison.Ordinal);
        ...
        Assert.Contains("operationalSuperiorityPromotionDecisionCode=missing-required-dimensions", operationalEvidence, StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionBlockers=choreography-handoff-ownership,durable-remediation-command-audit,durable-command-journal-replay-cursor", operationalEvidence, StringComparison.Ordinal);
        Assert.Contains("operationalSuperiorityPromotionBlockerCount=3", operationalEvidence, StringComparison.Ordinal);
        Assert.Contains("blockingOperationalSuperiorityDimensions=choreography-handoff-ownership,durable-remediation-command-audit,durable-command-journal-replay-cursor", operationalEvidence, StringComparison.Ordinal);
        Assert.Contains("blockingOperationalSuperiorityDimensionStatuses=choreography-handoff-ownership:not-claimed,durable-remediation-command-audit:not-claimed,durable-command-journal-replay-cursor:not-claimed", operationalEvidence, StringComparison.Ordinal);
        Assert.Contains("nextOperationalSuperiorityRequirements=choreography-handoff-ownership:activate-eventing-behavior-bridge-with-outbox-handoff,durable-remediation-command-audit:activate-durable-cross-node-remediation-command-journal,durable-command-journal-replay-cursor:provide-durable-remediation-command-replay-cursor", operationalEvidence, StringComparison.Ordinal);
        Assert.Contains("nextOperationalSuperiorityRequirementCount=3", operationalEvidence, StringComparison.Ordinal);
