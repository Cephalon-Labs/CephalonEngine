...
public sealed class EngineBuilderTests
{
    ...
    public async Task AddEventingProjectsSuperiorityProfileWithoutWolverine()
    {
        ...
        var appendSingleEventProofEntry = Assert.Single(
            benchmarkProofSurface.Entries,
            entry => entry.Id == "eventing.event-journal.append-single-event");
        var readStreamProofEntry = Assert.Single(
            benchmarkProofSurface.Entries,
            entry => entry.Id == "eventing.event-journal.read-stream");
        var getStreamVersionProofEntry = Assert.Single(
            benchmarkProofSurface.Entries,
            entry => entry.Id == "eventing.event-journal.get-stream-version");
        ...
        Assert.Equal("AppendSingleEvent", appendSingleEventProofEntry.Metadata["benchmark"]);
        Assert.Equal("Cephalon.Benchmarks.HotPath.EventSourcingBenchmarks-report.csv", appendSingleEventProofEntry.Metadata["benchmarkReport"]);
        Assert.Equal("durable-journal", appendSingleEventProofEntry.Metadata["guardrailFamily"]);
        Assert.Equal("10000", appendSingleEventProofEntry.Metadata["maxMeanNanoseconds"]);
        Assert.Equal("10000", appendSingleEventProofEntry.Metadata["maxAllocatedBytes"]);
        Assert.Equal("false", appendSingleEventProofEntry.Metadata["wolverineRequired"]);
        Assert.Equal("ReadStream", readStreamProofEntry.Metadata["benchmark"]);
        Assert.Equal("Cephalon.Benchmarks.HotPath.EventSourcingBenchmarks-report.csv", readStreamProofEntry.Metadata["benchmarkReport"]);
        Assert.Equal("durable-journal", readStreamProofEntry.Metadata["guardrailFamily"]);
        Assert.Equal("50000", readStreamProofEntry.Metadata["maxMeanNanoseconds"]);
        Assert.Equal("20000", readStreamProofEntry.Metadata["maxAllocatedBytes"]);
        Assert.Equal("false", readStreamProofEntry.Metadata["wolverineRequired"]);
        Assert.Equal("GetStreamVersion", getStreamVersionProofEntry.Metadata["benchmark"]);
        Assert.Equal("Cephalon.Benchmarks.HotPath.EventSourcingBenchmarks-report.csv", getStreamVersionProofEntry.Metadata["benchmarkReport"]);
        Assert.Equal("durable-journal", getStreamVersionProofEntry.Metadata["guardrailFamily"]);
        Assert.Equal("5000", getStreamVersionProofEntry.Metadata["maxMeanNanoseconds"]);
        Assert.Equal("5000", getStreamVersionProofEntry.Metadata["maxAllocatedBytes"]);
        Assert.Equal("false", getStreamVersionProofEntry.Metadata["wolverineRequired"]);
        ...
        Assert.Contains("guardrailIds=eventing.remediation.filter-summary-by-message-id,eventing.remediation.filter-retention-by-message-id,eventing.remediation.filter-latest-by-correlation-id,eventing.remediation.filter-oldest-by-dispatch-outcome,eventing.remediation.filter-operator-dashboard-selectors,eventing.host-cold-start.aspnetcore-core-operator,eventing.host-cold-start.worker,eventing.event-journal.append-single-event,eventing.event-journal.read-stream,eventing.event-journal.get-stream-version", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("benchmarks=FilterSummaryByMessageId,FilterRetentionByMessageId,FilterLatestByCorrelationId,FilterOldestByDispatchOutcome,FilterOperatorDashboardSelectors,BuildStartHandleFirstRequestAspNetCore,BuildStartWorkerHost,AppendSingleEvent,ReadStream,GetStreamVersion", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        ...
        Assert.Contains("Cephalon.Benchmarks.HotPath.EventSourcingBenchmarks-report.csv", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        ...
        Assert.Contains("coveredGuardrailFamilies=cold-start,durable-journal,remediation-filtered-reads", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
        Assert.Contains("missingGuardrailFamilies=broker-dispatch,provider-managed-eventing", dimensions["testability-and-benchmark-evidence"].Metadata["runtimeEvidence"], StringComparison.Ordinal);
