...
public sealed class WolverineEventingPackTests
{
    ...
    public async Task AddWolverineEventingProjectsManagedDispatchReportsIntoAdapterAndDispatchSurfaces()
    {
        ...
        Assert.Equal("/engine/event-dispatch-remediation-commands/outboxes/{outboxId}/retention", remediationEntry.Metadata["operatorCommandOutboxRetentionRoute"]);
        Assert.Equal("/engine/event-dispatch-remediation-commands/messages/{messageId}/retention", remediationEntry.Metadata["operatorCommandMessageRetentionRoute"]);
        Assert.Equal("/engine/event-dispatch-remediation-commands/outboxes/{outboxId}/latest", remediationEntry.Metadata["operatorCommandOutboxLatestRoute"]);
        Assert.Equal("/engine/event-dispatch-remediation-commands/messages/{messageId}/latest", remediationEntry.Metadata["operatorCommandMessageLatestRoute"]);
        ...
        Assert.Equal("retained-filter-server-side-retention", remediationEntry.Metadata["operatorCommandFilterRetentionPolicy"]);
        Assert.Equal("outboxes,messages,channels,operations,actors,correlations,reasons,dispatch-outcomes,outcomes", remediationEntry.Metadata["operatorCommandFilterRetentionRoutes"]);
        Assert.Equal(nameof(EventDispatchRemediationRuntimeRetention), remediationEntry.Metadata["operatorCommandFilterRetentionResponse"]);
        Assert.Equal("not-required", remediationEntry.Metadata["operatorCommandFilterRetentionMaterialization"]);
        Assert.Equal("retained-filter-server-side-latest", remediationEntry.Metadata["operatorCommandFilterLatestPolicy"]);
        ...
        Assert.Equal("all,in-doubt,observations,outboxes,messages,channels,operations,actors,correlations,reasons,dispatch-outcomes,outcomes", remediationEntry.Metadata["operatorCommandPaginationRoutes"]);
        ...
        Assert.Equal("/engine/event-dispatch-remediation-commands/outboxes/{outboxId}/retention", remediationCommandCatalogEntry.Metadata["commandOutboxRetentionRoute"]);
        Assert.Equal("/engine/event-dispatch-remediation-commands/outboxes/{outboxId}/latest", remediationCommandCatalogEntry.Metadata["commandOutboxLatestRoute"]);
        ...
        Assert.Equal("retained-filter-server-side-retention", remediationCommandCatalogEntry.Metadata["commandFilterRetentionPolicy"]);
        Assert.Equal("outboxes,messages,channels,operations,actors,correlations,reasons,dispatch-outcomes,outcomes", remediationCommandCatalogEntry.Metadata["commandFilterRetentionRoutes"]);
        Assert.Equal(nameof(EventDispatchRemediationRuntimeRetention), remediationCommandCatalogEntry.Metadata["commandFilterRetentionResponse"]);
        Assert.Equal("not-required", remediationCommandCatalogEntry.Metadata["commandFilterRetentionMaterialization"]);
        Assert.Equal("retained-filter-server-side-latest", remediationCommandCatalogEntry.Metadata["commandFilterLatestPolicy"]);
