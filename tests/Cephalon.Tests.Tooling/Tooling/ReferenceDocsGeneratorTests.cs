...
public sealed class ReferenceDocsGeneratorTests
{
    ...
    public void GenerateBuildsPagesForPackageBackedHostAssemblies()
    {
        ...
        var request = new ReferenceDocsRequest(
            ...
            outputPath: outputPath,
            configuration: GetCurrentBuildConfiguration(),
            assemblies: ["Cephalon.AspNetCore", "Cephalon.MultiTenancy.Governance.AspNetCore"]);

        var rendered = ReferenceDocsGenerator.Generate(request);
        var hostPage = Assert.Single(rendered.Files, file => file.Path == "cephalon-aspnetcore.md");
        var governanceHostPage = Assert.Single(rendered.Files, file => file.Path == "cephalon-multitenancy-governance-aspnetcore.md");
    ...
    [Fact]
    public void GenerateBuildsPageForMultiTenancyGovernanceSendGridDeliveryAssembly()
    {
        var outputPath = Path.Combine(Path.GetTempPath(), $"cephalon-reference-docs-multitenancy-sendgriddelivery-{Guid.NewGuid():N}");
        var request = new ReferenceDocsRequest(
            rootPath: GetRepositoryRoot(),
            outputPath: outputPath,
            configuration: GetCurrentBuildConfiguration(),
            assemblies: ["Cephalon.MultiTenancy.Governance.SendGridDelivery"]);

        var rendered = ReferenceDocsGenerator.Generate(request);
        var page = Assert.Single(rendered.Files, file => file.Path == "cephalon-multitenancy-governance-sendgriddelivery.md");

        Assert.Contains("SendGridInvitationDeliveryOptions", page.Contents, StringComparison.Ordinal);
        Assert.Contains("SendGridInvitationDeliveryServiceCollectionExtensions", page.Contents, StringComparison.Ordinal);
        Assert.Contains("ISendGridInvitationDeliveryClient", page.Contents, StringComparison.Ordinal);
        Assert.Contains("AddCephalonSendGridInvitationDelivery", page.Contents, StringComparison.Ordinal);
    }

    [Fact]
    public void GenerateBuildsPageForMultiTenancyGovernanceMailgunDeliveryAssembly()
    {
        var outputPath = Path.Combine(Path.GetTempPath(), $"cephalon-reference-docs-multitenancy-mailgundelivery-{Guid.NewGuid():N}");
        var request = new ReferenceDocsRequest(
            rootPath: GetRepositoryRoot(),
            outputPath: outputPath,
            configuration: GetCurrentBuildConfiguration(),
            assemblies: ["Cephalon.MultiTenancy.Governance.MailgunDelivery"]);

        var rendered = ReferenceDocsGenerator.Generate(request);
        var page = Assert.Single(rendered.Files, file => file.Path == "cephalon-multitenancy-governance-mailgundelivery.md");

        Assert.Contains("MailgunInvitationDeliveryOptions", page.Contents, StringComparison.Ordinal);
        Assert.Contains("MailgunInvitationDeliveryServiceCollectionExtensions", page.Contents, StringComparison.Ordinal);
        Assert.Contains("IMailgunInvitationDeliveryClient", page.Contents, StringComparison.Ordinal);
        Assert.Contains("AddCephalonMailgunInvitationDelivery", page.Contents, StringComparison.Ordinal);
    }

    [Fact]
    public void GenerateBuildsPageForMultiTenancyGovernanceAmazonSesDeliveryAspNetCoreAssembly()
    {
        var outputPath = Path.Combine(Path.GetTempPath(), $"cephalon-reference-docs-multitenancy-amazonsesdelivery-aspnetcore-{Guid.NewGuid():N}");
        var request = new ReferenceDocsRequest(
            rootPath: GetRepositoryRoot(),
            outputPath: outputPath,
            configuration: GetCurrentBuildConfiguration(),
            assemblies: ["Cephalon.MultiTenancy.Governance.AmazonSesDelivery.AspNetCore"]);

        var rendered = ReferenceDocsGenerator.Generate(request);
        var page = Assert.Single(rendered.Files, file => file.Path == "cephalon-multitenancy-governance-amazonsesdelivery-aspnetcore.md");

        Assert.Contains("AmazonSesInvitationDeliveryAspNetCoreOptions", page.Contents, StringComparison.Ordinal);
        Assert.Contains("AmazonSesInvitationDeliveryAspNetCoreServiceCollectionExtensions", page.Contents, StringComparison.Ordinal);
        Assert.Contains("MapCephalonAmazonSesInvitationDeliveryStatusCallbacks", page.Contents, StringComparison.Ordinal);
        Assert.Contains("AmazonSesInvitationDeliveryStatusCallbackResult", page.Contents, StringComparison.Ordinal);
    }

    [Fact]
    public void GenerateBuildsPageForMultiTenancyGovernanceMailgunDeliveryAspNetCoreAssembly()
    {
        var outputPath = Path.Combine(Path.GetTempPath(), $"cephalon-reference-docs-multitenancy-mailgundelivery-aspnetcore-{Guid.NewGuid():N}");
        var request = new ReferenceDocsRequest(
            rootPath: GetRepositoryRoot(),
            outputPath: outputPath,
            configuration: GetCurrentBuildConfiguration(),
            assemblies: ["Cephalon.MultiTenancy.Governance.MailgunDelivery.AspNetCore"]);

        var rendered = ReferenceDocsGenerator.Generate(request);
        var page = Assert.Single(rendered.Files, file => file.Path == "cephalon-multitenancy-governance-mailgundelivery-aspnetcore.md");

        Assert.Contains("MailgunInvitationDeliveryAspNetCoreOptions", page.Contents, StringComparison.Ordinal);
        Assert.Contains("MailgunInvitationDeliveryAspNetCoreServiceCollectionExtensions", page.Contents, StringComparison.Ordinal);
        Assert.Contains("MapCephalonMailgunInvitationDeliveryStatusCallbacks", page.Contents, StringComparison.Ordinal);
        Assert.Contains("MailgunInvitationDeliveryStatusCallbackResult", page.Contents, StringComparison.Ordinal);
    }
    ...
    public void GenerateBuildsPageForEventingAssembly()
    {
        ...
        Assert.Contains("EventingServiceCollectionExtensions", eventingPage.Contents, StringComparison.Ordinal);
        ...
        Assert.Contains("EventSubscriptionAttribute", eventingPage.Contents, StringComparison.Ordinal);
        Assert.Contains("IEventSubscriptionExecutionBindingCatalog", eventingPage.Contents, StringComparison.Ordinal);
        Assert.Contains("IEventSubscriptionDescriptorProvider", eventingPage.Contents, StringComparison.Ordinal);
    ...
    public void GenerateBuildsPageForMultiTenancyGovernanceAssembly()
    {
        ...
        Assert.Contains("ITenantInvitationStore", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantInvitationValidator", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantInvitationDeliveryStatusReconciler", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantInvitationDeliveryStatusReconciliationRequest", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantInvitationDeliveryStatusReconciliationResult", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipDescriptor", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipStore", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipValidator", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipVerificationWorkflow", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipProofEvaluator", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipProofChallengeIssuer", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofChallengeOutcomes", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofChallengeRequest", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofChallengeResult", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofChallengeMetadataKeys", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipProofPublicationPlanner", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipHttpProofPublisher", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofPublicationPlanOutcomes", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofPublicationPlanRequest", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofPublicationPlanResult", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofPublicationPlanMetadataKeys", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipHttpProofCollector", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipHttpProofCollectionOutcomes", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipHttpProofCollectionRequest", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipHttpProofCollectionResult", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipHttpProofCollectionMetadataKeys", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipDnsTxtProofCollector", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipDnsTxtProofCollectionOutcomes", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipDnsTxtProofCollectionRequest", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipDnsTxtProofCollectionResult", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipDnsTxtProofCollectionMetadataKeys", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantDomainOwnershipProofVerificationRunner", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipProofVerificationOutcomes", governancePage.Contents, StringComparison.Ordinal);
        ...
        Assert.Contains("ITenantDomainOwnershipProofPollingRuntimeCatalog", governancePage.Contents, StringComparison.Ordinal);
        ...
        Assert.Contains("TenantDomainOwnershipProofPollingRuntimeSnapshot", governancePage.Contents, StringComparison.Ordinal);
        ...
        Assert.Contains("TenantDomainOwnershipProofMetadataKeys", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantDomainOwnershipVerificationWorkflowRequest", governancePage.Contents, StringComparison.Ordinal);
        ...
        Assert.Contains("ITenantGovernanceActionStore", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("ITenantGovernanceActionWorkflow", governancePage.Contents, StringComparison.Ordinal);
        Assert.Contains("TenantGovernanceActionWorkflowRequest", governancePage.Contents, StringComparison.Ordinal);
    ...
    [Fact]
    public void GenerateBuildsPageForMultiTenancyGovernanceAspNetCoreAssembly()
    {
        var outputPath = Path.Combine(Path.GetTempPath(), $"cephalon-reference-docs-multi-tenancy-governance-aspnetcore-{Guid.NewGuid():N}");
        var request = new ReferenceDocsRequest(
            rootPath: GetRepositoryRoot(),
            outputPath: outputPath,
            configuration: GetCurrentBuildConfiguration(),
            assemblies: ["Cephalon.MultiTenancy.Governance.AspNetCore"]);
    ...
    public void GenerateDefaultCatalogIncludesCurrentShippedHostCompanions()
    {
        ...
        Assert.Contains(rendered.Files, static file => file.Path == "cephalon-multitenancy-governance-amazonsesdelivery-aspnetcore.md");
        ...
        Assert.Contains(
            assemblies,
            static assembly => string.Equals(assembly.GetProperty("AssemblyName").GetString(), "Cephalon.MultiTenancy.Governance.AmazonSesDelivery.AspNetCore", StringComparison.Ordinal));
        Assert.Contains(
            assemblies,
        ...
        Assert.Contains(
            assemblies,
            static assembly => string.Equals(assembly.GetProperty("AssemblyName").GetString(), "Cephalon.MultiTenancy.Governance.MailgunDelivery.AspNetCore", StringComparison.Ordinal));
        Assert.Contains(
            assemblies,
        ...
        Assert.Contains(
            assemblies,
            static assembly => string.Equals(assembly.GetProperty("AssemblyName").GetString(), "Cephalon.MultiTenancy.Governance.SmtpDelivery", StringComparison.Ordinal));
        Assert.Contains(
            assemblies,
