...
public sealed class CliApplicationTests
{
    ...
    public async Task RunAsyncDoctorReportsEngineCompletionScorecardSummary()
    {
        ...
        await File.WriteAllTextAsync(scorecardPath, """
            {
              ...
              "SrePostureEvidence": {
                ...
                "PendingStableBaselineCount": 5,
                "StableBaselineCount": 6,
                "StableBaselineManifest": "scripts/sre-stable-baselines.json",
                "StableBaselineRowCount": 6,
                "StableBaselineMeasurementCount": 8,
              ...
              "Summary": {
                ...
                "SrePendingStableBaselineCount": 5,
                "SreStableBaselineCount": 6,
        ...
        try
        {
            ...
            Assert.Equal(0, exitCode);
            Assert.Contains("[ok] Engine completion scorecard artifact: schema 1.10.0 from docs/engine-completion-scorecard.md; conformance matrix docs/conformance-matrix.md.", stdout.ToString(), StringComparison.Ordinal);
            ...
            Assert.Contains("[warn] Engine completion scorecard SRE posture: 11 SLIs; target-declared 11, pending stable baselines 5, stable baselines 6, stable baseline rows 6, stable baseline measurements 8, guardrail-mapped 6, pending guardrail coverage 0, guardrail not-applicable 5, guardrail references 8; stable baseline manifest scripts/sre-stable-baselines.json.", stdout.ToString(), StringComparison.Ordinal);
    ...
    public async Task RunAsyncDoctorFailsWhenScorecardSchemaIsUnsupported()
    {
        ...
        try
        {
            ...
            Assert.Equal(1, exitCode);
            Assert.Contains("[error] Engine completion scorecard artifact: Unsupported schema '1.0.0'. Doctor expects scorecard schema '1.10.0'.", stdout.ToString(), StringComparison.Ordinal);
            Assert.Contains("scorecard artifact blockers", stderr.ToString(), StringComparison.Ordinal);
        }
        finally
        {
