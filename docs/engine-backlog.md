...
- ENG-058-T41 REST declaration-style cleanup and explicit generic REST contracts: `ApiRoutes:Prefixes:Rest = ""` now deliberately mounts versioned REST routes at the root while `null` still falls back to `/api`; `[BehaviorAllowedTransports("http.rest")]` now acts as the annotation-driven generic REST activation path with fail-fast duplicate detection when `ConfigureTopology(...)` also declares `http.rest`; the HTTP companion package now supports `ViaHttpRest(rest => ...)` for one explicit generic REST method/route/query contract per behavior; and source-generation/docs/test coverage now lock the runtime-fallback plus authoring rules — **Shipped** · behavior source-generator tests + composition HTTP tests
- ENG-058-T42 attribute-only behavior baseline synthesis and transport alias normalization: behaviors can now run from `[BehaviorAllowedPatterns]` plus `[BehaviorAllowedTransports]` alone when exactly one pattern is declared; ambiguous multi-pattern declarations still fail fast until another topology source selects one; `http.grpc` is now accepted as an allowlist alias for canonical `grpc`; and composition/component-doc coverage now locks the attribute-only registration plus generic REST route behavior — **Shipped** · behavior baseline + HTTP binding tests
