...
- ENG-054 provider configuration standardization: `Cephalon.Data.MongoDB` and `Cephalon.Data.Redis` now align on `ConnectionStringName` plus `ConnectionString`, resolve named values from the root `ConnectionStrings` section through configuration-bound options callbacks, fail fast when both settings are supplied at once, keep localhost defaults only when neither setting is configured, and update the showcase host plus component docs to demonstrate the canonical provider-config contract — **Shipped**
