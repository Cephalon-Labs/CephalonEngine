...
- ENG-058-T32 behavior REST OpenAPI document selection follow-through: `BehaviorRestEndpointGroup.ApiVersion(major)` now places newly mapped behavior REST endpoints into named OpenAPI documents, uses the explicit API version as the operation-name version segment when configured, `Cephalon.AspNetCore` now registers named documents from `OpenApi:Documents` while keeping `/openapi/v1.json` and `/scalar/v1` as the default surface, and the showcase cart module plus authoring docs now demonstrate `.ApiVersion(1)` — **Shipped** · composition HTTP tests 13/13 + hosting tests 77/77
