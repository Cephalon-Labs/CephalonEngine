...
- keep the shipped HTTP, MongoDB, MySQL, NATS, Postgres, RabbitMQ, Redis, and SQL Server dependency-health companions reflected accurately in docs and project tracking while broader provider coverage stays explicit
...
- shipped OpenTelemetry companion packaging plus HTTP external API, MongoDB document database, MySQL database, NATS broker, Postgres database, RabbitMQ broker, Redis/cache, and SQL Server dependency-health companions, together with the shared diagnostics/event-id catalog for active packages and explicit release-validation guidance for health/export conventions
