...
- the smallest honest next proof is a host-agnostic runner that composes the owned services through one entry point without claiming DNS TXT collection, actual publication, provider mutation, background polling, tenant-admin workflow, or public onboarding
...
- publish `tenancy.domain-ownership.proof-verification-runner`, proof-verification runner runtime-surface metadata, DNS TXT collection and external polling ownership boundaries, and stable diagnostics `4534-4535` through the existing `tenant-domain-ownership` runtime surface
- prove the slice with focused composition, hosting, package-surface, reference-doc, and docs/source/planning alignment
