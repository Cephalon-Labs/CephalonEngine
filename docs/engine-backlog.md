...
- ENG-058-T34 Scalar multi-document selector follow-through: the ASP.NET Core host now keeps `/scalar` as the multi-document selector entry point instead of redirecting straight to `/scalar/v{major}`, preserving Scalar's built-in version dropdown when multiple OpenAPI documents are configured while keeping `/scalar/v1`, `/scalar/v2`, and similar paths as pinned-document deep links; hosting coverage and authoring docs now lock the selector-first behavior — **Shipped** · composition HTTP tests 14/14 + targeted hosting tests 2/2
