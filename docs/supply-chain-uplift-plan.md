...
*Shipped (workflow-side).* New tag-triggered [`.github/workflows/publish-release.yml`](../.github/workflows/publish-release.yml) wraps the validated package set with: fail-closed external-policy preflight via [`scripts/validate-supply-chain-external-policy-preflight.ps1`](../scripts/validate-supply-chain-external-policy-preflight.ps1), SLSA v1.1 build provenance via `actions/attest-build-provenance`, Sigstore Cosign keyless signature plus Rekor transparency log entry per `.nupkg`, CycloneDX 1.6 SBOM per `Cephalon.*` project, NuGet trusted-publishing login (GitHub OIDC federation) followed by `dotnet nuget push` of `.nupkg` and `.snupkg`. The workflow runs on `v*.*.*` tag push (and `workflow_dispatch` for dry-runs); the per-PR `release-validation.yml` gate is unchanged. [`docs/package-publishing.md`](package-publishing.md) carries the new *Signed release pipeline (tag-triggered)* section; [`docs/engineering-standards.md`](engineering-standards.md) security/supply-chain section carries the new *Signed release pipeline* subsection. See `ENG-324` in [`engine-backlog.md`](engine-backlog.md). The later `ENG-485` follow-through adds [`scripts/supply-chain-release-support.json`](../scripts/supply-chain-release-support.json) so the engine-completion scorecard emits `SupplyChainEvidence` from the workflow and keeps the nuget.org-side trusted-publishing, prefix-reservation, `NUGET_USER`, and preflight prerequisites visible. `ENG-531` adds archive-level package metadata validation through [`scripts/validate-package-metadata.ps1`](../scripts/validate-package-metadata.ps1), so the package set now proves readme/tag/license/repository/source/package-type/symbol-pairing truth before signing or publishing. `ENG-533` adds a fail-closed NuGet vulnerability audit through [`scripts/validate-nuget-vulnerability-audit.ps1`](../scripts/validate-nuget-vulnerability-audit.ps1), so release validation writes `artifacts/nuget-vulnerability-audit-release/nuget-vulnerability-audit.json` and the signed release bundle includes that advisory scan beside the package, SBOM, signature, and external-policy artefacts. `ENG-532` adds [`scripts/invoke-signed-release-dry-run.ps1`](../scripts/invoke-signed-release-dry-run.ps1) as the repo-owned release-manager probe for the `workflow_dispatch dry_run=true` evidence; it records workflow state, repository Actions permission state, dispatch attempt result, blocker class, and the run URL when GitHub accepts the dry run. `ENG-679` adds `SupplyChainEvidence.SignedReleaseDryRun` so the generated scorecard and release validation echo the current dry-run status, partial proof state, `dispatch-identity-actions-disabled` blocker, required command, output path, and handoff path without claiming the external identity blocker is resolved. `ENG-696` makes that handoff concrete as `artifacts/signed-release-dry-run/signed-release-dry-run-handoff.md`, generated beside the JSON report for the release manager who must enable or rerun with an Actions-enabled identity. Per-package nuget.org-side configuration (trusted-publishing policy pointing at this repo / this workflow / the tag pattern, plus `Cephalon.*` prefix reservation, plus `NUGET_USER` repository secret, plus the protected `CEPHALON_NUGET_TRUSTED_PUBLISHING_POLICY_CONFIRMED` and `CEPHALON_NUGET_PREFIX_RESERVATION_CONFIRMED` release-manager confirmation variables) must be in place before the first real tag push lands; the workflow itself ships ready to run but does not claim it can independently inspect nuget.org policy state or bypass an account-level Actions dispatch block. Follow-up: post-quantum hybrid signing (NIST ML-KEM / ML-DSA) once the .NET cryptography stack ships hybrid signing primitives in the `.NET 12` LTS lane; container base image signing propagation; integration testing the full flow once an Actions-enabled release-manager identity can create the first dry-run workflow run and the first `v*.*.*` tag is cut.

### `ENG-325` — `Cephalon.Analyzers` curated meta-package (Sprint 120)

*Shipped at `M2` adoption baseline.* New [`src/Cephalon.Analyzers/`](../src/Cephalon.Analyzers/) dependency-only meta-package bundles `Microsoft.CodeAnalysis.BannedApiAnalyzers`, `Microsoft.CodeAnalysis.PublicApiAnalyzers`, `Roslynator.Analyzers`, `Meziantou.Analyzer`, and `Microsoft.VisualStudio.Threading.Analyzers` (the SDK's `Microsoft.CodeAnalysis.NetAnalyzers` are inherited from the .NET 10 SDK and do not need a `PackageReference`). Each bundled analyzer flows transitively to consumers through `IncludeAssets="analyzers; build; buildtransitive"`, with PublicApiAnalyzers public-surface tracking suppressed by default until package authors set `<CephalonAnalyzersEnablePublicApiTracking>true</CephalonAnalyzersEnablePublicApiTracking>` and commit `PublicAPI.*.txt` files. The curated [`BannedSymbols.txt`](../src/Cephalon.Analyzers/BannedSymbols.txt) ships under `buildTransitive/` and is wired into the consumer's build automatically through [`Cephalon.Analyzers.props`](../src/Cephalon.Analyzers/buildTransitive/Cephalon.Analyzers.props); consumers opt out via `<CephalonAnalyzersUseBannedSymbols>false</CephalonAnalyzersUseBannedSymbols>`. The curated [`cephalon-analyzers.editorconfig`](../src/Cephalon.Analyzers/cephalon-analyzers.editorconfig) ships under `content/` for consumers to copy or include. `ENG-328` promoted the package to `M1` when `Cephalon.Abstractions` adopted the meta-package; `ENG-705` promoted it to `M2` by making `Cephalon.Analyzers` the default private analyzer package for generated `cephalon new` projects, all template-pack app/module starters, and the `cephalon doctor --app-root` generated-host package baseline. See [`docs/components/analyzers.md`](components/analyzers.md), [`getting-started.md`](getting-started.md), and `ENG-705` in [`engine-backlog.md`](engine-backlog.md). Further promotion requires scorecard or operator adoption gates with remediation guidance.
