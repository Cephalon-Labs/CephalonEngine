...
- `ENG-072` is now shipped: Neo4j and Qdrant now also expose provider-native `IEventDispatchStore` baselines for staged outboxes, so graph-store and vector-store workloads join the same consumer-managed or adapter-managed dispatch path without relaying through a relational bridge, while Cassandra, ClickHouse, and NATS remain explicit storage-model follow-through work rather than hidden parity gaps
- `ENG-073` is now shipped: NATS JetStream KV now also exposes a provider-native `IEventDispatchStore` baseline for staged outboxes by persisting mutable staged-message records in the same bucket, so ledger-store workloads join the same consumer-managed or adapter-managed dispatch path while Cassandra and ClickHouse remain explicit storage-model follow-through work
