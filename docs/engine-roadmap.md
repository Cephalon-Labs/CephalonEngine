...
- `Sprint 2`: exporter packaging is now part of the shipped phase-2 baseline, HTTP external API plus Redis/cache dependency-health packaging anchor the provider-specific follow-through, and broader dependency coverage remains next alongside `ENG-011`
...
1. operational hardening follow-through: broader dependency-health packs, readiness/liveness semantics, structured diagnostics, richer operator answers, and release-validation guidance now that the exporter path is shipped and HTTP plus Redis coverage have started
