...
- self-hosted collector and runtime defaults plus Azure Monitor, AWS, GCP, and Huawei Cloud are now shipped as the first slices on top of the cloud-neutral OTLP baseline, and Alibaba Cloud is now the next explicit child item while Cloudflare, DigitalOcean, Red Hat OpenShift, and VMware Tanzu stay later
...
- Alibaba Cloud companion follow-through is now the current next explicit child slice on top of the shared OpenTelemetry baseline
- additional cloud-targeted observability companion follow-through for Cloudflare, DigitalOcean, Red Hat OpenShift, and VMware Tanzu once each target is narrowed into its own child item
...
5. cloud and platform integrations, with self-hosted plus Azure Monitor plus AWS plus GCP plus Huawei Cloud shipped, Alibaba Cloud now active as the next explicit follow-up item, and Cloudflare plus DigitalOcean plus OpenShift plus Tanzu kept later
