...
- `Sprint 31`: **shipped** ENG-054 provider configuration follow-through — `Cephalon.Data.MongoDB` and `Cephalon.Data.Redis` now share the canonical `ConnectionStringName` plus `ConnectionString` options contract, resolve named values from the root `ConnectionStrings` section, fail fast when both settings are supplied, and remove host-owned `GetConnectionString(...)` plumbing from the showcase provider setup path
