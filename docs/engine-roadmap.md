...
- `ENG-052` follow-through now also lets hosts disable the built-in in-memory audit writer through `AuditRuntimeOptions.EnableInMemoryWriter` / `Engine:Audit:EnableInMemoryWriter`, and the runtime audit-store catalog now stays aligned with that choice instead of pretending the memory-backed store is active when it is not
