...
- `Sprint 2`: exporter packaging is now part of the shipped phase-2 baseline, HTTP external API plus MySQL, Postgres, RabbitMQ, Redis/cache, and SQL Server dependency-health packaging anchor the provider-specific follow-through, and the shared diagnostics/event-id catalog now anchors the structured diagnostics baseline
