...
- `ENG-058-T34` Scalar multi-document selector follow-through: `/scalar` now remains the selector-first Scalar entry point when multiple OpenAPI documents are configured, preserving the built-in version dropdown while `/scalar/v1`, `/scalar/v2`, and similar paths remain available as pinned-document deep links; hosting coverage and authoring docs now lock that behavior — composition HTTP tests 14/14 plus targeted hosting tests 2/2 (Sprint 31)
