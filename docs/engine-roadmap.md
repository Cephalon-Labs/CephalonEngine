...
- `Sprint 38–39 (Phase 12)`: planned migration and advanced coordination — the descriptor baseline for strangler fig plus BFF is now shipped, while saga choreography, feature flags, durable execution, and the runtime follow-through for migration/client-binding remain planned
...
- `strangler-fig` pattern descriptor and `IStranglerFigRouter` for incremental migration from legacy systems; the descriptor baseline is now shipped
