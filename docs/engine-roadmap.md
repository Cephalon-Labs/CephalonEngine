...
Planning baseline in this document reflects the repository state as of `April 9, 2026`.
...
- `ENG-058-T40` public REST documentation separation plus tag-metadata follow-through: generic behavior HTTP endpoints remain runnable but are now excluded from REST OpenAPI + Scalar descriptions by default, `MapBehaviorRestGroup(...)` can override tag names and descriptions explicitly, module XML `<summary>` plus `<remarks>` can now flow into top-level OpenAPI tag descriptions, and hosting/tooling coverage locks the public REST-vs-adapter-doc distinction — targeted hosting tests 3/3 plus package-surface tests 51/51 (Sprint 31)
- `ENG-058-T41` REST declaration-style cleanup plus explicit generic REST contracts: `ApiRoutes:Prefixes:Rest = ""` now intentionally mounts versioned REST routes at the root while `null` still falls back to `/api`; `[BehaviorAllowedTransports("http.rest")]` now acts as the annotation-driven generic REST activation path with fail-fast duplicate detection when `ConfigureTopology(...)` also declares `http.rest`; `Cephalon.Behaviors.Http` now exposes `ViaHttpRest(rest => ...)` so one behavior can choose a single generic REST method plus route/query remapping without leaking HTTP concerns into `Cephalon.Abstractions`; and source-generator/component-doc coverage now locks the runtime-fallback behavior for that richer REST contract — behavior source-generator tests plus composition HTTP tests (Sprint 31)
- `ENG-058-T42` attribute-only behavior baseline synthesis plus transport alias normalization: the runtime now synthesizes topology from `[BehaviorAllowedPatterns]` plus `[BehaviorAllowedTransports]` when exactly one pattern is declared and no explicit topology exists, ambiguous multi-pattern declarations fail fast with a clearer authoring error, `http.grpc` is accepted as an allowlist alias for canonical `grpc`, and composition/component-doc coverage now lock the attribute-only registration plus generic REST route behavior (Sprint 31)
