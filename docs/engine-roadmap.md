...
- self-hosted collector and runtime defaults plus Azure Monitor, AWS, GCP, and Huawei Cloud are now shipped as the first slices on top of the cloud-neutral OTLP baseline, while Alibaba Cloud, Cloudflare, DigitalOcean, Red Hat OpenShift, and VMware Tanzu stay later until the next explicit child item is chosen
...
5. cloud and platform integrations, with self-hosted plus Azure Monitor plus AWS plus GCP plus Huawei Cloud shipped and Alibaba Cloud plus Cloudflare plus DigitalOcean plus OpenShift plus Tanzu kept later until the next explicit follow-up item
