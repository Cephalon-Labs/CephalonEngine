...
- `Sprint 3`: runtime-answers follow-through, package distribution and trust follow-through, `ENG-013` planning readiness, the shipped `ENG-029` self-hosted OTLP follow-through slice, the shipped Azure Monitor first-vendor slice, and the AWS next-target split that keeps the next cloud-specific companion package explicit
...
- self-hosted collector and runtime defaults plus Azure Monitor are now shipped as the first slices on top of the cloud-neutral OTLP baseline, and AWS is now the next explicit vendor-specific follow-through while broader vendor/platform expansion stays behind additional child items
...
- AWS companion follow-through is the next explicit cloud-specific slice on top of the shared OpenTelemetry baseline
- additional cloud-targeted observability companion follow-through for GCP, Huawei Cloud, Alibaba Cloud, Red Hat OpenShift, and VMware Tanzu once each target is narrowed into its own child item
...
5. cloud and platform integrations, with self-hosted plus Azure Monitor shipped and AWS now the next explicit vendor-specific slice
