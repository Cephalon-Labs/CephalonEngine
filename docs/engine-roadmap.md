...
- `ENG-058-T32` behavior REST OpenAPI document selection follow-through: `BehaviorRestEndpointGroup.ApiVersion(major)` now assigns behavior REST endpoints to named OpenAPI documents, explicit API versions now override the operation-name version segment, `Cephalon.AspNetCore` now registers named docs from `OpenApi:Documents` while preserving the default `v1` document + Scalar redirect, and hosting coverage now locks `/openapi/v2.json` alongside the sample cart `.ApiVersion(1)` adoption — composition HTTP tests 13/13 plus hosting tests 77/77 (Sprint 31)
