...
## Modular-monolith REST/Worker/data adoption smoke

For a repo-native external-adoption replay that publishes a temporary package feed, installs `Cephalon.Cli`, scaffolds a modular-monolith REST host with the `CQRS` and `Outbox` data patterns, validates the generated data/Sfid package and split-configuration baseline, runs the ASP.NET Core host, probes `/health/ready`, `/engine`, `/engine/snapshot`, `/engine/dependencies`, `/engine/runtime-story`, and `/scalar`, then builds and runs a generic-host Worker probe from the same generated configuration, run:

```powershell
pwsh ./scripts/validate-modular-monolith-adoption.ps1
```

The script writes `artifacts/adoption-smoke/modular-monolith-adoption.json` by default; pass `-ReportPath <path>` when a release run needs the report in a different artifact folder. The adoption-smoke manifest carries this as the `modular-monolith-rest-worker-data` golden use case so scorecard and doctor readback can distinguish a REST + Worker + data-foundation proof from the basic generated host, template-pack, and staged-package proofs.
...
The same reports appear in `/engine/snapshot` under the versioned `ExtensionSections` entry whose id is `dependency-health`. Each entry exposes desired state `healthy`, the observed state, a normalized condition, and stable probe metadata. Its action list is empty today: observation is shipped, but automatic reconciliation/remediation is not yet claimed.
...
- `GET /engine/event-dispatch-remediation-commands/in-doubt?beforeUtc={beforeUtc}` returns retained
  ...
  return `400`.
- `GET /engine/event-dispatch-remediation-commands/in-doubt/summary?beforeUtc={beforeUtc}` returns
  the retained in-doubt roll-up for the same optional inclusive UTC cutoff, including reserved count
  and oldest retained reserved command posture, without materializing the list or using route paging.
...
- `GET /engine/event-dispatch-remediation-commands/outboxes/{outboxId}` filters command results by
  target outbox id
- `GET /engine/event-dispatch-remediation-commands/messages/{messageId}` filters command results by
  event message id
- `GET /engine/event-dispatch-remediation-commands/channels/{channelId}` filters command results by
  event channel id
...
- `GET /engine/event-dispatch-remediation-commands/reasons/{reason}` filters command results by
  the optional `operatorCommandReason` recorded with the accepted or rejected command
...
- `GET /engine/event-dispatch-remediation-commands/{filter}/{value}/latest` returns the newest
...
- `GET /engine/event-dispatch-remediation-commands/{filter}/{value}/oldest` returns the oldest
  ...
  correlation, reason, dispatch-outcome, and outcome filter families, and returns `404` when no
  retained command matches
- `GET /engine/event-dispatch-remediation-commands/{filter}/{value}/retention` returns retained
  command-history posture for the same outbox, message, channel, operation, actor, correlation,
  reason, dispatch-outcome, and outcome filter families, and returns an empty retention posture when
  no retained command matches
- list and filter command-result routes that return arrays reject invalid, zero, or negative
  ...
  filter-summary, filter-retention, filter-latest, filter-oldest, and single-command reads keep their existing direct contracts
...
- remediation command-result history is bounded and process-local by default; it gives operators a
  ...
  `commandListRoute`, `commandResultRoute`, `commandInDoubtRoute`,
  `commandInDoubtSummaryRoute`, `commandOldestInDoubtRoute`, and `commandOutboxRoute` metadata from
  `/engine/capabilities`, `/engine/technology-surfaces`, or `/engine/snapshot` to discover the root
list, in-doubt, in-doubt summary, oldest in-doubt, command-id, and outbox drill-down URLs without probing routes; use
  ...
  `commandInDoubtSummaryQuery`, `commandInDoubtSummaryPolicy`, and
  ...
  routes; use `commandFilterRetentionPolicy`, `commandFilterRetentionRoutes`,
  `commandFilterRetentionResponse`, and `commandFilterRetentionMaterialization` metadata to discover
  retained filter posture reads without materializing the detail list; use
  `commandFilterLatestPolicy`, `commandFilterLatestRoutes`,
...
`/engine/event-dispatch-remediation-commands/in-doubt?beforeUtc={beforeUtc}` when operators need
  ...
  `/engine/event-dispatch-remediation-commands/in-doubt/summary?beforeUtc={beforeUtc}` when
  ...
  `/engine/event-dispatch-remediation-commands/{filter}/{value}/retention` for retained-history
  posture over the same filter families without materializing the matching detail list; use
...
- `event-dispatch-remediation-commands` includes a catalog entry even when the command history is
  ...
  retention, in-doubt summary, filter-summary, filter-retention, filter-latest, filter-oldest, read-limit, observed-window, idempotency, provider-neutral, and Wolverine-free metadata
...
- `eventing-superiority-profile` exposes `durable-remediation-command-audit` and
  `durable-command-journal-replay-cursor` from the active command-journal descriptor; durable
  cross-node command audit is claimed only when the journal proves it, command-journal replay
...
- `broker-inbound-consumption-ownership` is the separate inbound-consumption boundary in that same
  profile; it stays `not-claimed` until a provider package owns a broker consumer loop, inbound
...
- `serialization-and-contract-versioning-ownership` is the separate wire-contract boundary in that
  ...
  envelope schema, and compatibility policy metadata, can report runtime serialization execution
  when a successful provider/runtime dispatch report supplies complete
  `EventDispatchSerializationExecutionMetadata` proof, and becomes `claimed` only when
  `EventDispatchWireContractMetadata` also supplies wire-envelope schema materialization,
  contract-version negotiation, and the final wire-contract proof id
