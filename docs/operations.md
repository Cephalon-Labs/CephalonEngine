...
- `Cephalon.MultiTenancy.Governance.SendGridDelivery.AspNetCore`: `4562-4564`
...
- `tenant-invitation-delivery-sendgrid-status-callbacks` reports the optional SendGrid ASP.NET Core callback translator's route/auth/body-limit/event-limit/translation/signature/replay posture, including safe signed-webhook verification metadata and bounded process-local signed-callback replay policy/key/scope/durability/retention/cache-limit truth without exposing SendGrid public keys, raw signatures, recipient emails, or raw payload bodies
...
- the package projects `tenant-invitation-delivery-sendgrid-status-callbacks` with signature verification and process-local replay posture; it still does not claim durable callback inboxes, distributed replay protection, provider polling, or exactly-once delivery
