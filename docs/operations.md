...
When enabled, retryable `sender-failed` dispatch outcomes are queued with a stable tenant/invitation/channel/sender boundary and can be replayed by calling `ITenantInvitationDeliveryRetryRunner.RetryPendingAsync(...)`. This is a local manual retry lane; schedule it from the host only when that is the desired operational model.
