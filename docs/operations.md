...
- `tenant-invitation-delivery-status-http-endpoints` reports the ASP.NET Core governance adapter's optional normalized callback endpoint posture, including route pattern, `POST` method, mapped/configured/disabled state, authorization requirement, optional policy, provider-message-match enforcement, endpoint-description visibility, provider-neutral callback signature verification configuration, safe signature header names, signing key-id configuration, timestamp tolerance, and application-managed boundaries for provider-specific payload translation, provider-specific signature verification, and provider polling
