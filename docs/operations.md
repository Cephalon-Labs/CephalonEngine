...
- `tenant-governance-actions` reports the governance companion's Cephalon-managed approval/remediation action catalog and decision posture, including action count, tenant count, contributor count, configured action count, decision ownership, status breakdown, action-kind breakdown, subject-kind breakdown, and contributing module ids without exposing individual action metadata
...
- the governance companion currently owns tenant membership cataloging/evaluation, tenant invitation cataloging/validation, declared tenant-domain ownership cataloging/validation, and approval/remediation action cataloging/decision
- DNS/HTTP domain verification execution, human approval workflow execution, remediation execution, durable membership/invitation/domain/action stores, invitation delivery, identity-provider synchronization, and backoffice/public-site tenant orchestration remain future companion work until a package owns those paths explicitly
