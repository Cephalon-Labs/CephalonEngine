...
- `cell-based-architecture` — Cell-Based Architecture (Platform): prepares the app for explicit module-owned cell boundaries, governed cell routes, blast-radius isolation, and later health-isolation or traffic-automation follow-through. Aliases: `CellBasedArchitecture`, `CellBased`, `Cells`.
