...
- `tenant-invitation-delivery-status-http-endpoints` technology surface — ASP.NET Core adapter posture for optional normalized delivery-status callback plus bounded observation read and filtered summary posture from `Cephalon.MultiTenancy.Governance.AspNetCore`
