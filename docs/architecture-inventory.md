...
- `tenant-invitation-delivery-status-http-endpoints` technology surface — ASP.NET Core adapter posture for optional normalized delivery-status callback plus bounded observation read, filtered summary posture, attention-category drill-down posture, remediation-action filter posture, and remediation-hint posture from `Cephalon.MultiTenancy.Governance.AspNetCore`
