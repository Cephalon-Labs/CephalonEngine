...
- `multi-tenancy-governance-http-delivery` — Multi-Tenancy Governance HTTP Delivery (`Cephalon.MultiTenancy.Governance.HttpDelivery`): optional provider-managed HTTP webhook sender for tenant-invitation delivery dispatch, including optional HMAC-SHA256 request signing and bounded in-process retry/backoff.
...
- `HttpInvitationDeliveryPayload` — JSON payload contract for the optional signed HTTP webhook invitation sender in `Cephalon.MultiTenancy.Governance.HttpDelivery`
