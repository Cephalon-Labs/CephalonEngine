...
- `MailgunInvitationDeliveryStatusCallbackResult` / `MailgunInvitationDeliveryStatusCallbackEventResult` — aggregate and per-event response contracts for accepted, skipped, and signed-webhook-verified Mailgun webhook translation in `Cephalon.MultiTenancy.Governance.MailgunDelivery.AspNetCore`
...
- `tenant-invitation-delivery-mailgun-status-callbacks` technology surface — ASP.NET Core Mailgun webhook translator and optional HMAC signature verification posture from `Cephalon.MultiTenancy.Governance.MailgunDelivery.AspNetCore`
...
- Multi-Tenancy Governance Mailgun Delivery ASP.NET Core (`Cephalon.MultiTenancy.Governance.MailgunDelivery.AspNetCore`) — event IDs 4568–4569
...
- `Cephalon.MultiTenancy.Governance.MailgunDelivery.AspNetCore` — ASP.NET Core Mailgun webhook translator and optional HMAC signed-webhook verifier for tenant-invitation delivery-status reconciliation
