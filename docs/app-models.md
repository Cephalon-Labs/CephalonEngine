...
The resolved app profile now also carries structured configuration selections for `Data`, `Databases`, `Identity`, `Tenancy`, `Audit`, `Messaging`, and `Resilience`. The new `Databases` block is the engine-owned physical-topology baseline for shared runtime tuning plus the first named roles (`Write`, `Read`, `Outbox`, `History`) and nested migration policy. `Audit.History` now also carries nested `Export` plus `Retention` inputs so the durable audit-history read and export surfaces stay visible in the same app-model contract. `Resilience` remains the contract-first phase-11 baseline for `Retry`, `Timeout`, `CircuitBreaker`, `Bulkhead`, and `RateLimiting` policy intent through both `/engine/app-model` and `/engine/resilience`. The ASP.NET Core endpoint-policy layer now projects effective public transport limiting through `RuntimeIntrospectionSnapshot.RateLimitingPolicies`, including request-response `grpc` coverage with gRPC-native `ResourceExhausted` rejection semantics; the gRPC adapter also enforces configured direct-module `Timeout`, `CircuitBreaker`, and `Bulkhead` policy for `IGrpcModule` calls, keeps those outcomes gRPC-native through `DeadlineExceeded` / `Unavailable` / `ResourceExhausted` responses with Cephalon metadata trailers, and reports the live `grpc-direct-module-resilience` posture through `/engine/technology-surfaces`. The JSON-RPC adapter now enforces the same configured direct-module timeout, circuit-breaker, and bulkhead posture for `IJsonRpcModule` endpoints, keeps those outcomes JSON-RPC-native through `-32053` / `-32029` error envelopes with HTTP `503` / `429` metadata, and reports the live `json-rpc-direct-module-resilience` posture through `/engine/technology-surfaces`. The base ASP.NET Core SSE and WebSocket mappers now enforce the same configured direct-module timeout, circuit-breaker, and bulkhead posture for `IServerSentEventsModule` and `IWebSocketModule` endpoints, keep those outcomes streaming-native through SSE `event: error` payloads and WebSocket text error frames, and report `sse-direct-module-resilience` plus `websocket-direct-module-resilience` through `/engine/technology-surfaces` with `wolverineRequired=false` and `consumerCodeRequired=false`, and publish cumulative direct-module timeout-occurrence, circuit-open transition, and circuit-open rejection counters with last-occurrence timestamps on those same streaming surfaces for operator observability. The GraphQL adapter now applies the same configured timeout, circuit-breaker, and bulkhead posture to built-in query, mutation, and subscription root-field execution, keeps outcomes GraphQL-native through `errors[].extensions`, and reports `graphql-execution-resilience` through `/engine/technology-surfaces/graphql` with `wolverineRequired=false` and `consumerCodeRequired=false`. The shipped behavior-execution follow-through also carries additive `BehaviorExecutionOverrides` plus effective retry, timeout, circuit-breaker, and bulkhead answers through `/engine/behavior-resilience` and `RuntimeIntrospectionSnapshot.BehaviorResiliencePolicies`, while the generic behavior HTTP adapters keep behavior-owned rate-limiting, bulkhead saturation, timeout, and open-circuit outcomes protocol-native; bulkhead saturation returns `429` metadata plus `behavior_execution_rejected`. Those overrides can now target behavior ids, transport ids, or both, and the runtime resolves them with behavior+transport precedence over behavior-only, transport-only, and default answers. The behavior contract layer now also lets authors declare replay safety explicitly through `BehaviorIdempotencyAttribute`, and `IBehaviorResilienceRuntimeCatalog.Resolve(...)` can reflect the resulting retry eligibility as `eligible`, `ineligible`, or `unknown` per behavior. Automatic retry execution now runs through the shared behavior pipeline with configured attempts, backoff, delays, and jitter only when the effective policy requests retry and the classifier sees an explicitly idempotent transient failure; non-idempotent or unknown behaviors still fail without automatic replay. Circuit breaker continues to participate in the same shared dispatch pipeline and publishes live runtime metadata such as circuit state and retry-after timing. `AppProfile.Databases` remains the requested topology selection, while the resolved operator-facing role truth now also lives beside it through `IDatabaseRoleCatalog`, `/engine/database-roles`, and `RuntimeIntrospectionSnapshot.DatabaseRoles`. The resolved operator-facing migration truth now lives beside it through `IDatabaseMigrationCatalog`, `/engine/database-migrations`, and `RuntimeIntrospectionSnapshot.DatabaseMigrations`. Legacy display-name aliases still resolve for compatibility, but the shipped starter and template surfaces now emit canonical kebab-case ids plus those structured sections by default.
...
`Engine:Technologies` selecting `event-driven-integration` activates the native `Cephalon.Eventing` package hints and scaffold wiring; generated hosts call `engine.AddEventingFromConfiguration(builder.Configuration)` so the selected app model can read `Engine:Messaging` channel and routing policy without handwritten host code. When the selected profile also includes `Outbox`, generated modules contribute a process-local starter `IOutbox` descriptor for the `application-events` channel so adoption replays can prove accepted outbox handoff from the generated app itself; durable broker dispatch remains a provider-pack concern. `Engine:Messaging:Provider = Wolverine` remains an explicit opt-in that adds `Cephalon.Eventing.Wolverine` and its provider-managed registration.
The native pack now reads `Engine:Messaging:Channels` through `engine.AddEventingFromConfiguration(configuration)` for environment-owned channel metadata, while subscription descriptors, descriptor-bearing executors, and direct execution middleware stay code-first through `EventingOptions.Subscriptions`, `IEventSubscriptionContributor`, `services.AddCephalonEventSubscriptionExecutor<TExecutor>()`, `EventSubscriptionAttribute`, optional `IEventSubscriptionDescriptorProvider`, and `services.AddCephalonEventSubscriptionExecutionMiddleware<TMiddleware>()`; direct `IEventSubscriptionExecutor` and `IEventSubscriptionExecutionMiddleware` registrations remain supported for hosts that need custom DI ownership. `Engine:Messaging:Subscriptions` and `Engine:Messaging:SubscriptionHandlers` are rejected so hot-path publish/subscribe behavior stays typed and fast. The same helper reads `Engine:Messaging:InProcessSubscriptions`, including `Idempotency:Store = inbox` for completed-execution duplicate suppression backed by exactly one registered `IInbox`; it also reads `Engine:Messaging:Publications:Routing` for provider-neutral event-type-to-channel publication routing and explicit-channel route validation, plus `Engine:Messaging:Publications:Scheduling` for bounded process-local delayed publication acceptance through `scheduledForUtc` or `delayMilliseconds` request metadata. The messaging model should keep evolving from a provider switch into a config-driven capability model for environment-owned policy and topology: provider, transport, routing, durability, delivery guarantee, retry/backoff, scheduling, dead-letter/replay, topology, tenant/correlation policy, observability, and compliance should be modeled as engine-owned selections first, then fulfilled by the native eventing pack or an optional provider companion. Wolverine parity is the minimum benchmark for those selections, not a reason to make Wolverine the default architecture.

Remediation command idempotency follows the same app-model split. Command execution remains code-owned by the dispatcher and mutable dispatch store, while the active command journal owns reservation, durability, and duplicate rejection. ENG-579 makes `reserve-before-mutation` the native default so apps do not need Wolverine, MassTransit, or NServiceBus package semantics to close duplicate operator-command retries.
