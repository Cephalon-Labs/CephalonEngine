...
The native pack now also reads `Engine:Messaging:InProcessSubscriptions` through `engine.AddEventingFromConfiguration(configuration)`, including `Idempotency:Store = inbox` for completed-execution duplicate suppression backed by exactly one registered `IInbox`; it also reads `Engine:Messaging:Publications:Scheduling` for bounded process-local delayed publication acceptance through `scheduledForUtc` or `delayMilliseconds` request metadata. The messaging model should keep evolving from a provider switch into a config-driven capability model: provider, transport, routing, durability, delivery guarantee, retry/backoff, scheduling, dead-letter/replay, topology, tenant/correlation policy, observability, and compliance should be modeled as engine-owned selections first, then fulfilled by the native eventing pack or an optional provider companion. Wolverine parity is the minimum benchmark for those selections, not a reason to make Wolverine the default architecture.
