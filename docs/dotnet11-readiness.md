...
- keeps source-generated and typed-contract remediation evidence separate from support promotion: `Cephalon.Behaviors` generated execution-slot hints now reduce source-generated dispatch startup reflection, `Cephalon.Behaviors.Patterns` generated `DurableExecutionSlot` registrations now reduce durable-execution adapter reflection for generated behaviors, and `Cephalon.Data.MySql` now removes hosted-service duck-typed failure metadata reflection through a typed internal contract, but global trim / Native AOT / single-file rows stay `not-claimed` while manifest-listed fallback, carrier-method, and third-party transport hazards remain
