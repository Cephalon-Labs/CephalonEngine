...
- keeps source-generated, registry-backed, and typed-contract remediation evidence separate from support promotion: `Cephalon.Behaviors` generated execution-slot hints now reduce source-generated dispatch startup reflection, `Cephalon.Behaviors.Patterns` generated `DurableExecutionSlot` registrations now reduce durable-execution adapter reflection for generated behaviors, the provider-native CDC hosted services now remove duck-typed failure metadata reflection through typed internal contracts, and the EventSourcing provider family now removes persisted type-name reflection through `IEventTypeRegistry`, but global trim / Native AOT / single-file rows stay `not-claimed` while manifest-listed fallback, carrier-method, and third-party transport hazards remain
