...
- **Known not-yet-classified hazard surface (`ENG-433` first-pass under broadened patterns).** The broadened `Grep` (May 5, 2026) surfaced additional `MakeGenericMethod` / `BindingFlags`-based hazards in `Cephalon.Behaviors` (`Modules/BehaviorModule.cs` lines 347/356/388/494; `Services/BehaviorExecutionSlot.cs` lines 73-74), `Cephalon.Behaviors.Http` beyond the four sites already classified by `ENG-432` (`Hosting/RestBehaviorProjection.cs` lines 43/207, `Hosting/RestBehaviorModuleBuilder.cs` line 230, `Hosting/BehaviorRestProfileResolver.cs` lines 155/192, `Abstractions/BehaviorRestMethodExtensions.cs` line 54, `Abstractions/BehaviorRestBindingSourceExtensions.cs` line 54), the CDC capture services in `Cephalon.Data.MySql` / `Cephalon.Data.Postgres` / `Cephalon.Data.Oracle` (each carries a `BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic` block plus `Cephalon.Data.MySql` accessing non-public members of the third-party `ReplicationClient` type), `Cephalon.Abstractions` (multiple enum-field `[EnumMember]` lookups via `typeof(EnumType).GetField(value.ToString())` across `RestEndpointOverrideBindingMode/Action/Status/SelectionBasis` plus `Cephalon.Behaviors.Http.Abstractions.BehaviorRestMethod/BindingSource`), and `Cephalon.ReferenceDocs` (`Generation/ReferenceDocsGenerator.cs` lines 295/301/308/313 enumerate constructors/fields/properties/methods on arbitrary types — by design for doc generation but trim-hostile). These are flagged here so a future audit slice can classify them; they are **not** yet recorded in the per-tier tables below to avoid landing classifications without a deeper read of each call site (the `ENG-432` precedent showed first-pass classifications materially understate hazards).
...
The remaining shipped `Cephalon.*` runtime packages — including (but not limited to) `Cephalon.Diagnostics`, `Cephalon.Resilience`, `Cephalon.AspNetCore`, `Cephalon.Worker`, `Cephalon.Eventing`, `Cephalon.Eventing.Wolverine`, `Cephalon.Agentics`, `Cephalon.Retrieval`, `Cephalon.Audit`, `Cephalon.Audit.EntityFramework`, `Cephalon.Identity`, `Cephalon.MultiTenancy`, the `Cephalon.MultiTenancy.Governance.*` companion family, the relational / non-relational `Cephalon.Data.*` *provider* family (the *core* `Cephalon.Data` package is now `medium` per the table above; the per-provider packs remain clean-baseline at the source-code layer modulo the CDC capture services flagged in *Scope and method*), the `Cephalon.Edge.*` family, the `Cephalon.Observability.*` family, `Cephalon.Cli`, and `Cephalon.Scaffolding` — show **no `Activator.CreateInstance`, `Type.GetType(string)`, `MakeGenericType`, or `Assembly.LoadFrom` use in their first-party source as of May 5, 2026**. The `Cephalon.Abstractions` enum-field lookups noted in *Scope and method* are excluded from `clean-baseline` pending a deeper-read classification.
