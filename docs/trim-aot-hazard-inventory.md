...
### `high` — additional `Cephalon.Behaviors` evidence: module-discovery pipeline + open-generic execution-slot dispatch (added via `ENG-436`)
...
- **`Modules/BehaviorModule.cs`** runs a two-phase auto-registration pipeline (line 302 `AutoRegisterBehaviors`). **Phase 1** (lines 331-406, `TrySourceGeneratedRegistration`) reads `assembly.GetCustomAttribute<ContainsBehaviorsAttribute>()` to find a source-generator-emitted registration class, then reflectively looks up and invokes its `Register` / `GetTopologyDescriptors` / `GetBehaviorsNeedingRuntimeTopology` methods by name; in isolation this is annotation-recoverable through `[DynamicallyAccessedMembers]` on the attribute's `RegistrationType` property. **Phase 2** (lines 416-466, `ReflectionScanAssembly`) is a canonical assembly-scan: `assembly.DefinedTypes` enumeration plus interface-shape checks plus `AppBehaviorAttribute` lookup. Both paths converge on `BuildTopologyDescriptorFromStaticMethod` (line 487) which reflectively looks up a static `ConfigureTopology` method on each behavior type. Because the phase-2 fallback exists, the package's overall blocker remains assembly scanning; remediation is the same source-gen-emitted compile-time module manifest + behavior-registration table that retires `BehaviorTypeRegistry` itself.
- **`Services/BehaviorExecutionSlot.cs`** materializes the per-behavior execution slot through open-generic *method* dispatch over the runtime `(behaviorType, TInput, TOutput)` triple discovered from the registry. The pattern is identical in shape to the `Cephalon.Behaviors.Http` `BehaviorRestEndpointGroup.cs:486` `MakeGenericMethod` that `ENG-432` reclassified — the difference is that `BehaviorExecutionSlot` consumes its input type from the assembly-scanned registry, so its remediation is part of the same source-gen-emitted dispatch table that retires `BehaviorTypeRegistry` rather than a separate per-behavior generator.
...
| --- | --- | --- | --- | --- |
| `Cephalon.Behaviors` | `Modules/BehaviorModule.cs` | 345 | `regType.GetMethod("Register", BindingFlags.Static \| BindingFlags.Public \| BindingFlags.NonPublic, null, [typeof(IServiceCollection), typeof(IBehaviorTypeRegistry)], null)` | Phase-1 reflective method lookup on a type provided through `ContainsBehaviorsAttribute.RegistrationType`. The matching `registerMethod?.Invoke(null, [services, typeRegistry])` follows on line 351. Annotation-recoverable in isolation via `[DynamicallyAccessedMembers]` on the attribute property; package tier still gated by phase-2. |
| `Cephalon.Behaviors` | `Modules/BehaviorModule.cs` | 354 | `regType.GetMethod("GetTopologyDescriptors", BindingFlags.Static \| BindingFlags.Public \| BindingFlags.NonPublic, null, Type.EmptyTypes, null)` | Phase-1 reflective lookup of the source-gen-emitted topology-descriptor enumerator. Matching `topologyMethod?.Invoke(null, null)` follows on line 360. Same annotation-recovery shape. |
| `Cephalon.Behaviors` | `Modules/BehaviorModule.cs` | 386 | `regType.GetMethod("GetBehaviorsNeedingRuntimeTopology", BindingFlags.Static \| BindingFlags.Public \| BindingFlags.NonPublic, null, Type.EmptyTypes, null)` | Phase-1 reflective lookup of the source-gen-emitted runtime-topology enumerator. Matching `runtimeTopologyMethod?.Invoke(null, null)` follows on line 392. Same annotation-recovery shape. |
| `Cephalon.Behaviors` | `Modules/BehaviorModule.cs` | 425 | `assembly.DefinedTypes.Select(ti => ti.AsType()).ToArray()` then `Attribute.GetCustomAttribute(type, typeof(AppBehaviorAttribute))` plus `type.GetInterfaces().FirstOrDefault(i => i.IsGenericType && i.GetGenericTypeDefinition() == typeof(IAppBehavior<,>))` | Phase-2 reflection-fallback assembly scan. Canonical assembly-scanning hazard — the dominant `high` driver in this file. Annotation alone is not sufficient; remediation is the same source-gen-emitted compile-time module manifest that retires `BehaviorTypeRegistry`. |
| `Cephalon.Behaviors` | `Modules/BehaviorModule.cs` | 491 | `behaviorType.GetMethod("ConfigureTopology", BindingFlags.Static \| BindingFlags.Public \| BindingFlags.FlattenHierarchy, null, [typeof(IBehaviorTopologyBuilder)], null)` | Reflective lookup of the optional static `ConfigureTopology` method on each behavior type. Matching `configMethod.Invoke(null, [builder])` follows on line 502. Reachable from both phase-1 and phase-2; the phase-2 reach makes this `high`. |
| `Cephalon.Behaviors` | `Services/BehaviorExecutionSlot.cs` | 73 | `typeof(BehaviorExecutionSlot).GetMethod(nameof(For), BindingFlags.Public \| BindingFlags.Static)!.MakeGenericMethod(behaviorType, tIn, tOut)` then `forMethod.Invoke(null, null)` (line 76) | Open-generic *method* dispatch over the runtime `(behaviorType, TInput, TOutput)` triple drawn from registry data. Same shape as `Cephalon.Behaviors.Http`'s `BehaviorRestEndpointGroup.cs:486` (reclassified `medium` by `ENG-432`); recorded here as `high` because `behaviorType` is sourced from the assembly-scanned registry rather than from a closed compile-time profile, and remediation is the same source-gen-emitted dispatch table that retires `BehaviorTypeRegistry`. |
...
**Update May 5, 2026 (`ENG-436`):** the `Cephalon.Behaviors` items flagged as not-yet-classified by `ENG-433` have now been classified through a deeper read of the actual call sites in `src/Cephalon.Behaviors/Modules/BehaviorModule.cs` and `src/Cephalon.Behaviors/Services/BehaviorExecutionSlot.cs`. Six additional `knownHazards` entries have been seeded into `scripts/deployment-mode-support.json` under the existing `Cephalon.Behaviors` package row (which was already `high` per `ENG-426`). The package's overall tier is unchanged because the new evidence sits inside an already-`high` package.

- `Modules/BehaviorModule.cs:345` — phase-1 reflective lookup of the source-gen-emitted `Register` method on `ContainsBehaviorsAttribute.RegistrationType` (matching `Invoke` on line 351). Annotation-recoverable in isolation.
- `Modules/BehaviorModule.cs:354` — phase-1 reflective lookup of `GetTopologyDescriptors` (matching `Invoke` on line 360). Same annotation-recovery shape.
- `Modules/BehaviorModule.cs:386` — phase-1 reflective lookup of `GetBehaviorsNeedingRuntimeTopology` (matching `Invoke` on line 392). Same annotation-recovery shape.
- `Modules/BehaviorModule.cs:425` — phase-2 reflection-fallback assembly scan (`assembly.DefinedTypes` + `Attribute.GetCustomAttribute(type, typeof(AppBehaviorAttribute))` + interface-shape check). Canonical assembly-scanning hazard; the dominant `high` driver in this file.
- `Modules/BehaviorModule.cs:491` — reflective lookup of static `ConfigureTopology` on each behavior type (matching `Invoke` on line 502). Reachable from both phases; the phase-2 reach makes it `high`.
- `Services/BehaviorExecutionSlot.cs:73` — open-generic *method* dispatch over the runtime `(behaviorType, TInput, TOutput)` triple (`MakeGenericMethod` plus `Invoke` on line 76). Same shape as `BehaviorRestEndpointGroup.cs:486` but `high` because `behaviorType` is sourced from the assembly-scanned registry.

Remediation across all six sites is uniform: a source-gen-emitted compile-time module manifest plus behavior-registration table that retires `BehaviorTypeRegistry`, the assembly-scan fallback in `BehaviorModule.cs`, and the open-generic dispatch in `BehaviorExecutionSlot.cs` together. Until that ships, `Cephalon.Behaviors` stays `high` and `not-claimed` for trim / Native AOT / single-file.

The aggregate posture table is unchanged: `4 high + 14 medium + 1 low + 2 excluded-by-design = 21 entries` (the package count does not move; only the per-package `knownHazards` list grows). The contract test suite at `tests/Cephalon.Tests.Scripts/deployment-mode-support-manifest.Tests.ps1` (31 tests after `ENG-431`) continues to pass against the seeded entries because the per-package shape, the path-truth assertions (`src/<packageName>/<packageName>.csproj` exists, first hazard `.cs` site exists for every appended `knownHazards.site`), and the `claimAuditTier` enum are all preserved. The harness publish surface, the deployment-mode claims (`not-claimed` for trim / nativeAot / singleFile), and the `representativePublishTargets.projects` value are unchanged by this slice. The line numbers used in the original `ENG-433` first-pass note (347/356/388/494) were one-line offsets from the actual `BindingFlags`-anchor lines in HEAD and have been corrected to 345/354/386/491 here and in the manifest.
