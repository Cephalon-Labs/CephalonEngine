...
### #5 — Provider-native CDC integration scenarios for `Cephalon.Data.SqlServer`, `Cephalon.Data.Postgres`, and `Cephalon.Data.MongoDB` (medium priority, **partially shipped through `ENG-487` / [PR #1091](https://github.com/Cephalon-Labs/CephalonEngine/pull/1091) and `ENG-490` / issue #1096**)

The shared CDC runtime catalog and the provider-native pumps (`sqlserver-cdc-capture-pump`, `postgresql-logical-replication-capture-pump`, `mongodb-change-stream-capture-pump`) are covered at the composition layer by `Cephalon.Tests.Composition`. `ENG-487` adds the first dedicated CDC integration lane, `tests/Cephalon.Tests.CdcIntegration`, and proves the MongoDB path against a disposable single-node replica set through `EphemeralMongo7`: real provider running, real change stream, real outbox staging, real runtime-state reporting, real execution-runtime aggregation, and real checkpoint commit. `ENG-490` adds the external-service gate for the same test project so SQL Server/Postgres live tests can be discovered but skipped by default, then opt into either pre-provisioned connection strings or Testcontainers-backed disposable services. The remaining gap is the provider-specific SQL Server/Postgres live CDC scenarios themselves.
