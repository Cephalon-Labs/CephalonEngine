...
- [ ] `SrePostureEvidence` reports expected guardrail and stable-baseline coverage: four benchmark-backed SLIs plus the deployment-mode claim-truth SLI are `stable-baseline-published`, the stable-baseline manifest has five rows / seven measurements / six pending baseline ids, guardrail-mapped SLIs list references that exist in `benchmarks/Cephalon.Benchmarks/guardrails/performance-guardrails.json`, the ASP.NET Core request-allocation SLI maps to the three `AspNetCoreRequestLoggingBenchmarks` entries, the deployment-mode claim-truth SLI maps to the release-validation claims-report gate/package-claim readback, and the remaining cold-start, wall-time, and flake-rate SLIs remain pending or not-applicable until their own stable-baseline lanes land
