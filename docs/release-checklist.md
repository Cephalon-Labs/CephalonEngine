...
3. **Tests** for `Cephalon.Tests.Composition` / `.Hosting` / `.Tooling` / `.CdcIntegration` / `.ProviderIntegration` (Pester suite for `tests/Cephalon.Tests.Scripts` runs separately in CI)
    ...
    6. **Reference-doc publishing** through `scripts/publish-reference-docs.ps1`; release validation records `artifacts/sre-release-validation/reference-docs-wall-time.json` before the scorecard step when this lane runs
    7. **Engine completion scorecard artifact** through `scripts/publish-engine-completion-scorecard.ps1`, including evidence-source references, per-package GA readiness rows, `DeploymentModeEvidence` from `scripts/deployment-mode-support.json` plus generated deployment-mode claims-report path/gate/target/warning/error/package-claim readback, adoption-smoke evidence from `scripts/adoption-smoke-support.json`, `ProviderIntegrationEvidence` from `scripts/provider-integration-support.json` plus provider live-lane runtime modes (pre-provisioned services or `CEPHALON_PROVIDER_TESTCONTAINERS=1` disposable services), dependency-health provider-family live managed-probe validation, and provider-manifest readback from `scripts/observability-dependency-health-providers.json`, `SrePostureEvidence` from `scripts/sre-posture-support.json` plus `scripts/sre-stable-baselines.json`, `SupplyChainEvidence` from `scripts/supply-chain-release-support.json` plus `.github/workflows/publish-release.yml`, including external-policy preflight checks, and `PublicApiCompatibilityEvidence` from the public API delta files; this step fails closed on scorecard hard blockers (blocked platform gates, supply-chain blocked evidence items, public API removals, invalid deployment-mode claims-report gate readback, missing dependency-health provider-manifest readback, SRE stable-baseline manifest drift, or dependency-health provider-row drift) while keeping `partial`, `not-claimed`, externally gated provider lanes, SRE pending baselines, and external-policy-pending items as explicit readback/exception posture; optionally run `cephalon doctor --scorecard artifacts/engine-completion-scorecard-release/engine-completion-scorecard.json` to get the local CLI summary, including deployment-mode global/package-scoped/hazard/publish-probe plus claims-report gate counts, provider integration row/live/composition/external-gate/default-skip/runtime-contract counts plus dependency-health provider-manifest readback, SRE target/baseline/stable-baseline-manifest/guardrail-coverage counts, supply-chain workflow/external-policy/preflight counts and status, and public API package/pending/addition/removal counts, over the generated artifact
    8. **Operational health and export conventions**
    9. **Phase-8 architecture, runtime, and starter conventions**
    10. **Benchmark smoke suite** + benchmark guardrail validation
    ...
    13. **Canonical validate-release wall-time** through `artifacts/sre-release-validation/validate-release-wall-time.json` when no release-validation skip switches changed the canonical full run
