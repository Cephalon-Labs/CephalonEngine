...
- [ ] `SrePostureEvidence` reports expected guardrail and stable-baseline coverage: four benchmark-backed hot-path SLIs plus the Worker cold-start mean-baseline proxy, deployment-mode claim-truth SLI, and restore/reference-docs/full validate-release wall-time SLIs are `stable-baseline-published`, the stable-baseline manifest has nine rows / eleven measurements / two pending baseline ids / two pending-baseline blocker rows, guardrail-mapped SLIs list references that exist in `benchmarks/Cephalon.Benchmarks/guardrails/performance-guardrails.json`, the ASP.NET Core request-allocation SLI maps to the three `AspNetCoreRequestLoggingBenchmarks` entries, the Worker cold-start SLI maps to `BuildStartWorkerHost`, the deployment-mode claim-truth SLI maps to the release-validation claims-report gate/package-claim readback, the restore, reference-docs, and full validate-release SLIs map to release-validation timing evidence, ASP.NET Core cold-start remains mapped to `ColdStartBenchmarks` as a regression cap with an `slo-target-miss` blocker, and flake-rate remains pending with a `ci-metadata-unavailable` blocker until GitHub Actions history exists
