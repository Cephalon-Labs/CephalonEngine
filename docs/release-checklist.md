...
- [ ] `SrePostureEvidence` reports expected guardrail and stable-baseline coverage: four benchmark-backed SLIs plus the deployment-mode claim-truth SLI and restore locked-mode wall-time SLI are `stable-baseline-published`, the stable-baseline manifest has six rows / eight measurements / five pending baseline ids, guardrail-mapped SLIs list references that exist in `benchmarks/Cephalon.Benchmarks/guardrails/performance-guardrails.json`, the ASP.NET Core request-allocation SLI maps to the three `AspNetCoreRequestLoggingBenchmarks` entries, the deployment-mode claim-truth SLI maps to the release-validation claims-report gate/package-claim readback, the restore SLI maps to release-validation timing evidence, both cold-start SLIs map to `ColdStartBenchmarks` guardrail rows as regression caps, and the remaining cold-start stable baselines, release/reference-docs wall-time, and flake-rate SLIs remain pending until their own stable-baseline lanes land
