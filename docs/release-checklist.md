...
- [ ] Run [`scripts/validate-release.ps1`](../scripts/validate-release.ps1) end-to-end on a clean workspace; it runs in order:
    ...
    6. **Engine completion scorecard artifact** through `scripts/publish-engine-completion-scorecard.ps1`, including evidence-source references, per-package GA readiness rows, adoption-smoke evidence from `scripts/adoption-smoke-support.json`, and `PublicApiCompatibilityEvidence` from the public API delta files; optionally run `cephalon doctor --scorecard artifacts/engine-completion-scorecard-release/engine-completion-scorecard.json` to get the local CLI summary, including public API package/pending/addition/removal counts, over the generated artifact
