...
- [ ] The harness's publish-probe lane reports the expected verdict for the manifest-declared `representativePublishTargets.projects`; `claim-validation-report.json` includes `PublishProbePolicy` and `PublishProbeGate` matching `publishProbePolicy`, and the default release path fails if the gated `singleFile` publish probe is skipped, targetless, failing, or warning-producing while global trim / Native AOT / single-file support rows remain `not-claimed`
