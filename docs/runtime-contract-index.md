...
| `GET /technology-surfaces` | `Cephalon.AspNetCore` | runtime technology capability surfaces (companion governance surfaces such as `tenant-memberships`, `tenant-invitations`, `tenant-domain-ownership`, `tenant-governance-actions`, and `tenant-administration` are projected here as `surfaceId` drill-downs rather than top-level `/engine/*` routes; eventing projects operator surfaces such as `event-dispatch-remediations` here; data providers also project `data-management` CDC surfaces such as `cdc-captures` and `cdc-capture-runtimes` here; observability exporter and logging packs project sanitized `telemetry-export-*` and `logging-provider-serilog` active-pack surfaces here; the gRPC adapter projects `grpc-direct-module-resilience` here when it enforces direct-module timeout, circuit-breaker, or bulkhead policy from `Engine:Resilience`; the JSON-RPC adapter projects `json-rpc-direct-module-resilience` here for the same direct-module timeout, circuit-breaker, and bulkhead policy; the base ASP.NET Core SSE and WebSocket mappers project `sse-direct-module-resilience` and `websocket-direct-module-resilience` here when the shared streaming endpoint filter enforces the same policy for direct module endpoints, including cumulative `timeoutOccurredCount` / `circuitOpenedCount` / `circuitRejectedWhileOpenCount` outcome counters with last-occurrence timestamps for operator observability; the GraphQL adapter projects `graphql-execution-resilience` under the `graphql` technology id when Hot Chocolate field middleware enforces the same policy for built-in query, mutation, and subscription root fields) | optional |
