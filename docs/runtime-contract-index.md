...
| `GET /technology-surfaces` | `Cephalon.AspNetCore` | runtime technology capability surfaces (companion governance surfaces such as `tenant-memberships`, `tenant-invitations`, `tenant-domain-ownership`, `tenant-governance-actions`, and `tenant-administration` are projected here as `surfaceId` drill-downs rather than top-level `/engine/*` routes; eventing projects operator surfaces such as `event-dispatch-remediations` here; data providers also project `data-management` CDC surfaces such as `cdc-captures` and `cdc-capture-runtimes` here; observability exporter and logging packs project sanitized `telemetry-export-*` and `logging-provider-serilog` active-pack surfaces here; the gRPC adapter projects `grpc-direct-module-resilience` here when it enforces direct-module timeout, circuit-breaker, or bulkhead policy from `Engine:Resilience`; the JSON-RPC adapter projects `json-rpc-direct-module-resilience` here for the same direct-module timeout, circuit-breaker, and bulkhead policy, including cumulative `timeoutOccurredCount` / `circuitOpenedCount` / `circuitRejectedWhileOpenCount` outcome counters with last-occurrence timestamps for operator observability; the base ASP.NET Core SSE and WebSocket mappers project `sse-direct-module-resilience` and `websocket-direct-module-resilience` here when the shared streaming endpoint filter enforces the same policy for direct module endpoints; the GraphQL adapter projects `graphql-execution-resilience` under the `graphql` technology id when Hot Chocolate field middleware enforces the same policy for built-in query, mutation, and subscription root fields) | optional |
...
ENG-633 adds the matching provider-reported proof lane. Runtime consumers can now distinguish dispatch-store terminal intent from complete broker DLQ/replay ownership because `EventDispatchBrokerDeadLetterReplayMetadata` adds `brokerDeadLetterReplayOwnership=provider-reported`, proof source, broker dead-letter queue id, replay action catalog id, replay cursor id, purge/quarantine id, and provider proof id only when a failed dispatch report carries complete provider proof; `broker-dead-letter-replay-ownership` becomes claimed only from that complete failed-dispatch proof set.
...
ENG-631 adds provider-reported partition ownership proof beside the topology proof. Runtime consumers can now distinguish route/channel governance, accepted outbox handoff, broker topology materialization, dispatch reports, and optional Wolverine dispatch from complete provider partition ownership because `EventDispatchProviderPartitionMetadata` adds `providerPartitionOwnership=provider-reported`, proof source, partition assignment id, partition affinity id, partition rebalancing id, partition ordering guarantee id, and provider partitioning id only when a successful dispatch report carries provider proof; `provider-partition-ownership` becomes claimed only from that complete proof set.
...
ENG-611 adds `IEventSchemaRegistryCatalog`, `event-schema-registries`, and `eventing.schema-registries` as the descriptor-backed schema-registry catalog layer under that boundary. Runtime consumers can now distinguish serializer descriptors whose schema registry ids resolve against cataloged registry availability from unresolved registry references, while executable schema lookup, executable payload serialization, upcaster execution, and provider-owned compatibility validation remain unclaimed until a provider or engine package owns those paths.

ENG-612 adds `IEventUpcasterCatalog`, `event-upcasters`, and `eventing.upcasters` as the descriptor-backed upcaster catalog layer under that boundary. Runtime consumers can now distinguish declared event version transitions whose source and target contracts resolve against cataloged contract metadata from unresolved upcaster references, while executable upcaster execution, executable payload serialization, executable schema lookup, and provider-owned compatibility validation remain unclaimed until a provider or engine package owns those paths.

ENG-634 adds the matching executable proof lane. Runtime consumers can now distinguish descriptor-backed contract, serializer, schema-registry, and upcaster catalog evidence from executable serialization ownership because `EventDispatchSerializationExecutionMetadata` adds `serializationExecutionOwnership=provider-reported`, proof source, executable payload serialization id, executable schema lookup id, executable upcaster execution id, executable compatibility validation id, and provider serialization id only when a successful dispatch report carries provider proof. `EventDispatchWireContractMetadata` then adds wire-contract ownership, wire-envelope schema materialization, contract-version negotiation, and the final wire-contract proof id; `serialization-and-contract-versioning-ownership` becomes claimed only from that complete successful-dispatch wire-contract proof set.
...
ENG-616 adds dispatch-store-read context evidence under that same boundary. Runtime consumers can now distinguish outbox-stage context proof from a pending dispatch-store read because active `IEventDispatchStore.ReadPendingAsync(...)` implementations return `EventDispatchItem` headers and `EventContextHandoffMetadataKeys` metadata that match the staged outbox proof, and `tenant-and-correlation-context-ownership` reports `durableDispatchContextPropagation=dispatch-store-read` only when the outbox handoff plus an active dispatch store are present.

ENG-617 adds dispatch-report context evidence under that same boundary. Runtime consumers can now distinguish pending-read proof from live dispatch-report proof because `EventDispatchContextReportMetadata` preserves staged `EventDispatchItem.Metadata`, adds `EventDispatchRuntimeMetadataKeys` boundary markers, and lets `IEventDispatchRuntimeCatalog` project `durableDispatchContextPropagation=dispatch-report-metadata` and `dispatchReportContextMetadata=reported` only when a dispatcher actually reports that metadata; provider/broker headers, consumer-side extraction, delivery completion, and cross-node context handoff remain unclaimed.

ENG-618 adds provider/broker context header projection under that same boundary. Runtime consumers can now distinguish dispatch-report proof from stable provider/broker handoff headers because `EventDispatchProviderBrokerContextHeaders` projects staged `EventDispatchItem` context into `EventContextHeaderNames`, optional Wolverine dispatch includes those headers in `DeliveryOptions`, and `tenant-and-correlation-context-ownership` reports `providerBrokerContextHeaders=projected` only when live dispatch runtime state contains that projection proof.

ENG-619 adds consumer-side context extraction under that same boundary. Runtime consumers can now distinguish provider/broker handoff headers from subscription execution extraction because `EventConsumerContextExtractor` records `consumerContextExtraction`, extraction source, header count, and header names on native in-process and optional Wolverine-managed subscription runtime observations; provider-side persistence, delivery completion, and cross-node context handoff stay separate proof boundaries.

ENG-620 adds provider-side context persistence under that same boundary. Runtime consumers can now distinguish provider/broker header projection from durable dispatch-store persistence because `EventDispatchProviderContextPersistenceMetadata` adds `providerSideContextPersistence`, persistence source, header count, and header names only when projected Cephalon context headers exist, `IEventDispatchProviderContextPersistenceStore` marks stores that can report the proof after `ApplyReportAsync(...)`, `Cephalon.Data.EntityFramework` persists the proof into outbox metadata, and optional Wolverine dispatch reports the enriched metadata only when the active store implements that contract; delivery completion and cross-node context handoff remain separate proof boundaries.

ENG-621 adds provider-reported cross-node context handoff under that same boundary. Runtime consumers can now distinguish provider-side context persistence plus local consumer extraction from a provider/runtime-reported cross-node handoff because `EventDispatchCrossNodeContextHandoffMetadata` adds `crossNodeContextHandoff=provider-reported`, source, producer node, consumer node, header count, and header names only when persisted dispatch proof, consumer extraction metadata, and distinct producer/consumer node ids are present; downstream delivery completion, destination commits, and exactly-once delivery remain unclaimed.

ENG-622 adds provider-reported downstream delivery completion under that same Eventing honesty boundary. Runtime consumers can now distinguish plain dispatch success from provider-reported completion because `EventDispatchDeliveryCompletionMetadata` adds `downstreamDeliveryCompletion=provider-reported`, completion source, provider receipt id, optional subscriber acknowledgement id, optional destination commit id, and `exactlyOnceDelivery=not-claimed` only when a successful dispatch report carries provider proof; destination commits and exactly-once guarantees remain separate stronger claims unless supplied by a future provider-owned proof path.

ENG-623 adds provider-proven exactly-once delivery under that same boundary. Runtime consumers can now distinguish partial provider completion from a complete provider exactly-once proof because `EventDispatchExactlyOnceDeliveryProofMetadata` adds `exactlyOnceDelivery=provider-proven`, proof source, proof id, and optional strategy only when successful dispatch metadata also supplies provider receipt, subscriber acknowledgement, and destination commit ids; `downstream-delivery-completion-ownership` becomes claimed only from that complete proof set.

ENG-624 adds provider-reported broker inbound-consumption proof under that same Eventing honesty boundary. Runtime consumers can now distinguish declared subscriptions, direct execution, hosted bindings, inbox links, and optional provider bindings from a complete broker consumer proof because `EventSubscriptionBrokerInboundConsumptionMetadata` adds `brokerInboundConsumption=provider-reported`, proof source, consumer-loop id, acknowledgement id, lease id, retry policy, poison-message handling, and offset-checkpoint id only when a successful subscription report carries provider proof; `broker-inbound-consumption-ownership` becomes claimed only from that complete proof set.

ENG-625 adds provider-reported durable retry queue proof under that same Eventing honesty boundary. Runtime consumers can now distinguish bounded retry observations from a complete durable retry queue proof because `EventDispatchDurableRetryQueueMetadata` adds `durableRetryQueue=provider-reported`, proof source, durable retry queue id, retry persistence id, broker error queue id, poison queue id, retry coordination id, and retry lease id only when a retry-scheduled dispatch report carries provider proof; `durable-retry-queue-ownership` becomes claimed only from that complete proof set.

ENG-626 adds provider-reported subscription idempotency proof under that same Eventing honesty boundary. Runtime consumers can now distinguish process-local/inbox duplicate-completed suppression from a complete provider idempotency proof because `EventSubscriptionProviderIdempotencyMetadata` adds `providerIdempotency=provider-reported`, proof source, provider idempotency key, broker deduplication id, exactly-once proof id, durable inbox command id, generic inbox command id, and cross-node idempotency lease id only when a successful subscription report carries provider proof; `idempotency-ownership` becomes claimed only from that complete proof set.

ENG-627 adds provider-reported subscription concurrency proof under that same Eventing honesty boundary. Runtime consumers can now distinguish declared subscriptions, direct execution, middleware, hosted links, and provider bindings from a complete provider concurrency proof because `EventSubscriptionConcurrencyMetadata` adds `subscriptionConcurrency=provider-reported`, proof source, positive per-subscription concurrency limit, parallel handler execution, positive consumer prefetch count, backpressure strategy, provider concurrency id, consumer lease id, work-stealing id, and distributed work-sharing id only when a successful subscription report carries provider proof; `subscription-concurrency-ownership` becomes claimed only from that complete proof set.

ENG-628 adds provider-reported subscription ordering proof under that same Eventing honesty boundary. Runtime consumers can now distinguish declared subscriptions, direct execution, middleware, hosted links, and provider bindings from a complete provider ordering proof because `EventSubscriptionOrderingMetadata` adds `subscriptionOrdering=provider-reported`, proof source, handler ordering guarantee id, local fan-out ordering id, per-key ordering key, partition ordering id, causal ordering id, replay ordering cursor id, cross-node ordering id, and provider ordering id only when a successful subscription report carries provider proof; `subscription-ordering-ownership` becomes claimed only from that complete proof set.

ENG-629 adds provider-reported process-manager state proof under that same Eventing honesty boundary. Runtime consumers can now distinguish declared subscriptions, direct execution, middleware, choreography handoff, outbox publication, hosted links, and provider bindings from a complete provider process-manager proof because `EventSubscriptionProcessManagerStateMetadata` adds `processManagerState=provider-reported`, proof source, saga state persistence id, saga correlation id, timeout scheduler id, compensation workflow id, process-manager concurrency id, process-manager recovery id, and provider process-manager id only when a successful subscription report carries provider proof; `process-manager-state-ownership` becomes claimed only from that complete proof set.
...
ENG-632 adds provider-reported scheduled/delayed delivery proof under that Eventing boundary. Runtime consumers can now distinguish bounded local scheduling from complete provider scheduling because `EventDispatchScheduledDeliveryMetadata` adds `scheduledDeliveryOwnership=provider-reported`, proof source, durable scheduled delivery id, provider delay queue id, broker scheduled delivery id, schedule coordination id, and schedule recovery id only when a successful dispatch report carries provider proof; `scheduled-and-delayed-delivery-ownership` becomes claimed only from that complete proof set.

ENG-603 adds `durable-retry-queue-ownership` to the same profile so runtime consumers can see that bounded in-process retry, dispatch runtime reports, and provider-managed retry observations are separate from durable retry queues, retry persistence, broker error queues, poison queue ownership, cross-node retry coordination, and retry leases; ENG-625 updates that dimension so it can move from `not-claimed` to `claimed` only from complete live provider proof.

ENG-604 adds `idempotency-ownership` to the same profile so runtime consumers can see that process-local or inbox-backed completed-execution duplicate suppression is separate from broker deduplication, exactly-once delivery, durable inbox command ownership, generic inbox command ownership, provider idempotency semantics, and cross-node idempotency leases; ENG-626 updates that dimension so it can move from `partial` to `claimed` only from complete live provider proof.

ENG-605 adds `subscription-concurrency-ownership` to the same profile so runtime consumers can see that declared subscriptions, in-process execution, code-first middleware, hosted bindings, and optional provider bindings are separate from per-subscription concurrency limits, parallel handler execution, consumer prefetch, backpressure, provider concurrency, consumer leases, work stealing, and distributed work sharing; ENG-627 updates that dimension so it can move from `not-claimed` to `claimed` only from complete live provider concurrency proof.

ENG-606 adds `subscription-ordering-ownership` to the same profile so runtime consumers can see that declared subscriptions, in-process execution, code-first middleware, hosted bindings, and optional provider bindings are separate from handler ordering, local fan-out ordering, per-key ordering, partition ordering, causal ordering, replay ordering, cross-node ordering, and provider ordering; ENG-628 updates that dimension so it can move from `not-claimed` to `claimed` only from complete live provider ordering proof.

ENG-607 adds `process-manager-state-ownership` to the same profile so runtime consumers can see that declared subscriptions, in-process execution, code-first middleware, choreography bridge handoff, outbox publication, hosted bindings, and optional provider bindings are separate from durable saga/process-manager state, correlation, timeout scheduling, compensation workflow, concurrency, and recovery; ENG-629 updates that dimension so it can move from `not-claimed` to `claimed` only from complete live provider process-manager state proof.
