...
ENG-620 adds provider-side context persistence under that same boundary. Runtime consumers can now distinguish provider/broker header projection from durable dispatch-store persistence because `EventDispatchProviderContextPersistenceMetadata` adds `providerSideContextPersistence`, persistence source, header count, and header names only when projected Cephalon context headers exist, `IEventDispatchProviderContextPersistenceStore` marks stores that can report the proof after `ApplyReportAsync(...)`, `Cephalon.Data.EntityFramework` persists the proof into outbox metadata, and optional Wolverine dispatch reports the enriched metadata only when the active store implements that contract; delivery completion and cross-node context handoff remain unclaimed.
