...
ENG-634 adds the matching provider-reported executable proof lane. Runtime consumers can now distinguish descriptor-backed contract, serializer, schema-registry, and upcaster catalog evidence from complete executable serialization ownership because `EventDispatchSerializationExecutionMetadata` adds `serializationExecutionOwnership=provider-reported`, proof source, executable payload serialization id, executable schema lookup id, executable upcaster execution id, executable compatibility validation id, and provider serialization id only when a successful dispatch report carries provider proof. `EventDispatchWireContractMetadata` adds the complementary `wireContractOwnership=provider-reported`, proof source, executable payload serialization id, wire-envelope schema id, schema lookup id, contract-version negotiation id, upcaster execution id, compatibility validation id, and a single wire-contract proof id only when a successful dispatch report carries provider proof; `serialization-and-contract-versioning-ownership` becomes claimed only from either complete successful-dispatch proof set.
