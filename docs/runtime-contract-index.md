...
| `GET /event-dispatch-remediation-commands*` | `Cephalon.AspNetCore` | bounded event-dispatch remediation command results with command-id, outbox, message, channel, operation, actor, correlation-id, dispatch-outcome, command-outcome, and duplicate-command recovery drill-downs | optional |
