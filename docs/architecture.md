...
- lets modules and hosts declare outbox-linked CDC posture through `ICdcCaptureContributor`, `ICdcCaptureCatalog`, and `engine.AddCdcCapture(...)` without turning provider-specific change-stream execution into a host-only registry
...
- module-owned CDC captures can now be inspected through `ICdcCaptureCatalog`, `/engine/cdc-captures`, and `/engine/snapshot`, with explicit `OutboxId` linkage and validation against active source modules and outboxes instead of a host-only sync registry
