...
ENG-634 records provider-reported serialization execution ownership through `EventDispatchSerializationExecutionMetadata` only when successful dispatch metadata includes executable payload serialization, executable schema lookup, executable upcaster execution, executable compatibility validation, and provider serialization proof ids, and provider-reported wire-contract proof through `EventDispatchWireContractMetadata` only when successful dispatch metadata includes executable payload serialization, wire-envelope schema, schema lookup, contract-version negotiation, upcaster execution, compatibility validation, and a single wire-contract proof id. `serialization-and-contract-versioning-ownership` now separates descriptor-backed contract, serializer, schema-registry, and upcaster catalog evidence from executable serialization and wire-contract ownership: the dimension stays `partial` for descriptor catalogs and becomes `claimed` only when one of those complete successful-dispatch provider proofs is present, and Wolverine remains an optional companion rather than the serialization contract.
