...
`Cephalon.Agentics` owns a narrow dispatcher/run-state lane for registered tools, including explicitly configured bounded process-local retry, process-local duplicate-completed suppression, and opt-in active `IInbox`-backed completed-run markers for `toolId + runId`, while autonomous planning, memory persistence, durable retry queues, provider-owned durable inbox command processing beyond that marker, cross-node exactly-once delivery, distributed schedulers, and provider-specific AI orchestration stay outside the current proof. `Cephalon.MultiTenancy` keeps tenant resolution in the base pack while `Cephalon.MultiTenancy.Governance` owns concrete governance proofs for membership, invitations, delivery dispatch/retry/status reconciliation, delivery-status observation storage, tenant administration, domain-ownership verification, proof collection, polling, and governance actions. The optional ASP.NET Core, HTTP delivery, SMTP delivery, SendGrid delivery, Mailgun delivery, Amazon SES delivery, Microsoft Graph delivery, Microsoft Graph Azure Identity, SendGrid delivery ASP.NET Core, Mailgun delivery ASP.NET Core, and Amazon SES delivery ASP.NET Core companions keep routing, provider sender handoff, provider token acquisition, provider payload translation, optional SendGrid signed-webhook verification, bounded process-local signed-callback replay protection, observation-store-backed SendGrid event-id idempotency, optional Mailgun HMAC signed-webhook verification, bounded process-local Mailgun replay-token rejection, observation-store-backed Mailgun event-id idempotency, SNS-wrapped SES event translation, opt-in SNS signature verification, bounded process-local SNS replay protection, observation-store-backed Amazon SNS message-id idempotency, opt-in verified SNS subscription confirmation, and opt-in verified SNS unsubscribe-confirmation observation additive without moving notification-provider APIs into the governance core. Actual DNS proof publication, provider-backed proof mutation, distributed/provider-backed governance storage, Microsoft Entra app registration/consent/mailbox access policy, additional provider-specific email APIs beyond the shipped SMTP/SendGrid/Mailgun/Amazon SES/Microsoft Graph senders, SMS/chat/CRM/identity-provider senders, distributed retry queues, cross-node retry leases, identity-provider synchronization, provider-specific or distributed callback inboxes, distributed callback replay ledgers, distributed event-id ledgers, automatic resubscribe/restore, subscription lifecycle governance, callback translation beyond shipped SendGrid/Mailgun/Amazon SES translators, provider-specific callback signature semantics beyond shipped SendGrid/Mailgun/Amazon SNS hardening, provider polling, public onboarding, and tenant-admin UI/backoffice flows stay explicitly outside the current proof until a package owns them.

`Cephalon.Eventing` follows the same honesty rule: its direct in-process subscription lane can retry transient handler failures inside a bounded process-local attempt budget, can suppress duplicate completed subscription executions inside a bounded process-local retention window, can accept delayed publication requests through a bounded process-local scheduler, can enforce code-first context policy headers before direct execution or outbox enqueue, can stage validated context proof metadata into outbox messages, can expose that staged context proof through provider-neutral dispatch-store pending reads, can carry the same context proof into dispatch runtime report metadata, can project the dispatch-item context into stable provider/broker handoff headers, can persist provider-side context proof when a capable dispatch store applies the report, can extract consumer-side context into subscription execution metadata, can record provider-reported cross-node context handoff only when persisted dispatch proof, consumer extraction, and distinct producer/consumer nodes are present, can record provider-reported downstream delivery completion only when a successful dispatch report includes a provider receipt, can record provider-proven exactly-once delivery only when a successful dispatch report includes provider receipt, subscriber acknowledgement, destination commit, and exactly-once proof ids, can record provider-reported broker inbound consumption only when a successful subscription report includes consumer-loop, acknowledgement, lease, retry/poison, and offset-checkpoint proof, can record provider-reported broker dead-letter/replay ownership only when a failed dispatch report includes broker DLQ, replay action catalog, replay cursor, purge/quarantine, and provider proof ids, can record provider-reported durable retry queue ownership only when a retry-scheduled dispatch report includes durable retry queue, retry persistence, broker error queue, poison queue, cross-node coordination, and retry lease proof, can record provider-reported subscription idempotency only when a successful subscription report includes provider idempotency, broker deduplication, exactly-once processing, inbox command, and cross-node lease proof ids, can record provider-reported subscription concurrency only when a successful subscription report includes concurrency limit, parallel handler execution, prefetch, backpressure, provider concurrency, consumer lease, work-stealing, and distributed work-sharing proof ids, and can report latest publication runtime state for local direct execution, scheduled local handoff, or outbox `accepted` handoff. Event-dispatch remediation command idempotency and operator reads now flow through the provider-neutral `IEventDispatchRemediationCommandJournal`; the core pack provides a process-local fallback, while `Cephalon.Data.EntityFramework` can supply durable cross-node command history when the write-side `DbContext` opts into the journal context and outbox mapping. EF also implements `IEventDispatchRemediationCommandReplayCursorCatalog`, giving provider code a durable command-journal replay cursor over stable `(ObservedAtUtc, CommandId)` positions without turning ASP.NET Core paging tokens into replay state. Code-first `EventContractDescriptor`, `EventSerializerDescriptor`, `EventSchemaRegistryDescriptor`, and `EventContextPolicyDescriptor` registrations now flow through `IEventContractCatalog`, `IEventSerializerCatalog`, `IEventSchemaRegistryCatalog`, `IEventContextPolicyCatalog`, `event-contracts`, `event-serializers`, `event-schema-registries`, `event-context-policies`, `eventing.contracts`, `eventing.serializers`, `eventing.schema-registries`, and `eventing.context-policies`, giving wire-contract and context-profile dimensions descriptor-backed partial evidence without making Wolverine or string-configured handler binding required. `eventing-superiority-profile` now mirrors that active command-journal descriptor through `durable-remediation-command-audit` and `durable-command-journal-replay-cursor`, while `broker-dead-letter-replay-ownership`, `broker-topology-materialization-ownership`, `provider-partition-ownership`, `downstream-delivery-completion-ownership`, `broker-inbound-consumption-ownership`, `serialization-and-contract-versioning-ownership`, `tenant-and-correlation-context-ownership`, `scheduled-and-delayed-delivery-ownership`, `durable-retry-queue-ownership`, `idempotency-ownership`, and `subscription-concurrency-ownership` keep broker DLQ/replay, broker topology, provider partition ownership, downstream delivery completion, inbound broker consumption, event wire-contract ownership, context propagation ownership, durable/provider scheduler ownership, durable retry ownership, broader idempotency ownership, and subscription concurrency ownership separate until complete live provider proof exists. Broker-owned retries without complete provider proof, durable retry queues without complete provider proof, provider idempotency without complete subscription proof, subscription concurrency without complete provider proof, durable inbox ownership beyond explicit `IInbox` paths, durable scheduled delivery, provider delay queues, broker scheduled delivery, cross-node schedule coordination, schedule recovery, durable or distributed scheduling, downstream dispatch completion beyond provider-reported proofs, inbound broker consumption beyond complete provider subscription proof, broker topology materialization, provider partition assignment/rebalancing/ordering, broker replay cursors without complete failed-dispatch proof, executable payload serialization, executable schema registry lookup, upcasters, provider-side context persistence beyond capable dispatch-store proofs, and cross-node context propagation beyond provider-reported handoff proof stay outside the core pack until a provider or companion package truly owns them. `Cephalon.Eventing.Behaviors` now sits beside that baseline as an explicit bridge companion pack: it lets behavior-authored saga choreography hand off into the shared eventing publish path without making either baseline package absorb the other's contract ownership.

ENG-613 extends that metadata lane with code-first `EventContextPolicyDescriptor` registrations, `IEventContextPolicyCatalog`, the `event-context-policies` technology surface, and the `eventing.context-policies` capability. ENG-614 then makes the publisher side executable: `EventContextHeaderNames` supplies stable Cephalon headers, native publisher paths reject missing required headers when validating policies declare them, and runtime metadata records publisher-enforced validation plus direct in-process tenant/correlation/causation/baggage forwarding evidence. ENG-615 carries that proof into the native outbox handoff: `EventContextHandoffMetadataKeys` names the staged context metadata, outbox-backed publisher messages preserve required/present header proof plus propagation posture, and publication runtime metadata reports `outboxContextHandoff = staged-headers`. ENG-616 proves the next native boundary by reading the same headers and staged metadata back through provider-neutral `IEventDispatchStore.ReadPendingAsync(...)` and reporting `durableDispatchContextPropagation=dispatch-store-read`. ENG-617 carries the same proof into dispatch runtime reports through `EventDispatchContextReportMetadata` and reports `durableDispatchContextPropagation=dispatch-report-metadata` only when live `IEventDispatchRuntimeCatalog` state proves the report metadata exists. ENG-618 projects that same dispatch-item proof into stable provider/broker handoff headers through `EventDispatchProviderBrokerContextHeaders`, lets optional Wolverine dispatch include those headers in `DeliveryOptions`, and advances only `providerBrokerContextHeaders=projected`. ENG-619 extracts consumer-side context through `EventConsumerContextExtractor` for native in-process and optional Wolverine-managed subscription execution. ENG-620 records provider-side context persistence through `EventDispatchProviderContextPersistenceMetadata` and `IEventDispatchProviderContextPersistenceStore`; the Entity Framework dispatch store persists that proof into outbox metadata, and optional Wolverine reports it only when the active store supports the contract. ENG-621 records provider-reported cross-node context handoff through `EventDispatchCrossNodeContextHandoffMetadata` only when provider-side persistence, consumer extraction, and distinct producer/consumer node ids are present. ENG-622 records provider-reported downstream completion through `EventDispatchDeliveryCompletionMetadata` only when successful dispatch metadata includes a completion source and provider receipt id, optionally adds subscriber acknowledgement and destination commit ids, and keeps exactly-once delivery unclaimed until a stronger provider proof owns it. ENG-623 records provider-proven exactly-once delivery through `EventDispatchExactlyOnceDeliveryProofMetadata` only when successful dispatch metadata includes provider receipt, subscriber acknowledgement, destination commit, and exactly-once proof ids. ENG-624 records provider-reported broker inbound consumption through `EventSubscriptionBrokerInboundConsumptionMetadata` only when successful subscription metadata includes consumer-loop, acknowledgement, lease, retry/poison, and offset-checkpoint proof. ENG-625 records provider-reported durable retry queue ownership through `EventDispatchDurableRetryQueueMetadata` only when retry-scheduled dispatch metadata includes durable retry queue, retry persistence, broker error queue, poison queue, cross-node retry coordination, and retry lease proof ids. ENG-626 records provider-reported subscription idempotency through `EventSubscriptionProviderIdempotencyMetadata` only when successful subscription metadata includes provider idempotency key, broker deduplication id, exactly-once proof id, durable inbox command id, generic inbox command id, and cross-node idempotency lease id. ENG-627 records provider-reported subscription concurrency through `EventSubscriptionConcurrencyMetadata` only when successful subscription metadata includes a positive concurrency limit, parallel handler execution, positive prefetch count, backpressure strategy, provider concurrency id, consumer lease id, work-stealing id, and distributed work-sharing id.

ENG-628 records provider-reported subscription ordering through `EventSubscriptionOrderingMetadata` only when successful subscription metadata includes handler ordering guarantee, local fan-out ordering, per-key ordering, partition ordering, causal ordering, replay ordering, cross-node ordering, and provider ordering proof ids. `subscription-ordering-ownership` now follows the same runtime-proof rule as concurrency: declared subscriptions, direct execution, code-first middleware, hosted bindings, and optional provider bindings are visible evidence but cannot claim ordering until the live proof set is complete.

ENG-629 records provider-reported process-manager state through `EventSubscriptionProcessManagerStateMetadata` only when successful subscription metadata includes saga state persistence, saga correlation, timeout scheduler, compensation workflow, process-manager concurrency, process-manager recovery, and provider process-manager proof ids. `process-manager-state-ownership` now follows the same runtime-proof rule as ordering: declared subscriptions, direct execution, code-first middleware, choreography handoff, outbox publication, hosted bindings, and optional provider bindings are visible evidence but cannot claim saga/process-manager ownership until the live proof set is complete.

ENG-630 records provider-reported broker topology materialization through `EventDispatchBrokerTopologyMetadata` only when successful dispatch metadata includes exchange provisioning, queue provisioning, topic provisioning, partition provisioning, topology verification, and provider topology proof ids. `broker-topology-materialization-ownership` now follows the same runtime-proof rule from the dispatch side: routing, channel governance, accepted outbox handoff, dispatch reports without topology proof, provider partition evidence, and optional Wolverine dispatch are visible evidence but cannot claim broker topology materialization until the live proof set is complete.

ENG-631 records provider-reported partition ownership through `EventDispatchProviderPartitionMetadata` only when successful dispatch metadata includes partition assignment, partition affinity, partition rebalancing, partition ordering guarantee, and provider partitioning proof ids. `provider-partition-ownership` now follows the same runtime-proof rule from the dispatch side: routing, channel governance, accepted outbox handoff, broker topology proof, dispatch reports without partition proof, and optional Wolverine dispatch are visible evidence but cannot claim provider partition ownership until the live proof set is complete.

ENG-632 records provider-reported scheduled/delayed delivery ownership through `EventDispatchScheduledDeliveryMetadata` only when successful dispatch metadata includes durable scheduled delivery, provider delay queue, broker scheduled delivery, cross-node schedule coordination, and schedule recovery proof ids. `scheduled-and-delayed-delivery-ownership` now separates bounded process-local scheduling from provider-owned scheduling: local scheduling can keep the profile `partial`, but the dimension becomes `claimed` only from complete provider proof, and Wolverine remains an optional companion rather than the scheduling contract.

ENG-633 records provider-reported broker dead-letter/replay ownership through `EventDispatchBrokerDeadLetterReplayMetadata` only when failed dispatch metadata includes broker dead-letter queue, replay action catalog, replay cursor, purge/quarantine, and provider proof ids. `broker-dead-letter-replay-ownership` now separates dispatch-store terminal intent and command-journal replay from broker DLQ mutation: the dimension becomes `claimed` only from complete failed-dispatch provider proof, and Wolverine remains an optional companion rather than the broker DLQ/replay contract.

ENG-634 records provider-reported serialization execution ownership through `EventDispatchSerializationExecutionMetadata` only when successful dispatch metadata includes executable payload serialization, executable schema lookup, executable upcaster execution, executable compatibility validation, and provider serialization proof ids. `EventDispatchWireContractMetadata` then records the stricter wire-contract proof only when that same successful dispatch path also supplies wire-envelope schema materialization, contract-version negotiation, and the final wire-contract proof id. `serialization-and-contract-versioning-ownership` now separates descriptor-backed contract, serializer, schema-registry, upcaster catalog, and serialization execution evidence from complete wire-contract ownership: the dimension stays `partial` until the full wire proof is present, and Wolverine remains an optional companion rather than the serialization or wire-contract runtime contract.

ENG-579 makes that command journal fail closed before mutation rather than only after a final result exists. The dispatcher now calls `ReserveAsync(...)` before applying a dispatch-store report, and the EF journal persists a `reserved` row that duplicate retries can observe across process/provider rebuilds until the final command result overwrites it. That keeps Wolverine optional while closing the in-doubt window between client retry uncertainty and dispatch-store mutation.

ENG-593 adds a durable replay seam to the EF command journal itself. The abstraction stays provider-neutral through `IEventDispatchRemediationCommandReplayCursorCatalog`, and the EF implementation reads command records oldest-first after the supplied `(ObservedAtUtc, CommandId)` cursor while leaving broker dead-letter replay as a separate provider-owned responsibility.

ENG-580 makes the same in-doubt state readable without a detail scan. `EventDispatchRemediationRuntimeSummary.ReservedCount` and `HasInDoubtCommands` now report retained `reserved` records in both process-local and durable journal lanes, while technology-surface metadata mirrors the posture for snapshot-driven operator UIs.

ENG-583 makes stale in-doubt detail reads provider-optimized: `IEventDispatchRemediationRuntimeCatalog.GetInDoubtBefore(...)` and `/engine/event-dispatch-remediation-commands/in-doubt?beforeUtc={beforeUtc}` return retained `reserved` records observed at or before an inclusive UTC cutoff, with invalid cutoff values rejected at the host boundary.

ENG-584 adds the single-record stale in-doubt read: `IEventDispatchRemediationRuntimeCatalog.GetOldestInDoubtBefore(...)` and `/engine/event-dispatch-remediation-commands/in-doubt/oldest?beforeUtc={beforeUtc}` return the oldest retained `reserved` command at or before the optional cutoff, return `404` when no retained row matches, and avoid list/pagination config because the query is intentionally direct.

ENG-585 adds the aggregate stale in-doubt read: `IEventDispatchRemediationRuntimeCatalog.GetInDoubtSummaryBefore(...)` and `/engine/event-dispatch-remediation-commands/in-doubt/summary?beforeUtc={beforeUtc}` return the retained `reserved` command summary at or before the optional cutoff, reject invalid cutoff values, and avoid list/pagination config because the query is intentionally a roll-up.

ENG-586 adds retained filter summaries for the same command journal. Outbox, message, channel, operation, actor, correlation, reason, outcome, and dispatch-outcome dashboards can call the matching `GetSummaryBy*` catalog method or `/engine/event-dispatch-remediation-commands/{filter}/{value}/summary` route and receive `EventDispatchRemediationRuntimeSummary` without materializing list rows, keeping the operator aggregate path provider-neutral and Wolverine-free.

ENG-587 adds retained filter latest reads beside those summaries. Outbox, message, channel, operation, actor, correlation, reason, outcome, and dispatch-outcome dashboards can call the matching `GetLatestBy*` catalog method or `/engine/event-dispatch-remediation-commands/{filter}/{value}/latest` route and receive the newest retained `EventDispatchRemediationRuntimeState`, or `404` when no retained row matches, without materializing list rows or installing Wolverine.

ENG-588 adds retained filter oldest reads beside the latest reads. The same outbox, message, channel, operation, actor, correlation, reason, outcome, and dispatch-outcome dashboards can call the matching `GetOldestBy*` catalog method or `/engine/event-dispatch-remediation-commands/{filter}/{value}/oldest` route and receive the oldest retained `EventDispatchRemediationRuntimeState`, or `404` when no retained row matches, without materializing list rows, adding hot-path route config, or installing Wolverine.

ENG-589 adds retained filter retention reads beside the summary/latest/oldest reads. The same outbox, message, channel, operation, actor, correlation, reason, outcome, and dispatch-outcome dashboards can call the matching `GetRetentionBy*` catalog method or `/engine/event-dispatch-remediation-commands/{filter}/{value}/retention` route and receive an `EventDispatchRemediationRuntimeRetention` posture, with an empty retention contract when no retained row matches, without materializing list rows, adding hot-path route config, or installing Wolverine.
...
- event dispatch remediation should flow through the host-agnostic `IEventDispatchRemediationDispatcher`, `IEventDispatchRemediationRuntimeCatalog`, `IEventDispatchRemediationCommandJournal`, `EventDispatchRemediationRuntimeSummary`, `EventDispatchRemediationRuntimeRetention`, `event-dispatch-remediations`, `event-dispatch-remediation-commands`, `POST /engine/event-dispatches/{outboxId}/commands/{operationId}`, `/engine/event-dispatch-remediation-commands*` including `/engine/event-dispatch-remediation-commands`, `/engine/event-dispatch-remediation-commands/summary`, `/engine/event-dispatch-remediation-commands/latest`, `/engine/event-dispatch-remediation-commands/retention`, `/engine/event-dispatch-remediation-commands/in-doubt?beforeUtc={beforeUtc}`, `/engine/event-dispatch-remediation-commands/in-doubt/summary?beforeUtc={beforeUtc}`, `/engine/event-dispatch-remediation-commands/in-doubt/oldest?beforeUtc={beforeUtc}`, `/engine/event-dispatch-remediation-commands/observations?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/observations/summary?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/{commandId}`, `/engine/event-dispatch-remediation-commands/outboxes/{outboxId}`, `/engine/event-dispatch-remediation-commands/messages/{messageId}`, `/engine/event-dispatch-remediation-commands/channels/{channelId}`, `/engine/event-dispatch-remediation-commands/operations/{operationId}`, `/engine/event-dispatch-remediation-commands/actors/{actorId}`, `/engine/event-dispatch-remediation-commands/correlations/{correlationId}`, `/engine/event-dispatch-remediation-commands/reasons/{reason}`, `/engine/event-dispatch-remediation-commands/dispatch-outcomes/{dispatchOutcome}`, and `/engine/event-dispatch-remediation-commands/outcomes/{outcome}`, and `/engine/snapshot` `TechnologySurfaces`; bounded dispatch-store commands can claim `retry-now`, `retry-later`, `skip`, `quarantine`, and dispatch-store `dead-letter` only when an active `IEventDispatchStore` owns the outbox, command ids must be reserved before dispatch-store mutation through the active journal, duplicate command ids must reject without mutation under the native `unique-command-id` policy even when the existing record is still `reserved`, latest/summary/retention/in-doubt/stale-in-doubt/stale-in-doubt-summary/oldest-in-doubt/observation-window reads, observation-window summaries, filter summaries, filter retention reads, filter latest reads, and filter oldest reads must remain tied to authoritative command records rather than duplicate HTTP attempts, `event-dispatch-remediation-commands` must expose a catalog entry even when history is empty so route/retention/idempotency/provider-neutral/Wolverine-free metadata is discoverable before the first command, metadata must advertise list/result/in-doubt/in-doubt-summary/oldest-in-doubt/outbox/filter-summary/filter-retention/filter-latest/filter-oldest routes plus in-doubt cutoff, observed-window, read-limit, reservation policy, reserved/in-doubt summary posture, and oldest reserved command readback instead of forcing operator UIs to infer it from routes, list/filter reads may apply a positive `limit` query to return newest retained records without changing the host-agnostic catalog, in-doubt-summary and filter-summary reads must remain direct aggregate queries, filter-retention reads must remain direct retention-posture queries, oldest-in-doubt, filter-latest, and filter-oldest reads must remain direct single-record queries, retention readback must say when bounded process-local command history has truncated older records, summary readback must surface reserved/in-doubt counts, oldest retained reserved command id/timestamp, dropped-command, and incomplete-summary warnings when truncation affects the aggregate scope, and broker-specific dead-letter/replay remains unclaimed until a package owns that path
