...
- event dispatch remediation should flow through the host-agnostic `IEventDispatchRemediationDispatcher`, `IEventDispatchRemediationRuntimeCatalog`, `event-dispatch-remediations`, `event-dispatch-remediation-commands`, `POST /engine/event-dispatches/{outboxId}/commands/{operationId}`, `/engine/event-dispatch-remediation-commands*` including `/engine/event-dispatch-remediation-commands/operations/{operationId}`, and `/engine/snapshot` `TechnologySurfaces`; bounded dispatch-store commands can claim `retry-now`, `retry-later`, `skip`, `quarantine`, and dispatch-store `dead-letter` only when an active `IEventDispatchStore` owns the outbox, duplicate command ids must reject without mutation under the native `unique-command-id` policy, and broker-specific dead-letter/replay remains unclaimed until a package owns that path
