...
- event dispatch remediation should flow through the host-agnostic `IEventDispatchRemediationDispatcher`, `IEventDispatchRemediationRuntimeCatalog`, `EventDispatchRemediationRuntimeSummary`, `EventDispatchRemediationRuntimeRetention`, `event-dispatch-remediations`, `event-dispatch-remediation-commands`, `POST /engine/event-dispatches/{outboxId}/commands/{operationId}`, `/engine/event-dispatch-remediation-commands*` including `/engine/event-dispatch-remediation-commands/summary`, `/engine/event-dispatch-remediation-commands/latest`, `/engine/event-dispatch-remediation-commands/retention`, `/engine/event-dispatch-remediation-commands/observations?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/observations/summary?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/messages/{messageId}`, `/engine/event-dispatch-remediation-commands/channels/{channelId}`, `/engine/event-dispatch-remediation-commands/operations/{operationId}`, `/engine/event-dispatch-remediation-commands/actors/{actorId}`, `/engine/event-dispatch-remediation-commands/correlations/{correlationId}`, `/engine/event-dispatch-remediation-commands/reasons/{reason}`, and `/engine/event-dispatch-remediation-commands/dispatch-outcomes/{dispatchOutcome}`, and `/engine/snapshot` `TechnologySurfaces`; bounded dispatch-store commands can claim `retry-now`, `retry-later`, `skip`, `quarantine`, and dispatch-store `dead-letter` only when an active `IEventDispatchStore` owns the outbox, duplicate command ids must reject without mutation under the native `unique-command-id` policy, latest/summary/retention/observation-window reads and observation-window summaries must remain tied to authoritative command records rather than duplicate HTTP attempts, list/filter reads may apply a positive `limit` query to return newest retained records without changing the host-agnostic catalog, retention readback must say when bounded process-local command history has truncated older records, summary readback must surface dropped-command and incomplete-summary warnings when truncation affects the aggregate scope, and broker-specific dead-letter/replay remains unclaimed until a package owns that path
