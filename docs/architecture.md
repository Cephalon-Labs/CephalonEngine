...
- event dispatch remediation should flow through the host-agnostic `IEventDispatchRemediationDispatcher`, `IEventDispatchRemediationRuntimeCatalog`, `IEventDispatchRemediationCommandJournal`, `EventDispatchRemediationRuntimeSummary`, `EventDispatchRemediationRuntimeRetention`, `event-dispatch-remediations`, `event-dispatch-remediation-commands`, `POST /engine/event-dispatches/{outboxId}/commands/{operationId}`, `/engine/event-dispatch-remediation-commands*` including `/engine/event-dispatch-remediation-commands`, `/engine/event-dispatch-remediation-commands/summary`, `/engine/event-dispatch-remediation-commands/latest`, `/engine/event-dispatch-remediation-commands/retention`, `/engine/event-dispatch-remediation-commands/in-doubt?beforeUtc={beforeUtc}`, `/engine/event-dispatch-remediation-commands/in-doubt/oldest?beforeUtc={beforeUtc}`, `/engine/event-dispatch-remediation-commands/observations?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/observations/summary?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/{commandId}`, `/engine/event-dispatch-remediation-commands/outboxes/{outboxId}`, `/engine/event-dispatch-remediation-commands/messages/{messageId}`, `/engine/event-dispatch-remediation-commands/channels/{channelId}`, `/engine/event-dispatch-remediation-commands/operations/{operationId}`, `/engine/event-dispatch-remediation-commands/actors/{actorId}`, `/engine/event-dispatch-remediation-commands/correlations/{correlationId}`, `/engine/event-dispatch-remediation-commands/reasons/{reason}`, `/engine/event-dispatch-remediation-commands/dispatch-outcomes/{dispatchOutcome}`, and `/engine/event-dispatch-remediation-commands/outcomes/{outcome}`, and `/engine/snapshot` `TechnologySurfaces`; bounded dispatch-store commands can claim `retry-now`, `retry-later`, `skip`, `quarantine`, and dispatch-store `dead-letter` only when an active `IEventDispatchStore` owns the outbox, command ids must be reserved before dispatch-store mutation through the active journal, duplicate command ids must reject without mutation under the native `unique-command-id` policy even when the existing record is still `reserved`, latest/summary/retention/in-doubt/stale-in-doubt/oldest-in-doubt/observation-window reads and observation-window summaries must remain tied to authoritative command records rather than duplicate HTTP attempts, `event-dispatch-remediation-commands` must expose a catalog entry even when history is empty so route/retention/idempotency/provider-neutral/Wolverine-free metadata is discoverable before the first command, metadata must advertise list/result/in-doubt/oldest-in-doubt/outbox routes plus in-doubt cutoff, observed-window, read-limit, reservation policy, reserved/in-doubt summary posture, and oldest reserved command readback instead of forcing operator UIs to infer it from routes, list/filter reads may apply a positive `limit` query to return newest retained records without changing the host-agnostic catalog, oldest-in-doubt reads must remain direct single-record queries, retention readback must say when bounded process-local command history has truncated older records, summary readback must surface reserved/in-doubt counts, oldest retained reserved command id/timestamp, dropped-command, and incomplete-summary warnings when truncation affects the aggregate scope, and broker-specific dead-letter/replay remains unclaimed until a package owns that path
