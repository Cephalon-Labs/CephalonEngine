...
- event dispatch remediation should flow through the host-agnostic `IEventDispatchRemediationDispatcher`, `event-dispatch-remediations`, `POST /engine/event-dispatches/{outboxId}/commands/{operationId}`, and `/engine/snapshot` `TechnologySurfaces`; bounded dispatch-store commands can claim `retry-now`, `retry-later`, `skip`, and `quarantine` only when an active `IEventDispatchStore` owns the outbox, while broker-specific dead-letter/replay remains unclaimed until a package owns that path
