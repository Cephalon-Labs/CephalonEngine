...
- event dispatch remediation advice should flow through the `event-dispatch-remediations` technology surface and `/engine/snapshot` `TechnologySurfaces`, deriving only from reported dispatch state until provider-neutral replay, dead-letter, skip, retry-later, or quarantine commands exist
