...
- lets modules and hosts declare explicit cell boundaries through `ICellBoundaryContributor`, `ICellBoundaryCatalog`, and `engine.AddCellBoundary(...)` without turning cells into a new blueprint
...
- explicit cell boundaries can be inspected through `ICellBoundaryCatalog`, `/engine/cells`, and `/engine/snapshot`, and active boundaries now auto-select `cell-based-architecture` without creating a second topology registry
...
- deeper cell routing, health-isolation, and provider-aware traffic automation on top of the shipped cell-boundary catalog
