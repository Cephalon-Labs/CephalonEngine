...
`Cephalon.Eventing` follows the same honesty rule: its direct in-process subscription lane can retry transient handler failures inside a bounded process-local attempt budget, can suppress duplicate completed subscription executions inside a bounded process-local retention window, can accept delayed publication requests through a bounded process-local scheduler, and can report latest publication runtime state for local direct execution, scheduled local handoff, or outbox `accepted` handoff. Event-dispatch remediation command idempotency and operator reads now flow through the provider-neutral `IEventDispatchRemediationCommandJournal`; the core pack provides a process-local fallback, while `Cephalon.Data.EntityFramework` can supply durable cross-node command history when the write-side `DbContext` opts into the journal context and outbox mapping. `eventing-superiority-profile` now mirrors that active command-journal descriptor through `durable-remediation-command-audit`, claiming durable command audit only when the descriptor is durable and cross-node while keeping replay cursors explicit as unclaimed. Broker-owned retries, durable retry queues, durable inbox ownership beyond explicit `IInbox` paths, durable or distributed scheduling, cross-node exactly-once delivery, downstream dispatch completion, and durable replay cursors stay outside the core pack until a provider or companion package truly owns them. `Cephalon.Eventing.Behaviors` now sits beside that baseline as an explicit bridge companion pack: it lets behavior-authored saga choreography hand off into the shared eventing publish path without making either baseline package absorb the other's contract ownership.
