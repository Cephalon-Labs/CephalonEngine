...
- lets modules and hosts declare outbox-linked CDC posture through `ICdcCaptureContributor`, `ICdcCaptureCatalog`, `ICdcCaptureRuntimeStateCatalog`, and `engine.AddCdcCapture(...)` without turning provider-specific change-stream execution into a host-only registry
...
- explicit cell boundaries, governed cell routes, cell health-isolation posture, and configuration-driven traffic automation can be inspected through `ICellBoundaryCatalog`, `ICellRouteCatalog`, `ICellHealthIsolationCatalog`, `ICellTrafficAutomationRuntimeCatalog`, `/engine/cells`, `/engine/cell-routes`, `/engine/cell-health-isolations`, `/engine/cell-traffic-automations`, and `/engine/snapshot`, and active boundaries, routes, health-isolation answers, or automation overlays now auto-select `cell-based-architecture` without creating a second topology, traffic, health, or automation registry
...
- module-owned CDC captures can now be inspected through `ICdcCaptureCatalog`, `ICdcCaptureRuntimeStateCatalog`, `/engine/cdc-captures`, `/engine/cdc-captures/runtime`, and `/engine/snapshot`, with explicit `OutboxId` linkage, linked `OutboxDispatchState` when the publication path reports it, and validation against active source modules and outboxes instead of a host-only sync registry
