...
ENG-634 records provider-reported serialization execution ownership through `EventDispatchSerializationExecutionMetadata` only when successful dispatch metadata includes executable payload serialization, executable schema lookup, executable upcaster execution, executable compatibility validation, and provider serialization proof ids. `serialization-and-contract-versioning-ownership` now separates descriptor-backed contract, serializer, schema-registry, and upcaster catalog evidence from executable serialization ownership: the dimension stays `partial` for descriptor catalogs and becomes `claimed` only from complete successful-dispatch provider proof, and Wolverine remains an optional companion rather than the serialization contract.
