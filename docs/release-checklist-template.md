...
- [ ] Engine completion scorecard artifact + source references + per-package GA rows + deployment-mode evidence including generated claims-report gate/target/warning/error/package-claim readback + adoption-smoke evidence including execution-report path/schema/required-field, golden use-case, and per-use-case report readback + provider-integration evidence including dependency-health provider-manifest release-validation/doctor readback + Eventing operational-superiority runtime concordance + SRE posture evidence including guardrail coverage counts, stable-baseline manifest readback, pending-baseline blocker rows, and Actions/workflow/dispatch readiness + supply-chain release evidence including package metadata validation, vulnerability audit, external-policy preflight count/status, and signed-release dry-run status/blocker/scope/diagnostic readback + `TestCoverageEvidence` layered-project/gap/recommendation/quarantine readback + public API compatibility evidence + optional `cephalon doctor --scorecard artifacts/engine-completion-scorecard-release/engine-completion-scorecard.json` readback with deployment-mode claims-report, adoption-smoke execution-report/golden-use-case/per-use-case-report readback, provider integration, dependency-health provider-manifest, Eventing runtime concordance, SRE, supply-chain preflight and signed-release dry-run, test coverage, and public API counts ✅
- [ ] `EventingOperationalSuperiorityEvidence` from `scripts/eventing-operational-superiority-support.json` includes native Eventing promotion contract/status/dimension/coverage/hot-path/Wolverine/runtime-concordance readback, and release validation plus optional `cephalon doctor --scorecard` agree ✅
...
- [ ] [`docs/engine-completion-scorecard.md`](engine-completion-scorecard.md) reviewed; generated scorecard artifact, source references, per-package GA rows, deployment-mode evidence plus generated claims-report gate/target/warning/error/package-claim readback, adoption-smoke evidence plus execution-report path/schema/required-field readback, golden use-case map, and per-use-case execution-report readback, provider-integration evidence with dependency-health provider-manifest release-validation/doctor readback, SRE posture evidence including benchmark guardrail coverage mappings plus stable-baseline manifest rows/measurements/pending ids, pending-baseline blocker rows, and Actions/workflow/dispatch readiness, supply-chain release evidence including package metadata validation, vulnerability audit, external-policy preflight count/status, and signed-release dry-run status/blocker/scope/diagnostic readback, `TestCoverageEvidence` matching the test-coverage roadmap, public API compatibility evidence, release-validation and optional `cephalon doctor --scorecard` summaries with deployment-mode claims-report, adoption-smoke execution-report/golden-use-case/per-use-case-report readback, provider integration, dependency-health provider-manifest, SRE, supply-chain preflight and signed-release dry-run, test coverage, and public API counts, and gate posture match the owning source docs
- [ ] `EventingOperationalSuperiorityEvidence` matches the Eventing operational-superiority support manifest, keeps hot-path publish/subscription code-first, keeps Wolverine optional, and reports runtime concordance against `EventingSuperiorityProfileRuntimeSurfaceContributor.cs`
...
- [ ] **Dry-run pass** via `pwsh ./scripts/invoke-signed-release-dry-run.ps1 -RequireRunCreated`; `artifacts/signed-release-dry-run/signed-release-dry-run-readiness.json` and `artifacts/signed-release-dry-run/signed-release-dry-run-handoff.md` record `Status = submitted`, `BlockerClass`, `DispatchBlockerScope`, `RepositoryWorkflowDispatchReady`, `WorkflowDispatchPrerequisitesStatus`, `ReadinessDiagnostic`, `DispatchActor`, `DispatchIdentityStatus`, `DispatchCommand`, `DispatchAttempted`, `RunCreated = true`, `RunUrl`, and `RequiredReleaseManagerAction`
...
- [ ] Confirmed SRE guardrail coverage counts, stable-baseline row/measurement counts, pending baseline ids, pending-baseline blocker rows, Actions/workflow/dispatch readiness fields, and guardrail references in generated `SrePostureEvidence`
- [ ] No SLI burned more than 25% of monthly budget in 7-day window (or freeze in place on affected family)
- [ ] Test flake rate within target from `artifacts/sre-ci-flake-rate/ci-flake-rate.json`; if the report is still `promotionAllowed=false` only because `readinessBlockerClass=no-completed-actions-history`, keep the flake-rate SLI pending rather than treating ready Actions/workflows as a failed release gate; new flaky tests quarantined within 24 hours
