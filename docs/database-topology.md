...
These templates are guidance, not execution orchestration. They keep the production bundle/script-first path visible in the same runtime surface as the migration target itself, while still allowing startup apply for local or controlled-host scenarios. `/engine/database-migration-playbook` now selects the canonical ordered production-versus-manual guidance from that lower-level catalog, including shared-target coordination hints when multiple logical migration targets point at one physical database, while the showcase operator console keeps those descriptors rich by showing their stable ids, human-readable labels, descriptions, production recommendation flags, typed execution-order hints, typed role-health and role-migration runtime notes, command metadata previews, and shared-target warnings instead of only dumping raw command text. The sample then derives repo-root runnable commands on top so operators can execute the showcase migration flow without translating the engine template manually, while still consuming the engine-owned ordered playbook instead of inventing a second sequencing contract locally.
...
- the engine now surfaces shared physical-target coordination truth through `/engine/database-roles`, `/engine/database-migration-playbook`, and `/engine/database-topology`, but it does not yet generate one combined bundle/script artifact or one orchestrated execution flow when multiple `DbContext` models point at the same physical database
