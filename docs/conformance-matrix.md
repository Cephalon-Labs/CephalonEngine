...
| Package | Maturity | Ownership | Engine routes | Snapshot keys | Catalog interfaces | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| `Cephalon.Diagnostics` | M2 | cephalon-managed | — | — | — | engine-level OpenTelemetry semantic-convention adapter; publishes canonical `ActivitySource` / `Meter` names plus `cephalon.*` attribute keys; `Cephalon.Engine` consumes the canonical names for `engine.build`, `module.{phase}`, and `runtime.*` spans / metrics; `Cephalon.AspNetCore` declares its diagnostics convention against `CephalonActivitySources.AspNetCore`; `Cephalon.Worker` emits `worker.lifecycle.start` / `worker.lifecycle.stop` spans against `CephalonActivitySources.Worker` |
...
- **Narrow managed execution (M2):** transport adapters, behaviors HTTP, EntityFramework data, relational data providers (SqlServer, Postgres, MySql, Oracle, MongoDB), multi-tenancy core + governance + delivery senders, edge core, observability core, Sfid, ReferenceDocs, **`Cephalon.Diagnostics`** (host adapters route telemetry through the canonical name set)
