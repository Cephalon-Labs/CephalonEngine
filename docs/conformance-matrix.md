...
| `Cephalon.Data.Neo4j` | M1 | provider-managed | — | — | — | Neo4j graph database pack (family-covered by maturity audit) |
| `Cephalon.Data.Cassandra` | M1 | provider-managed | — | — | — | Apache Cassandra pack (family-covered by maturity audit) |
| `Cephalon.Data.ClickHouse` | M1 | provider-managed | — | — | — | ClickHouse OLAP pack (family-covered by maturity audit) |
| `Cephalon.Data.Elasticsearch` | M1 | provider-managed | — | — | — | Elasticsearch pack (family-covered by maturity audit) |
| `Cephalon.Data.OpenSearch` | M1 | provider-managed | — | — | — | OpenSearch pack (family-covered by maturity audit) |
| `Cephalon.Data.Qdrant` | M1 | provider-managed | — | — | — | Qdrant vector database pack (family-covered by maturity audit) |
| `Cephalon.Data.Nats` | M1 | provider-managed | — | — | — | NATS messaging pack (family-covered by maturity audit) |
| `Cephalon.Data.Debezium` | M1 | provider-managed | — | — | `ICdcCaptureCatalog` | Debezium-managed CDC sourcing pack (family-covered by maturity audit) |

`Cephalon.Data` core ships as M3 with truthful cross-provider CDC truth. Relational providers (`SqlServer`, `Postgres`, `MySql`, `Oracle`) own schema/role/migration truth at M2 and contribute provider-native CDC where applicable. Non-relational providers (`Redis`, `Neo4j`, `Cassandra`, `ClickHouse`, `Elasticsearch`, `OpenSearch`, `Qdrant`, `Nats`, `Debezium`) stay at M1 catalog/descriptor maturity until their managed-execution proof lands; the maturity audit now carries a family-level row that records that shared `M1` `provider-managed` stance and names per-pack managed-execution proof (read/write/projection lifecycle for store packs; managed connector orchestration for Debezium) as the promotion gate.
...
- **Data non-relational provider audit coverage** (resolved May 2026 via `ENG-393`): the maturity audit now carries a family-level row for the nine non-relational `Cephalon.Data.*` packs (`Redis`, `Neo4j`, `Cassandra`, `ClickHouse`, `Elasticsearch`, `OpenSearch`, `Qdrant`, `Nats`, `Debezium`) at `M1` `provider-managed` catalog-only, with per-pack managed-execution proof (read/write/projection lifecycle for store packs; managed connector orchestration for Debezium) as the promotion gate. The matrix Notes-field tag on those nine rows now reads "(family-covered by maturity audit)" instead of "(audit pending)" so the matrix mirrors the audit-doc truth, and the Data family blurb now points at the audit row instead of leaving the catalog-only stance implicit. The relational provider packs continue to ride the existing `Cephalon.Data` shared CDC runtime row at `M3` and stay outside this family-level entry. With this slice landed, the conformance matrix carries no remaining "(audit pending)" Notes-field tags.
