...
| Package | Maturity | Ownership | Engine routes | Snapshot keys | Catalog interfaces | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| `Cephalon.Diagnostics` | M3 | cephalon-managed | `/engine/diagnostics-conventions` | — | — | engine-level OpenTelemetry semantic-convention adapter; publishes canonical `ActivitySource` / `Meter` names plus `cephalon.*` attribute keys; `Cephalon.Engine` consumes the canonical names for `engine.build`, `module.{phase}`, and `runtime.*` spans / metrics; `Cephalon.AspNetCore` declares its diagnostics convention against `CephalonActivitySources.AspNetCore` and projects the canonical name set as a `DiagnosticsConventionsSurface` record at `/engine/diagnostics-conventions`; `Cephalon.Worker` emits `worker.lifecycle.start` / `worker.lifecycle.stop` spans against `CephalonActivitySources.Worker` |
...
- **Broad managed execution (M3):** eventing core, eventing Wolverine bridge, agentics, retrieval, data core, edge providers (Kubernetes Gateway, Traefik), **`Cephalon.Diagnostics`** (host adapters route telemetry through the canonical name set; `/engine/diagnostics-conventions` operator surface ships through `Cephalon.AspNetCore`)
