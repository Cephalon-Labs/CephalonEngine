...
| Package | Maturity | Ownership | Engine routes | Snapshot keys | Catalog interfaces | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| `Cephalon.Diagnostics` | M1 | cephalon-managed | — | — | — | engine-level OpenTelemetry semantic-convention adapter; publishes canonical `ActivitySource` / `Meter` names plus `cephalon.*` attribute keys; `Cephalon.Engine` consumes the canonical names so `engine.build`, `module.{phase}`, and `runtime.*` spans / metrics flow through this package |
