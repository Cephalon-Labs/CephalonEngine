...
- **M1 redaction emission-site annotations on per-package rows** (resolved May 2026 via `ENG-418`): the `Cephalon.Diagnostics` row in the matrix correctly stated "M1 wired at seven real engine emission sites" but the per-package rows for the five emission-source packages (`Cephalon.AspNetCore`, `Cephalon.Engine`, `Cephalon.Worker`, `Cephalon.Eventing.Wolverine`, `Cephalon.Agentics`, `Cephalon.Retrieval`) didn't mention which class is the M1 emission site or that span tags route through the `RedactionPipeline` before exporter dispatch. Each per-package row now names its emission class (`HttpRequestResponseLoggingMiddleware` / `EngineRuntime` / `RuntimeHostedService` / `WolverineEventDispatchHostedService` / `AgentToolDispatcher` / `KnowledgeIndexer` + `KnowledgeQueryEngine`) and the closing-slice ENG reference (`ENG-365` / `ENG-366` / `ENG-412` / `ENG-374` / `ENG-401` / `ENG-402`) so a security reviewer scanning the matrix sees which packages are protected at the engine boundary without cross-referencing the Diagnostics row.
