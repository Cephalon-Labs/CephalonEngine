...
The stable SRE baseline manifest lives at `scripts/sre-stable-baselines.json`. It records the May 8, 2026 BenchmarkDotNet evidence for the four hot-path SLI rows that have enough evidence to promote to `stable-baseline-published`: `engine.behavior.dispatch.latency.p95`, `engine.behavior.dispatch.latency.p99`, `engine.behavior.dispatch.alloc.bytes-per-op`, and `engine.aspnetcore.request.alloc.bytes-per-op`. The same manifest also records `engine.worker.cold-start.p95` from `Cephalon.Benchmarks.Runtime.ColdStartBenchmarks-report.csv` as a `benchmark-mean-baseline-proxy` over `BuildStartWorkerHost`, because the current short-run export does not emit percentile columns and the measured mean remains far below the 500 ms SLO target. The manifest also records the release-validation claims-report baseline for `engine.deployment-mode-claims.truthful-fraction` plus release-validation timing baselines for `engine.dotnet.restore.wall-time.lock-mode`, `engine.reference-docs.wall-time`, and `engine.validate-release.wall-time`; two remaining SLI baselines stay pending so scorecard readback stays honest. The ASP.NET Core cold-start benchmark guardrail row remains mapped as a regression cap, but its stable SLO baseline stays pending because the latest `BuildStartHandleFirstRequestAspNetCore` run measured about `953.630 ms` against the 800 ms target. The flake-rate row remains pending because GitHub Actions has no run history to compute a 7-day baseline. Both pending rows now carry blocker evidence in the same manifest.
