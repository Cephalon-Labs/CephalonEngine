...
The stable SRE baseline manifest lives at `scripts/sre-stable-baselines.json`. It records the May 8, 2026 focused BenchmarkDotNet run for the four benchmark-backed SLI rows that have enough evidence to promote to `stable-baseline-published`: `engine.behavior.dispatch.latency.p95`, `engine.behavior.dispatch.latency.p99`, `engine.behavior.dispatch.alloc.bytes-per-op`, and `engine.aspnetcore.request.alloc.bytes-per-op`. The same manifest also records the release-validation claims-report baseline for `engine.deployment-mode-claims.truthful-fraction` and the release-validation restore locked-mode wall-time baseline for `engine.dotnet.restore.wall-time.lock-mode`; five remaining SLI baselines stay pending so scorecard readback stays honest. The cold-start benchmark guardrail rows are mapped as regression caps, but their stable SLO baselines remain pending until the cold-start optimization lane lands.
