...
The same workflow also carries a dedicated `.NET 11` readiness lane through `scripts/validate-dotnet-readiness.ps1`. That lane exists to assess future-SDK compatibility without changing the stable `net10.0` shipping floor. The workflow lane uses the readiness script's `-TestFilter` to exclude SDK-preview package/template `dotnet pack` and install roundtrips while keeping the normal `net10.0` release-validation lane as the package-shipping authority. When package publication quality or support claims need to be assessed under a higher SDK explicitly, use the readiness script directly without that workflow filter so the output report, `scripts/deployment-mode-support.json`, [deployment-mode support](deployment-mode-support.md), and the package-publishing docs stay truthful about what was actually validated.
