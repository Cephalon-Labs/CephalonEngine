...
| #3 — Deployment-mode claim validation needs a real harness | `partial` | `ENG-331`, `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458` | The harness exists and runs in CI, the inventory feeding it is repo-owned, every package is classified, the default representative publish target set now covers five sample hosts, `Cephalon.Diagnostics` is the first package-scoped single-file claim, the manifest-backed inventory now emits as `HazardInventory` plus `hazard-inventory.json`, the active low-tier enum-field reflection row has been removed, source-generated behavior dispatch now has closed-generic execution-slot hints, and source-generated durable execution now has closed durable-slot registrations. The remaining gating work is structural remediation for broader modes and intentionally removing `-SkipPublish` from release validation when full publish probing is ready to become a gate. |
