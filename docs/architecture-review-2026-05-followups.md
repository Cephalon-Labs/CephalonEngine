...
> Current scorecard checkpoint (`ENG-482`, May 6, 2026): `scripts/publish-engine-completion-scorecard.ps1` schema `1.3.0` now validates `scripts/adoption-smoke-support.json`, emits `AdoptionSmokeEvidence` for the out-of-repo generated-app package-stage replay, and emits `PublicApiCompatibilityEvidence` from `src/Cephalon.*/PublicAPI.Unshipped.txt` plus matching shipped baselines. The scorecard remains a read model over owning docs and scripts; it now fails fast if adoption probes drift away from `scripts/validate-out-of-tree-package-adoption.ps1` or if public API delta files lose their shipped baseline/project pairing.
