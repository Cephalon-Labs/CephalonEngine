...
| #3 — Deployment-mode claim validation needs a real harness | `partial` | `ENG-331`, `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454` | The harness exists and runs in CI, the inventory feeding it is repo-owned, every package is classified, the default representative publish target set now covers five sample hosts, and `Cephalon.Diagnostics` is the first package-scoped single-file claim. The remaining gating work is inventory emission, structural remediation for broader modes, and intentionally removing `-SkipPublish` from release validation when full publish probing is ready to become a gate. |
