...
| Recommendation | Status | Closing slice(s) | Closed date | Notes |
| --- | --- | --- | --- | --- |
| Close the truthful trim/AOT/single-file claim story end-to-end: project properties, manifest, validation script, workflow, docs, project memory, package-publishing guidance, and any roadmap/backlog cards updated together in one slice | `partial` | `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458`, `ENG-459` | 2026-05-06 | Inventory + manifest seed + path-truth Pester layer shipped, and the `ENG-432`..`ENG-439` deeper-read wave classified every previously-unclassified hazard surface. `ENG-449` moved the publish-probe surface forward by expanding the default target list to five adoption-quality sample hosts and proving the expanded set publishes cleanly under `singleFile` after retiring `ManifestModule.Name` / `Assembly.Location` analyzer blockers. `ENG-454` adds the first clean-baseline package-scoped claim (`Cephalon.Diagnostics` single-file) and keeps it separate from the global support rows. `ENG-455` emits the manifest-backed inventory as `HazardInventory` and `hazard-inventory.json`. `ENG-456` removes the active low-tier enum-field reflection row entirely. `ENG-457` narrows `Cephalon.Behaviors` startup reflection for source-generated behaviors by emitting generated execution-slot hints; `ENG-458` narrows `Cephalon.Behaviors.Patterns` durable execution for source-generated behaviors by emitting generated durable slots; `ENG-459` removes MySQL hosted-service duck-typed capture failure metadata reflection through a typed internal contract. The current aggregate posture is still 5 `high` + 14 `medium` + 0 `low` + 2 `excluded-by-design` = 21 active structural entries, plus 1 clean-baseline scoped `singleFile` claim, and the emitted inventory summarizes 22 package entries, 19 packages with known hazards, 49 known hazard entries, and transitive-hazard hints for all three modes. Remaining: structural remediation for `Cephalon.Engine`, `Cephalon.Behaviors`, and `Cephalon.Behaviors.Patterns` through generated/compile-time manifests; a public/upstream binlog transport path for `Cephalon.Data.MySql`; uniform remediation for the 10 `medium` `Cephalon.EventSourcing.*` packs and the 2 remaining `medium` CDC capture services; and release validation intentionally dropping `-SkipPublish` only when full publish probing should become a gate. |
...
| #3 — Deployment-mode claim validation needs a real harness | `partial` | `ENG-331`, `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458`, `ENG-459` | The harness exists and runs in CI, the inventory feeding it is repo-owned, every package is classified, the default representative publish target set now covers five sample hosts, `Cephalon.Diagnostics` is the first package-scoped single-file claim, the manifest-backed inventory now emits as `HazardInventory` plus `hazard-inventory.json`, the active low-tier enum-field reflection row has been removed, source-generated behavior dispatch now has closed-generic execution-slot hints, source-generated durable execution now has closed durable-slot registrations, and MySQL hosted-service capture failure metadata no longer uses duck-typed reflection. The remaining gating work is structural remediation for broader modes and intentionally removing `-SkipPublish` from release validation when full publish probing is ready to become a gate. |
