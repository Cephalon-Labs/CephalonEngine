...
| #3 — Deployment-mode claim validation needs a real harness | `partial` | `ENG-331`, `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449` | The harness exists and runs in CI, the inventory feeding it is repo-owned, every package is classified, and the default representative publish target set now covers five sample hosts. The gating work is the first claim-truthful flip, the harness publish-phase emitting the inventory programmatically, and intentionally removing `-SkipPublish` from release validation when full publish probing is ready to become a gate. |
