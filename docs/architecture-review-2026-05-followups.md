...
> Current scorecard checkpoint (`ENG-483`, May 6, 2026): `scripts/publish-engine-completion-scorecard.ps1` schema `1.3.0` validates `scripts/adoption-smoke-support.json`, emits `AdoptionSmokeEvidence` for the out-of-repo generated-app package-stage replay, and emits `PublicApiCompatibilityEvidence` from `src/Cephalon.*/PublicAPI.Unshipped.txt` plus matching shipped baselines. `cephalon doctor --scorecard <path>` now reads the same schema `1.3.0` artifact and reports public API package, pending-package, addition, and removal counts after checking the summary counts match `PublicApiCompatibilityEvidence`. The scorecard remains a read model over owning docs and scripts; it fails fast if adoption probes drift away from `scripts/validate-out-of-tree-package-adoption.ps1`, if public API delta files lose their shipped baseline/project pairing, or if the generated summary drifts from the detailed public API evidence.
