...
> Current deployment-mode checkpoint (`ENG-477`, May 6, 2026): add `ENG-471` through `ENG-477` to the partial deployment-mode follow-through rows below. The latest manifest-backed inventory reports 6 package entries, 2 packages with known hazards, 14 known hazard entries, 2 `high` + 0 `medium` + 3 `excluded-by-design` + 1 `clean-baseline`, zero active `low` entries, and 1 scoped `singleFile` claim. `ENG-471` and `ENG-472` retired the remaining `Cephalon.Behaviors` execution-slot and type-registry runtime fallbacks, returning that package to clean-baseline absence from the active table; `ENG-473` retired `Cephalon.Behaviors.Http` `MapProfile<TBehavior>()` attribute/profile fallback by requiring generated or explicitly registered descriptors; `ENG-474` retired the profile input-shape fallback by making explicit binding validation consume descriptor-backed input contracts; `ENG-475` retired the manual/profile route-contract fallback by requiring generated or explicitly registered `BehaviorContractDescriptor` metadata; `ENG-476` retired the broader `Cephalon.Engine` module-discovery blocker through generated `ModuleDiscoveryDescriptor` metadata; and `ENG-477` moved the MySQL SciSharp reflective transport path out of core `Cephalon.Data.MySql` into optional `Cephalon.Data.MySql.SciSharpReplication`. Remaining deployment-mode gating work is the optional `Cephalon.Data.MySql.SciSharpReplication` transport adapter posture, the by-design `Cephalon.ReferenceDocs` posture, transitive validation, and the deliberate publish-probe gating decision.
...
| Begin the deployment-mode validation harness work (PowerShell script + companion doc + first claim audit) so the trim/AOT/single-file claim story has a path to truthful proof before the .NET 11 final release window | `partial` | `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458`, `ENG-459`, `ENG-460`, `ENG-461`, `ENG-462`, `ENG-463`, `ENG-464`, `ENG-465`, `ENG-466`, `ENG-467`, `ENG-468`, `ENG-469`, `ENG-470`, `ENG-471`, `ENG-472`, `ENG-473`, `ENG-474`, `ENG-475`, `ENG-476`, `ENG-477` | 2026-05-06 | The harness PowerShell + Pester baseline shipped earlier (`ENG-331`); `ENG-426` added the per-package hazard inventory and `ENG-427` ported it into machine-readable manifest entries. The `ENG-431`..`ENG-439` wave strengthened path-truth coverage and classified every previously-unclassified runtime hazard surface, leaving the aggregate posture at 5 `high` + 14 `medium` + 1 `low` + 2 `excluded-by-design` = 22 structural entries, with `Cephalon.ReferenceDocs` as the first package with a machine-checkable permanent `not-claimed` posture. `ENG-449` expanded `representativePublishTargets.projects` to five publish-probe hosts and retired the first single-file analyzer blockers found by that expansion. `ENG-454` adds the first package-scoped claim for clean-baseline `Cephalon.Diagnostics`; `ENG-455` emits `HazardInventory` plus `hazard-inventory.json`; `ENG-456` retires the active `low` tier. `ENG-457` through `ENG-477` then retire behavior/durable/saga/REST/Data/EventSourcing/CDC/Engine hazards in stages, including the final `Cephalon.Behaviors` execution-slot/type-registry blockers, the `Cephalon.Behaviors.Http` profile attribute, input-shape, and route-contract fallbacks, the `Cephalon.Engine` module-discovery reflection blocker, and the core MySQL package's implicit SciSharp transport dependency. The current inventory reports 6 package entries, 2 packages with known hazards, and 14 known hazard entries. Still pending: the optional `Cephalon.Data.MySql.SciSharpReplication` transport adapter posture, the by-design `Cephalon.ReferenceDocs` posture, transitive validation, and the explicit release-validation decision to remove `-SkipPublish` once the publish probe should become a gate. |
...
| Recommendation | Status | Closing slice(s) | Closed date | Notes |
| --- | --- | --- | --- | --- |
| Close the truthful trim/AOT/single-file claim story end-to-end: project properties, manifest, validation script, workflow, docs, project memory, package-publishing guidance, and any roadmap/backlog cards updated together in one slice | `partial` | `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458`, `ENG-459`, `ENG-460`, `ENG-461`, `ENG-462`, `ENG-463`, `ENG-464`, `ENG-465`, `ENG-466`, `ENG-467`, `ENG-468`, `ENG-469`, `ENG-470`, `ENG-471`, `ENG-472`, `ENG-473`, `ENG-474`, `ENG-475`, `ENG-476`, `ENG-477` | 2026-05-06 | Inventory + manifest seed + path-truth Pester layer shipped, and the `ENG-432`..`ENG-439` deeper-read wave classified every previously-unclassified hazard surface. `ENG-449` moved the publish-probe surface forward; `ENG-454` adds the first clean-baseline package-scoped claim; `ENG-455` emits the manifest-backed inventory; `ENG-456` removes the active low-tier enum-field reflection row; `ENG-457` through `ENG-477` retire the behavior, durable, saga, CDC, EventSourcing, Data, Behaviors.Http metadata/dispatch/input-shape/route-contract, Engine module-discovery, and core MySQL implicit SciSharp transport fallbacks now recorded in the manifest. The current aggregate posture is 2 `high` + 0 `medium` + 0 `low` + 3 `excluded-by-design` = 5 active structural entries, plus 1 clean-baseline scoped `singleFile` claim, and the emitted inventory summarizes 6 package entries, 2 packages with known hazards, 14 known hazard entries, and transitive-hazard hints for all three modes. Remaining: a public/upstream or first-party binlog transport path for optional `Cephalon.Data.MySql.SciSharpReplication`; the permanent by-design `Cephalon.ReferenceDocs` posture; transitive validation; and release validation intentionally dropping `-SkipPublish` only when full publish probing should become a gate. |
...
| #3 — Trim, AOT, and single-file claim story is still not closed | `partial` | `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458`, `ENG-459`, `ENG-460`, `ENG-461`, `ENG-462`, `ENG-463`, `ENG-464`, `ENG-465`, `ENG-466`, `ENG-467`, `ENG-468`, `ENG-469`, `ENG-470`, `ENG-471`, `ENG-472`, `ENG-473`, `ENG-474`, `ENG-475`, `ENG-476`, `ENG-477` | See the Next-90-days recommendation above. The active remediation wave now retires the low-tier enum-field row, CDC failure metadata reflection, EventSourcing persisted type-name reflection, core Data command/query dispatch reflection, Behaviors.Http route/projection/module ownership reflection, Behaviors.Http result-envelope/generated-profile carrier/assembly-scan/profile-attribute/input-shape/route-contract fallback rows, Behaviors generated-carrier/assembly-scan/static-topology/execution-slot/type-registry rows, Behaviors.Patterns durable/saga fallback rows, Engine module-discovery reflection, and the core MySQL implicit SciSharp transport dependency while keeping global support `not-claimed`. |
...
| #3 — Deployment-mode claim validation needs a real harness | `partial` | `ENG-331`, `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458`, `ENG-459`, `ENG-460`, `ENG-461`, `ENG-462`, `ENG-463`, `ENG-464`, `ENG-465`, `ENG-466`, `ENG-467`, `ENG-468`, `ENG-469`, `ENG-470`, `ENG-471`, `ENG-472`, `ENG-473`, `ENG-474`, `ENG-475`, `ENG-476`, `ENG-477` | The harness exists and runs in CI, the inventory feeding it is repo-owned, every package is classified, the default representative publish target set now covers five sample hosts, `Cephalon.Diagnostics` is the first package-scoped single-file claim, the manifest-backed inventory now emits as `HazardInventory` plus `hazard-inventory.json`, and the active remediation wave has removed REST wire-name enum reflection, generated behavior/durable execution open-generic startup reflection, CDC failure metadata reflection, EventSourcing persisted type-name reflection, Data command/query dispatch reflection, Behaviors.Http REST metadata/carrier/assembly-scan/profile-attribute/input-shape/route-contract fallback rows, Behaviors generated-carrier/assembly-scan/static-topology/execution-slot/type-registry rows, Behaviors.Patterns durable/saga fallback rows, Engine module-discovery reflection, and the core MySQL implicit SciSharp transport dependency. The remaining gating work is the optional MySQL SciSharp transport adapter posture, by-design ReferenceDocs posture, transitive validation, and intentionally removing `-SkipPublish` from release validation when full publish probing is ready to become a gate. |
