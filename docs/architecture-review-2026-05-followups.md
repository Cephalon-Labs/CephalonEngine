...
> Current scorecard checkpoint (`ENG-481`, May 6, 2026): `scripts/publish-engine-completion-scorecard.ps1` schema `1.2.0` now validates `scripts/adoption-smoke-support.json` and emits `AdoptionSmokeEvidence` for the out-of-repo generated-app package-stage replay. The scorecard remains a read model over owning docs and scripts; it now fails fast if the adoption manifest's required assertions, required script tokens, or runtime probes drift away from `scripts/validate-out-of-tree-package-adoption.ps1`.
