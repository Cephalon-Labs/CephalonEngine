...
> Current deployment-mode checkpoint (`ENG-473`, May 6, 2026): add `ENG-471` through `ENG-473` to the partial deployment-mode follow-through rows below. The latest manifest-backed inventory reports 7 package entries, 4 packages with known hazards, 11 known hazard entries, 3 `high` + 1 `medium` + 2 `excluded-by-design` + 1 `clean-baseline`, zero active `low` entries, and 1 scoped `singleFile` claim. `ENG-471` and `ENG-472` retired the remaining `Cephalon.Behaviors` execution-slot and type-registry runtime fallbacks, returning that package to clean-baseline absence from the active table; `ENG-473` retired `Cephalon.Behaviors.Http` `MapProfile<TBehavior>()` attribute/profile fallback by requiring generated or explicitly registered descriptors. Remaining deployment-mode gating work is the broader `Cephalon.Engine` module-discovery blocker, `Cephalon.Data.MySql` third-party transport blocker, `Cephalon.Behaviors.Http` input-shape/manual-route blockers, the by-design `Cephalon.ReferenceDocs` posture, and the deliberate publish-probe gating decision.
...
| Recommendation | Status | Closing slice(s) | Closed date | Notes |
| --- | --- | --- | --- | --- |
| Close the truthful trim/AOT/single-file claim story end-to-end: project properties, manifest, validation script, workflow, docs, project memory, package-publishing guidance, and any roadmap/backlog cards updated together in one slice | `partial` | `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458`, `ENG-459`, `ENG-460`, `ENG-461`, `ENG-462`, `ENG-463`, `ENG-464`, `ENG-465`, `ENG-466`, `ENG-467`, `ENG-468`, `ENG-469`, `ENG-470`, `ENG-471`, `ENG-472`, `ENG-473` | 2026-05-06 | Inventory + manifest seed + path-truth Pester layer shipped, and the `ENG-432`..`ENG-439` deeper-read wave classified every previously-unclassified hazard surface. `ENG-449` moved the publish-probe surface forward; `ENG-454` adds the first clean-baseline package-scoped claim; `ENG-455` emits the manifest-backed inventory; `ENG-456` removes the active low-tier enum-field reflection row; `ENG-457` through `ENG-473` retire the behavior, durable, saga, CDC, EventSourcing, Data, and Behaviors.Http metadata/dispatch fallbacks now recorded in the manifest. The current aggregate posture is 3 `high` + 1 `medium` + 0 `low` + 2 `excluded-by-design` = 6 active structural entries, plus 1 clean-baseline scoped `singleFile` claim, and the emitted inventory summarizes 7 package entries, 4 packages with known hazards, 11 known hazard entries, and transitive-hazard hints for all three modes. Remaining: structural remediation for `Cephalon.Engine`; a public/upstream binlog transport path for `Cephalon.Data.MySql`; the `Cephalon.Behaviors.Http` input-shape/manual-route reflection row; the permanent by-design `Cephalon.ReferenceDocs` posture; and release validation intentionally dropping `-SkipPublish` only when full publish probing should become a gate. |
...
| #3 — Trim, AOT, and single-file claim story is still not closed | `partial` | `ENG-426`, `ENG-427`, `ENG-431`..`ENG-439`, `ENG-449`, `ENG-454`, `ENG-455`, `ENG-456`, `ENG-457`, `ENG-458`, `ENG-459`, `ENG-460`, `ENG-461`, `ENG-462`, `ENG-463`, `ENG-464`, `ENG-465`, `ENG-466`, `ENG-467`, `ENG-468`, `ENG-469`, `ENG-470`, `ENG-471`, `ENG-472`, `ENG-473` | See the Next-90-days recommendation above. The active remediation wave now retires the low-tier enum-field row, CDC failure metadata reflection, EventSourcing persisted type-name reflection, core Data command/query dispatch reflection, Behaviors.Http route/projection/module ownership reflection, Behaviors.Http result-envelope/generated-profile carrier/assembly-scan/profile-attribute fallback rows, Behaviors generated-carrier/assembly-scan/static-topology/execution-slot/type-registry rows, and Behaviors.Patterns durable/saga fallback rows while keeping global support `not-claimed`. |
