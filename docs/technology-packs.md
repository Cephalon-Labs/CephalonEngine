...
- `Cephalon.MultiTenancy.Governance`
  ...
  - registers `ITenantInvitationDeliveryRetryStore` and `ITenantInvitationDeliveryRetryRunner` when the opt-in invitation delivery retry queue is enabled, with an in-memory default and opt-in file-backed durable store
  ...
  - projects `tenant-invitations` for the Cephalon-managed invitation catalog, store, validation, delivery dispatch, sender readiness, delivery-run, opt-in retry queue, delivery-status reconciliation, and normalized observation-store proof
...
- `Cephalon.MultiTenancy.Governance`
    ...
    - `ITenantInvitationDeliveryRetryStore` and `ITenantInvitationDeliveryRetryRunner` for the opt-in local retry queue and bounded manual retry path over retryable sender failures
