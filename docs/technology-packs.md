...
- `Cephalon.Eventing`
  ...
  - can discover event channel descriptors from `Engine:Messaging:Channels` through `engine.AddEventingFromConfiguration(configuration)`, while subscription descriptors, descriptor-bearing executors, and direct execution middleware remain code-first through `EventingOptions.Subscriptions`, `IEventSubscriptionContributor`, `IEventSubscriptionExecutor`, optional `IEventSubscriptionDescriptorProvider`, and `IEventSubscriptionExecutionMiddleware`; `Engine:Messaging:Subscriptions` and `Engine:Messaging:SubscriptionHandlers` are rejected so publish/subscribe behavior is not string-config driven
