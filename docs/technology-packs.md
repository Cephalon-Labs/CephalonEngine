...
- `Cephalon.Eventing`
  ...
  - registers the abstraction-level `IEventDispatchRemediationRuntimeCatalog` beside that dispatcher, allowing host adapters to expose `/engine/event-dispatch-remediation-commands*` including `/engine/event-dispatch-remediation-commands/messages/{messageId}`, `/engine/event-dispatch-remediation-commands/channels/{channelId}`, `/engine/event-dispatch-remediation-commands/operations/{operationId}`, `/engine/event-dispatch-remediation-commands/actors/{actorId}`, `/engine/event-dispatch-remediation-commands/correlations/{correlationId}`, and `/engine/event-dispatch-remediation-commands/dispatch-outcomes/{dispatchOutcome}` and technology surfaces to report accepted and rejected remediation command results without inferring command history from the latest outbox state; command ids follow the native `unique-command-id` policy, and duplicate command ids are rejected without mutating dispatch-store state or overwriting the original command record
