...
- `Cephalon.MultiTenancy.Governance.AspNetCore`
  ...
  - maps `MapCephalonTenantInvitationDeliveryStatusObservations()` so normalized observation history can be read, filtered by attention or remediation action, and summarized through a bounded fail-closed ASP.NET Core endpoint
