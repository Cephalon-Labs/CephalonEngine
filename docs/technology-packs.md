...
- `Cephalon.Eventing`
  ...
  - can discover event channel descriptors from `Engine:Messaging:Channels` through `engine.AddEventingFromConfiguration(configuration)`, while subscription descriptors and executors remain code-first through `EventingOptions.Subscriptions`, `IEventSubscriptionContributor`, and `IEventSubscriptionExecutor`; `Engine:Messaging:Subscriptions` and `Engine:Messaging:SubscriptionHandlers` are rejected so publish/subscribe behavior is not string-config driven
...
- `Cephalon.Eventing`
  ...
  - `engine.AddEventingFromConfiguration(configuration)` plus `Engine:Messaging:Channels` for config-driven native channel descriptor discovery; subscription descriptors and executors stay code-first through `EventingOptions.Subscriptions`, `IEventSubscriptionContributor`, and `IEventSubscriptionExecutor`
