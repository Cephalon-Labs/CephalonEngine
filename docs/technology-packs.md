...
- `Cephalon.MultiTenancy.Governance.AmazonSesDelivery`
  ...
  - emits `Cephalon.MultiTenancy.Governance.AmazonSesDelivery` diagnostics while leaving AWS account/IAM/identity verification, DKIM/SPF/DMARC, sandbox exit, configuration-set event publishing, bounce/complaint callbacks, provider polling, callback inboxes, SMS, chat, CRM, identity-provider onboarding, distributed retry queues, and provider-specific reconciliation semantics to future provider packs or applications
