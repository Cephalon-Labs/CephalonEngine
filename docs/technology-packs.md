...
- `Cephalon.Eventing`
  ...
  - `durable-retry-queue-ownership` for runtime evidence that bounded direct retry and provider-reported retry observations do not imply durable retry queue, retry persistence, broker error queue, poison queue, cross-node retry, or retry-lease ownership
