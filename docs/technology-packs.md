...
- `Cephalon.MultiTenancy.Governance`
  ...
  - registers `ITenantInvitationDeliveryStatusReconciler` when invitation delivery status reconciliation is enabled; provider-specific callback translation, provider-specific signature verification, and provider polling stay outside the core pack and belong in host adapters or provider companions such as the SendGrid and Mailgun ASP.NET Core translators
...
- `Cephalon.MultiTenancy.Governance.MailgunDelivery.AspNetCore`
  ...
  - reads `Engine:MultiTenancy:Governance:MailgunInvitationDelivery:AspNetCore` or code-first options for route, authorization, provider-message matching, body/event limits, event source/actor, engagement-event mapping, and Mailgun message-id angle-bracket normalization
  ...
  - projects `tenant-invitation-delivery-mailgun-status-callbacks` with route/auth/limit/translation/signature/replay posture while leaving Mailgun signature verification/replay-token protection, durable callback inboxes, distributed replay protection, distributed event-id ledgers, provider polling, cross-node exactly-once delivery, and other non-SendGrid provider translation/signature semantics to future provider packs or applications
...
- `Cephalon.MultiTenancy.Governance.SendGridDelivery.AspNetCore`
  ...
  - projects `tenant-invitation-delivery-sendgrid-status-callbacks` with route/auth/limit/translation/signature/replay/event-id-idempotency posture while leaving durable callback inboxes, distributed replay protection, distributed event-id ledgers, provider polling, cross-node exactly-once delivery, and non-SendGrid provider translation/signature semantics to future provider packs or applications
- `Cephalon.MultiTenancy.Governance.SmtpDelivery`
  ...
  - emits `Cephalon.MultiTenancy.Governance.SmtpDelivery` diagnostics while leaving Mailgun Messages API handoff to `Cephalon.MultiTenancy.Governance.MailgunDelivery`, SendGrid Mail Send API handoff to `Cephalon.MultiTenancy.Governance.SendGridDelivery`, and leaving SES, Microsoft Graph, SMS, chat, CRM, identity-provider onboarding, bounce handling, provider polling, callback inboxes, and provider-specific reconciliation semantics to future provider packs or applications
- `Cephalon.Edge`
...
`Cephalon.MultiTenancy.Governance.MailgunDelivery.AspNetCore` owns Mailgun webhook payload translation only. It reports route, authorization, body/event limits, provider-message matching, translation ownership, signature/replay non-ownership, and inbox/polling boundaries through `tenant-invitation-delivery-mailgun-status-callbacks`. Mailgun HMAC signature verification, replay-token protection, durable callback inboxes, distributed replay/event-id ledgers, provider polling, and cross-node exactly-once delivery remain outside this proof.
