...
- `Cephalon.Eventing`
  ...
  - registers the abstraction-level `IEventDispatchRemediationRuntimeCatalog` beside that dispatcher, allowing host adapters to expose `/engine/event-dispatch-remediation-commands*` including `/engine/event-dispatch-remediation-commands/summary`, `/engine/event-dispatch-remediation-commands/latest`, `/engine/event-dispatch-remediation-commands/retention`, `/engine/event-dispatch-remediation-commands/observations?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/observations/summary?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/messages/{messageId}`, `/engine/event-dispatch-remediation-commands/channels/{channelId}`, `/engine/event-dispatch-remediation-commands/operations/{operationId}`, `/engine/event-dispatch-remediation-commands/actors/{actorId}`, `/engine/event-dispatch-remediation-commands/correlations/{correlationId}`, `/engine/event-dispatch-remediation-commands/reasons/{reason}`, `/engine/event-dispatch-remediation-commands/dispatch-outcomes/{dispatchOutcome}`, and `/engine/event-dispatch-remediation-commands/outcomes/{outcome}` and technology surfaces to report a catalog entry, accepted/rejected remediation command results, bounded-history truncation posture, inclusive observed-UTC incident windows, retained-window roll-ups, and metadata-advertised observed-window/read-limit policies without inferring command history from the latest outbox state; the catalog entry is present even when history is empty and keeps route, retention, idempotency, provider-neutral, and `wolverineRequired = false` metadata discoverable; command ids follow the native `unique-command-id` policy, and duplicate command ids are rejected without mutating dispatch-store state, overwriting the original command record, replacing the latest authoritative record, or inflating summary/retention counts
