...
- `Cephalon.MultiTenancy.Governance`
  ...
  - projects `tenant-invitations` for the Cephalon-managed invitation catalog, store, validation, delivery dispatch, sender readiness, delivery-run, delivery-status reconciliation, and normalized observation-store proof
...
- `Cephalon.MultiTenancy.Governance.HttpDelivery`
  ...
  - emits `Cephalon.MultiTenancy.Governance.HttpDelivery` diagnostics while leaving provider-specific email, SMS, chat, CRM, identity-provider, durable retry queue, provider-specific delivery-status callback translation, provider-specific callback signature verification, provider polling, and provider-reconciliation semantics to future provider packs or applications
- `Cephalon.Edge`
