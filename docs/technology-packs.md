...
- `Cephalon.MultiTenancy.Governance.AmazonSesDelivery`
  ...
  - emits `Cephalon.MultiTenancy.Governance.AmazonSesDelivery` diagnostics while leaving Amazon SES over SNS callback translation, opt-in SNS signature verification, and bounded process-local SNS replay protection to `Cephalon.MultiTenancy.Governance.AmazonSesDelivery.AspNetCore`; AWS account/IAM/identity verification, DKIM/SPF/DMARC, sandbox exit, SES configuration-set event destination setup, SNS topic/subscription creation, provider polling, callback inboxes, SMS, chat, CRM, identity-provider onboarding, distributed retry queues, and provider-specific reconciliation semantics remain future provider-pack or application work
- `Cephalon.MultiTenancy.Governance.AmazonSesDelivery.AspNetCore`
  - optional ASP.NET Core Amazon SES over SNS callback translator, SNS signature-verification, and process-local SNS replay-protection companion for `Cephalon.MultiTenancy.Governance`
  ...
  - reads `Engine:MultiTenancy:Governance:AmazonSesInvitationDelivery:AspNetCore` or code-first options for route, authorization, provider-message matching, body/event limits, event source/actor, engagement-event mapping, raw SES replay/test payload acceptance, SNS signature policy, and SNS replay cache posture
  ...
  - projects `tenant-invitation-delivery-amazon-ses-status-callbacks` with route/auth/limit/translation/SNS-signature/process-local-replay/inbox/subscription-confirmation posture while leaving SNS subscription confirmation automation, observation-store-backed SNS message-id idempotency, durable callback inboxes, distributed replay protection, distributed event-id ledgers, provider polling, and cross-node exactly-once delivery to future provider packs or applications
...
`Cephalon.MultiTenancy.Governance.AmazonSesDelivery.AspNetCore` owns Amazon SES over SNS payload translation, opt-in SNS signature verification, and bounded process-local SNS replay protection. It reports route, authorization, body/event limits, provider-message matching, translation ownership, SNS signature verification posture, SNS signature-version/topic/certificate policy, process-local SNS replay posture, SNS subscription-confirmation posture, and inbox/polling boundaries through `tenant-invitation-delivery-amazon-ses-status-callbacks`. SNS subscription confirmation automation, observation-store-backed SNS message-id idempotency, durable callback inboxes, distributed replay/event-id ledgers, provider polling, and cross-node exactly-once delivery remain outside this proof.
