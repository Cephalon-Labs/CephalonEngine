...
- `Cephalon.Eventing`
  ...
  - registers the abstraction-level `IEventDispatchRemediationDispatcher` when a mutable dispatch store owns an outbox, allowing host adapters to expose `retry-now`, `retry-later`, `skip`, and `quarantine` through `POST /engine/event-dispatches/{outboxId}/commands/{operationId}` without requiring Wolverine or claiming broker dead-letter ownership
...
- `Cephalon.Eventing`
  ...
  - `engine.AddEventingFromConfiguration(configuration)` plus `Engine:Messaging:Channels` for config-driven native channel descriptor discovery; subscription descriptors, descriptor-bearing executors, and direct execution middleware stay code-first through `EventingOptions.Subscriptions`, `IEventSubscriptionContributor`, `services.AddCephalonEventSubscriptionExecutor<TExecutor>()`, optional `IEventSubscriptionDescriptorProvider`, and `services.AddCephalonEventSubscriptionExecutionMiddleware<TMiddleware>()`, with direct interface registrations still supported for custom DI ownership
