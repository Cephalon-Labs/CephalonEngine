...
- `Cephalon.Eventing`
  ...
  - registers the abstraction-level `IEventDispatchRemediationRuntimeCatalog` beside that dispatcher, allowing host adapters to expose `/engine/event-dispatch-remediation-commands*` including `/engine/event-dispatch-remediation-commands/summary`, `/engine/event-dispatch-remediation-commands/latest`, `/engine/event-dispatch-remediation-commands/retention`, `/engine/event-dispatch-remediation-commands/observations?fromUtc={fromUtc}&toUtc={toUtc}`, `/engine/event-dispatch-remediation-commands/messages/{messageId}`, `/engine/event-dispatch-remediation-commands/channels/{channelId}`, `/engine/event-dispatch-remediation-commands/operations/{operationId}`, `/engine/event-dispatch-remediation-commands/actors/{actorId}`, `/engine/event-dispatch-remediation-commands/correlations/{correlationId}`, `/engine/event-dispatch-remediation-commands/reasons/{reason}`, and `/engine/event-dispatch-remediation-commands/dispatch-outcomes/{dispatchOutcome}` and technology surfaces to report accepted/rejected remediation command results plus bounded-history truncation posture and inclusive observed-UTC incident windows without inferring command history from the latest outbox state; command ids follow the native `unique-command-id` policy, and duplicate command ids are rejected without mutating dispatch-store state, overwriting the original command record, replacing the latest authoritative record, or inflating summary/retention counts
