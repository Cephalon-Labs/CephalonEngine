...
- `Cephalon.Eventing`
  ...
  - reports `durable-remediation-command-audit`, `durable-command-journal-replay-cursor`, `broker-dead-letter-replay-ownership`, `broker-topology-materialization-ownership`, `provider-partition-ownership`, `downstream-delivery-completion-ownership`, `broker-inbound-consumption-ownership`, and `serialization-and-contract-versioning-ownership` separately so durable command audit, provider-owned command-journal replay, dispatch-store dead-letter intent, broker replay ownership, broker topology materialization, provider partition ownership, downstream delivery completion, inbound broker consumption, and wire-contract ownership cannot be conflated
