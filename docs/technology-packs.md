...
- `Cephalon.MultiTenancy.Governance`
  ...
  - `MultiTenancyGovernanceOptions` for host-defined memberships, invitations, governance actions, store paths, and validation/evaluation/decision/workflow enablement
- `Cephalon.Eventing`
...
When multi-tenancy is selected, `Cephalon.MultiTenancy` owns one narrow tenant-resolution loop. The base package resolves configured tenant ids, tenant keys, host names, defaults, and single-tenant fallback through `ITenantResolver`, then exposes the ambient answer through `ITenantContextAccessor` and the `tenant-resolution` surface. `Cephalon.MultiTenancy.Governance` now owns concrete companion proofs beside that base package: modules, hosts, or `ITenantMembershipStore` contribute `TenantMembershipDescriptor` values for `ITenantMembershipCatalog` plus `ITenantMembershipEvaluator`, modules, hosts, or `ITenantInvitationStore` contribute `TenantInvitationDescriptor` values for `ITenantInvitationCatalog` plus `ITenantInvitationValidator`, `TenantDomainOwnershipDescriptor` values for `ITenantDomainOwnershipCatalog` plus `ITenantDomainOwnershipValidator`, and `TenantGovernanceActionDescriptor` values for `ITenantGovernanceActionCatalog` plus `ITenantGovernanceActionDecider`, while `ITenantGovernanceActionWorkflow` can create and transition runtime action state through `ITenantGovernanceActionStore`. The membership, invitation, and action stores are in-memory by default and become Cephalon-managed local JSON durable stores when `MembershipStoreFilePath`, `InvitationStoreFilePath`, or `GovernanceActionStoreFilePath` are configured. DNS/HTTP verification execution, remediation execution beyond state transitions, durable domain stores, distributed or provider-backed governance storage, notification/delivery, invitation delivery, identity-provider synchronization, public onboarding, and tenant-administration workflows remain outside the current managed proof until the governance package owns those paths explicitly.
