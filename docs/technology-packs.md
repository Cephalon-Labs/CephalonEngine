...
- `Cephalon.MultiTenancy.Governance`
  ...
  - projects `tenant-invitations` for the Cephalon-managed invitation catalog, store, and validation proof
  - projects `tenant-domain-ownership` for the Cephalon-managed declared domain-ownership catalog, store, validation, in-process verification-workflow, proof-challenge issuance, proof-publication planning, reported-proof evaluation, and HTTP file proof-collection proof
...
- `Cephalon.MultiTenancy.Governance`
    ...
    - `ITenantDomainOwnershipProofVerificationRunner` for the current Cephalon-managed proof-verification orchestration path over challenge issuance, publication planning, reported-proof evaluation, and optional HTTP file collection
    - `MultiTenancyGovernanceOptions` for host-defined memberships, invitations, domain ownerships, governance actions, store paths, proof challenge defaults, proof publication planning, HTTP proof collection, proof verification runner orchestration, proof evaluation, and validation/evaluation/decision/workflow enablement
- `Cephalon.Eventing`
...
When multi-tenancy is selected, `Cephalon.MultiTenancy` owns one narrow tenant-resolution loop. The base package resolves configured tenant ids, tenant keys, host names, defaults, and single-tenant fallback through `ITenantResolver`, then exposes the ambient answer through `ITenantContextAccessor` and the `tenant-resolution` surface. `Cephalon.MultiTenancy.Governance` now owns concrete companion proofs beside that base package: modules, hosts, or `ITenantMembershipStore` contribute `TenantMembershipDescriptor` values for `ITenantMembershipCatalog` plus `ITenantMembershipEvaluator`, modules, hosts, or `ITenantInvitationStore` contribute `TenantInvitationDescriptor` values for `ITenantInvitationCatalog` plus `ITenantInvitationValidator`, modules, hosts, or `ITenantDomainOwnershipStore` contribute `TenantDomainOwnershipDescriptor` values for `ITenantDomainOwnershipCatalog` plus `ITenantDomainOwnershipValidator`, `ITenantDomainOwnershipVerificationWorkflow` can request, verify, reject, suspend, or expire runtime domain ownership state through `ITenantDomainOwnershipStore`, `ITenantDomainOwnershipProofChallengeIssuer` can issue expected proof values plus DNS TXT or HTTP file publication hints into that same store, `ITenantDomainOwnershipProofPublicationPlanner` can turn issued challenge metadata into DNS TXT or HTTP file publication instructions and optional plan metadata, `ITenantDomainOwnershipProofEvaluator` can verify or reject reported proof evidence through that same workflow, `ITenantDomainOwnershipHttpProofCollector` can collect bounded host-matched HTTPS HTTP file proof content and feed it into that evaluator, `ITenantDomainOwnershipProofVerificationRunner` can orchestrate the challenge, publication-plan, observed-proof evaluation, and optional HTTP collection paths through one entry point, and `TenantGovernanceActionDescriptor` values feed `ITenantGovernanceActionCatalog` plus `ITenantGovernanceActionDecider`, while `ITenantGovernanceActionWorkflow` can create and transition runtime action state through `ITenantGovernanceActionStore`. The membership, invitation, domain-ownership, and action stores are in-memory by default and become Cephalon-managed local JSON durable stores when `MembershipStoreFilePath`, `InvitationStoreFilePath`, `DomainOwnershipStoreFilePath`, or `GovernanceActionStoreFilePath` are configured. Actual DNS/HTTP proof publication, DNS TXT proof collection or external polling, remediation execution beyond state transitions, distributed or provider-backed governance storage, notification/delivery, invitation delivery, identity-provider synchronization, public onboarding, and tenant-administration workflows remain outside the current managed proof until the governance package owns those paths explicitly.
