...
- `Cephalon.MultiTenancy.Governance.SendGridDelivery.AspNetCore`
  ...
  - projects `tenant-invitation-delivery-sendgrid-status-callbacks` with route/auth/limit/translation/signature/replay posture while leaving durable callback inboxes, distributed replay protection, provider polling, cross-node exactly-once delivery, and non-SendGrid provider translation/signature semantics to future provider packs or applications
