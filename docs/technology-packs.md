...
- `Cephalon.Eventing`
  ...
  - can discover event channel and subscription descriptors from `Engine:Messaging:Channels` and `Engine:Messaging:Subscriptions` through `engine.AddEventingFromConfiguration(configuration)`, loading configuration before module contributors so host configuration can override package defaults without rewriting application code
  - can bind configured subscriptions to `IEventSubscriptionHandler` types through `Engine:Messaging:Subscriptions:{id}:HandlerType` or `Engine:Messaging:SubscriptionHandlers`, adapting them into native in-process executors so hosts avoid hand-written `IEventSubscriptionExecutor` glue when Wolverine is not selected
...
- `Cephalon.Eventing`
  ...
  - `engine.AddEventingFromConfiguration(configuration)` plus `Engine:Messaging:Channels` and `Engine:Messaging:Subscriptions` for config-driven native descriptor discovery; configured descriptors are merged before module contributors and configured subscriptions project `descriptorSource` / `configurationPath` metadata
  - `IEventSubscriptionHandler`, `EventSubscriptionHandlerDescriptor`, and `EventingOptions.SubscriptionHandlers` for config-driven native handler binding; configured handler types are adapted into in-process executors and project `handlerBindingSource`, `handlerType`, and `configurationPath` binding metadata
