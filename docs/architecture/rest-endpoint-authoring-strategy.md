...
- the next shorthand metadata-and-document truthfulness follow-through is now shipped through
  ...
  `ENG-058-T111`, `ENG-058-T112`, `ENG-058-T113`, `ENG-058-T114`, and `ENG-058-T115`: same-value endpoint-metadata, tag, and document-name rewrites plus reorder-only
  ...
  `snapshot.RestEndpoints`, and `ENG-058-T115` now keeps event `5204`
  ...
  `PreserveSourceImplicitFallback` and `PreserveRemainingBodyFallback`
...
- within that constrained pattern slice, placeholder-preserving rewrites stay the default and
  ...
  `metadata.bindingFallbackMode = preserve-source-implicit-fallback` remains compatibility-only
  metadata, and broader implicit-property promotion beyond that bounded query-fallback slice
  remains later work
