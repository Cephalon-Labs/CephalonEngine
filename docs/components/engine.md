...
- additive cell-boundary and cell-route contracts, runtime catalogs, and technology-surface projection
...
hosts can contribute `CellBoundaryDescriptor` entries and governed `CellRouteDescriptor` entries,
the engine composes them through `ICellBoundaryCatalog` plus `ICellRouteCatalog`, validates route
ownership against the active module-owned boundary graph, auto-selects the built-in
`cell-based-architecture` profile when either boundaries or routes are active, and projects the
same answers through `/engine/cells`, `/engine/cell-routes`, `snapshot.CellBoundaries`,
`snapshot.CellRoutes`, and the `cell-boundaries` plus `cell-routes` technology runtime surfaces
without inventing a second host-only topology or traffic registry. Health isolation or
provider-aware traffic automation remain later follow-through over those same catalogs.
