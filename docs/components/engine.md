...
- versioned additive runtime-introspection extension sections with deterministic ordering, duplicate-id rejection, shared desired/observed-state entries, operator conditions, and host-neutral action declarations through `IRuntimeIntrospectionSectionContributor`
...
- `Runtime/IRuntimeIntrospectionSectionContributor.cs`
- `Runtime/RuntimeIntrospectionSection.cs`
- `Runtime/RuntimeIntrospectionSectionEntry.cs`
- `Runtime/RuntimeOperatorCondition.cs`
- `Runtime/RuntimeOperatorAction.cs`
- `Runtime/DependencyHealthRuntimeIntrospectionSectionContributor.cs`
...
`Cephalon.Engine` still does not claim global trim, Native AOT, or single-file support. The current trim/AOT readiness work has retired bounded analyzer blockers by moving package manifest JSON parsing to `PackageDefinitionFileJsonContext`, preserving feature-flag provider constructors for DI, and making module-owned behavior registration's type-preservation and type-based compatibility boundaries explicit. Focused trim publish probes now leave dynamic package assembly loading as the remaining trim blocker; Native AOT also still needs a source-generated or typed behavior-input JSON path before it can be promoted.

## How it fits

New package-specific operator projections can join `snapshot.ExtensionSections` without adding another top-level snapshot property; existing typed fields remain intact for compatibility. The built-in `dependency-health` section is the first consumer and projects desired/observed state plus conditions and probe metadata without claiming remediation execution.
...
Capability collection remains deterministic, but shared provider-family keys are additive. If two different modules publish the same capability key, the engine keeps a single manifest entry, preserves stable matching metadata, and adds aggregate `sourceModuleIds`, `providers`, `packs`, and `contributorCount` metadata so `/engine/capabilities` can describe the whole active provider family. A duplicate key from the same source module still fails composition, because that is a module-authoring error rather than a provider-family aggregate.
