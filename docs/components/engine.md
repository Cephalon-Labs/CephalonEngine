...
That same rule now applies to the first shipped database-topology baseline. `Engine:Databases` is the engine-owned physical-topology contract for shared runtime tuning, the first named roles (`Write`, `Read`, `Outbox`, `History`), nested migration policy, and the new `/engine/databases` introspection surface. Relational follow-through, role references, and durable audit-history storage are still later slices, but the engine now owns the contract instead of leaving it to sample-only host code. See [Database topology](../database-topology.md).
