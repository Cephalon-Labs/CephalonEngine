...
- configuration binding for engine, trust, localization, failure policy, options, and the phase-8 `Data`, `Databases`, `Identity`, `Tenancy`, `Audit`, and `Messaging` sections, including `Engine:Audit:History:Retention`
...
That same rule now applies to the shipped database-topology and durable audit-history baseline. `Engine:Databases` is the engine-owned physical-topology contract for shared runtime tuning, the first named roles (`Write`, `Read`, `Outbox`, `History`), nested migration policy, and the `/engine/databases` introspection surface. `Engine:Audit:History` now lets additive provider packs such as `Cephalon.Audit.EntityFramework` target the logical `history` role by default, or another supported engine-owned role through `Engine:Audit:History:DatabaseRole`, while `Engine:Audit:History:Retention` now drives the first engine-owned retention baseline and `/engine/audit-history` exposes the first queryable operator surface when a durable reader is active. Richer role references, non-relational history providers, replay UX, and export follow-through remain later slices, but the engine now owns the contract instead of leaving it to sample-only host code. See [Database topology](../database-topology.md).
