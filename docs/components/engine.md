...
- additive CDC capture contracts plus runtime CDC descriptor and state catalogs
...
capture outcome, counts, checkpoints, errors, and optional linked `OutboxDispatchState` from the
existing event-dispatch runtime truth. Provider-specific federation, self-serve data infrastructure,
and provider-native CDC execution/freshness follow-through remain later work over those same
catalogs.
