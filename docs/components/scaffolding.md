...
The scaffold output now also carries the narrow phase-8 starter contract: canonical kebab-case `Engine` ids, structured `Engine:Data`, `Engine:Identity`, `Engine:Tenancy`, `Engine:Audit`, and `Engine:Messaging` sections, low-ceremony `Sfid` plus `Audit` defaults, and test placeholders under `Architecture/CompositionSmokeTests.cs` plus `Features/*BehaviorSpecifications.cs` so generated apps start from business-focused seams instead of framework boilerplate. When `EventDrivenIntegration` and `Outbox` are selected together, generated modules also contribute a process-local starter `IOutbox` plus matching `IOutboxContributor` descriptor for the generated `application-events` channel, enough for `/engine/event-publications` to prove accepted outbox handoff without claiming durable broker dispatch. Generated non-test projects now include `Cephalon.Analyzers` as a private analyzer package, so apps and modules inherit the Cephalon quality baseline by default while PublicApiAnalyzers public-surface tracking remains an explicit package-author opt-in. When `RestApi` is selected, the generated public module stubs now also default to `RestBehaviorModuleBase` with `ConfigureRestBehaviors(...)` and `MapProfile<TBehavior>()`, add `Cephalon.Behaviors.SourceGen` as a private analyzer package, and keep `Cephalon.Engine.SourceGen` on generated module projects, keeping scaffold output aligned with the shipped module-owned REST strategy while leaving `cephalon-rest-module` as the generic non-behavior package path. When a generated `Microservice` selects `RestApi`, `JsonRpc`, and `Grpc`, the scaffold now wires `builder.AddJsonRpcTransport()`, `builder.AddGrpcTransport()`, a generated module that implements `IJsonRpcModule` and `IGrpcModule`, and `Cephalon.Observability.HttpDependencies` defaults for an opt-in HTTP dependency-health lane. `scripts/validate-microservice-multi-transport-adoption.ps1` proves that generated shape outside the repository by packaging local NuGet artifacts, running the host, and calling the REST, JSON-RPC, and gRPC business endpoints plus `/engine/transports`, `/engine/snapshot`, `/engine/dependencies`, `/engine/diagnostics`, `/engine/runtime-story`, and `/scalar`.
