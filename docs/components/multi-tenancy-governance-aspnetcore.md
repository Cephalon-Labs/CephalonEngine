...
`scripts/validate-saas-tenant-governance-audit-adoption.ps1` now proves the generated consumer-app path for this adapter. The script maps `app.MapCephalonTenantAdministrationCommands()`, `app.MapCephalonTenantInvitationDeliveryDispatches()`, `app.MapCephalonTenantInvitationDeliveryStatusCallbacks()`, and `app.MapCephalonTenantInvitationDeliveryStatusObservations()` in a generated ASP.NET Core host, deliberately disables authorization only for the local smoke host, then calls the real `/engine/tenant-administration/commands`, `/engine/tenant-invitations/delivery-dispatches`, `/engine/tenant-invitations/delivery-status`, and `/engine/tenant-invitations/delivery-status/observations` endpoints before checking `/engine/technology-surfaces/multi-tenancy` and `/engine/snapshot`.
