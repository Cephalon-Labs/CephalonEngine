...
- bounded and filterable observation reads by tenant id, invitation id, status, outcome, source, correlation id, attention category, reconciled, recorded, and limit
- filtered observation rollup summaries by status, attention, outcome, source, channel, sender, and tenant before the response record limit is applied
