...
- index state implementation, freshness calculation, query counters, and runtime-surface contribution for introspection
...
The read-side index-state contract now lives in `Cephalon.Abstractions.Retrieval` as
`IKnowledgeIndexCatalog`, `KnowledgeIndexState`, `KnowledgeIndexingOutcomes`, and
`KnowledgeIndexFreshnessStates`. `Cephalon.Retrieval` remains the implementation owner for the
managed lexical index and query loop, while `Cephalon.Engine` and host adapters can expose
`/engine/knowledge-indexes`, `/engine/knowledge-indexes/{collectionId}`, and
`snapshot.KnowledgeIndexes` without taking a direct dependency on this implementation package.
