...
`KnowledgeIndexingRequest`, and `KnowledgeIndexingResult`. `Cephalon.Retrieval` remains the
implementation owner for the managed lexical index and query loop, while `Cephalon.Engine` and host
adapters can expose `/engine/knowledge-indexes`, `/engine/knowledge-indexes/{collectionId}`,
