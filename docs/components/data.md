...
> **Maturity:** `M3` · **Ownership:** mixed: `cephalon-managed` + `provider-managed` — authoritative truth in [`engine-surface-maturity-audit.md`](../engine-surface-maturity-audit.md)

`Cephalon.Data` is the runtime-neutral data execution companion package for Cephalon.

## What it owns

- registers default `IReadStore` and `IWriteStore` implementations backed by registered command/query dispatch descriptors over the existing handler abstractions
...
Default command/query dispatch is now registry-backed. Hosts or generated module startup code register each dispatchable shape with `AddCephalonDataCommand<TCommand>()`, `AddCephalonDataCommand<TCommand, TResult>()`, or `AddCephalonDataQuery<TQuery, TResult>()` alongside the matching handler service. The default stores then dispatch through those closed generic descriptors instead of discovering handler methods through runtime `MakeGenericMethod`, which keeps the consumer code small while making the AOT/trim posture explicit and machine-auditable. If a command or query shape is missing from the dispatch registry, the store fails with a registration-oriented message rather than attempting hidden reflection.
