...
The remaining provider-specific gap is deliberate rather than hidden: `Cephalon.Data.Cassandra` and `Cephalon.Data.ClickHouse` still stage durable outbox records, but they need deeper storage-model decisions before Cephalon can claim the same provider-native dispatch-store truth there.
