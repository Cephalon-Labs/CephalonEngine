...
Configured subscriptions receive `descriptorSource = configuration` and `configurationPath` metadata in `event-subscriptions`, and configured descriptors are added before module contributors. That ordering keeps installed packages reusable while letting a host replace module defaults by declaring the same descriptor id in configuration.
