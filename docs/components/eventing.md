...
- an abstraction-level `IEventDispatchRemediationRuntimeCatalog` command-result read model plus bounded process-local command history controlled by `EventingOptions.RemediationCommandHistoryLimit`, including explicit retention posture through `EventDispatchRemediationRuntimeRetention` and inclusive observed-UTC window reads through `GetByObservedAt(...)`, so accepted and rejected operator commands can be audited without inferring command history from the latest outbox state or mistaking a truncated bounded list for complete history
...
The command-result route family also includes `/engine/event-dispatch-remediation-commands/observations?fromUtc={fromUtc}&toUtc={toUtc}`. It is backed by `IEventDispatchRemediationRuntimeCatalog.GetByObservedAt(...)`, filters retained records by an inclusive observed UTC window, returns results latest-first, and rejects reversed bounds so incident-timeline reads remain deterministic without requiring Wolverine or a broker-specific audit store.
