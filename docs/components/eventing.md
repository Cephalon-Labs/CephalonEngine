...
- configuration-owned event channel and subscription descriptors through `Engine:Messaging:Channels` and `Engine:Messaging:Subscriptions`, loaded by `engine.AddEventingFromConfiguration(configuration)` before module contributors so host configuration can override module defaults by id without editing application code
- configuration-owned subscription handler bindings through `Engine:Messaging:Subscriptions:{id}:HandlerType` or `Engine:Messaging:SubscriptionHandlers`, adapted into `IEventSubscriptionExecutor` instances for the native in-process lane without requiring consumer code to implement the executor contract directly
...
- `Services/IEventSubscriptionHandler.cs`
- `Services/EventSubscriptionHandlerDescriptor.cs`
...
- `Services/ConfiguredEventSubscriptionExecutor.cs`
    ...
    "Messaging": {
      ...
      "Subscriptions": {
        "audit-projector": {
          ...
          "Description": "Projects audit events into the compliance read model.",
          "ChannelId": "audit",
          "HandlerId": "configured-audit-projector",
          "HandlerType": "MyCompany.Audit.ConfiguredAuditProjectorHandler, MyCompany.Audit",
          "DeliveryMode": "message-handler",
...
Configured subscriptions receive `descriptorSource = configuration` and `configurationPath` metadata in `event-subscriptions`, and configured descriptors are added before module contributors. When `HandlerType` is present and the type implements `IEventSubscriptionHandler`, the native pack adapts it into an in-process executor and projects `binding.handlerBindingSource`, `binding.handlerType`, and `binding.configurationPath` metadata. That ordering keeps installed packages reusable while letting a host replace module defaults by declaring the same descriptor id in configuration.
...
- `Cephalon.Eventing` now also has an opt-in `cephalon-managed` direct in-process subscription execution baseline, with config-driven channel/subscription descriptor discovery, optional bounded process-local retries using fixed/exponential backoff plus deterministic jitter metadata, duplicate-completed execution suppression backed either by process-local memory or by exactly one registered `IInbox`, config-driven publication routing, and bounded process-local scheduled publication acceptance, for lightweight hosts that do not need a durable broker or Wolverine
