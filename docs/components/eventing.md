...
The command-result read model is intentionally separate from the dispatch-state catalog. `IEventDispatchRuntimeCatalog` answers the latest state of an outbox dispatch path, while `IEventDispatchRemediationRuntimeCatalog` answers which operator commands were accepted or rejected, including command id, outbox id, message id, operation, actor, outcome, dispatch outcome, timestamp, error, and safe metadata. Command ids are unique operator action ids, not reusable operation names. Operation ids are stable action families, so operators can drill into all recorded `retry-now`, `retry-later`, `skip`, `quarantine`, or `dead-letter` command results without parsing the latest dispatch-state metadata. Actor ids are optional command metadata; when supplied, `/engine/event-dispatch-remediation-commands/actors/{actorId}` returns the accepted and rejected command results recorded for that actor without treating duplicate rejected command-id attempts as a second command-history entry. If a client retries with a command id that is already recorded, the dispatcher returns `rejected` with `duplicateCommand = true`, `existingCommandOutcome`, `existingCommandOperationId`, `existingCommandOutboxId`, and `existingCommandObservedAtUtc`; the original command record remains authoritative and dispatch-state mutation is skipped. ASP.NET Core exposes those records through `/engine/event-dispatch-remediation-commands*`, including `/engine/event-dispatch-remediation-commands/operations/{operationId}` and `/engine/event-dispatch-remediation-commands/actors/{actorId}`, and the technology-surface entry `event-dispatch-remediation-commands` keeps the same command-result truth available to snapshot consumers through `TechnologySurfaces`.
