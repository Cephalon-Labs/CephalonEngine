...
- explicit `cephalon doctor --scorecard <path>` summaries over generated engine-completion scorecard JSON artifacts
...
When release managers already have a generated `engine-completion-scorecard.json` artifact, `cephalon doctor --scorecard <path>` gives a local readback of schema/source truth, platform-gate posture, validated evidence-reference counts, and per-package GA-readiness counts without parsing Markdown, duplicating scorecard authority, or promoting GA/support claims.
