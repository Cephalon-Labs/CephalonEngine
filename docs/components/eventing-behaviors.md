...
- lets `Cephalon.Eventing` report `choreography-handoff-ownership` as `claimed` in `eventing-superiority-profile` only when this explicit bridge and an outbox-backed publish path are active
