...
This pack sits on top of `Cephalon.Data`, not in place of it. `Cephalon.Data` still owns the runtime-neutral `IReadStore` / `IWriteStore` dispatching surface. `Cephalon.Data.MongoDB` adds the MongoDB-backed outbox and inbox persistence paths that let event-driven workloads stage and track messages without switching to a relational store, and it now also exposes the same staged outbox through `IEventDispatchStore` so consumer-managed or adapter-owned dispatch loops can read pending items and persist durable dispatch outcomes without MongoDB-specific host glue.
