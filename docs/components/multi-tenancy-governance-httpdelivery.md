...
This is intentionally not a provider-specific email, SMS, chat, CRM, or identity-provider connector. Provider-specific authentication models, message templates, user provisioning, durable retry queues, provider-specific delivery-status callback translation, provider-specific callback signature verification, provider polling, human inboxes, and external provider reconciliation remain future companion or application-managed work until a package owns those paths explicitly. ASP.NET Core hosts that need a generic normalized callback ingress can install `Cephalon.MultiTenancy.Governance.AspNetCore` and map `MapCephalonTenantInvitationDeliveryStatusCallbacks()` over the core reconciler; that adapter can verify provider-neutral HMAC signatures and reject bounded process-local signed replays, while the core governance pack can retain the resulting normalized observations in memory or an opt-in local JSON file. This sender package remains outbound-only.
