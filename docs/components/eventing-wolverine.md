...
Wolverine is a benchmark input, not the boundary of this package or the engine. When this adapter proves a useful capability such as scheduled delivery, retry policy, dead-letter management, handler execution, diagnostics, or operator tooling, the long-term target is to generalize the capability into Cephalon-owned eventing options, contracts, runtime catalogs, and `/engine/snapshot` readback before treating it as core engine value. The native pack now owns a bounded process-local delayed publication proof through `Engine:Messaging:Publications:Scheduling`, while durable broker-delayed delivery and provider-managed scheduling remain companion-owned when a runtime truly owns those paths. Wolverine-specific APIs and semantics stay inside this companion; Cephalon apps should be able to switch to the native lane or a future provider pack by changing engine-owned configuration rather than rewriting application code.
