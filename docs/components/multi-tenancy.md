...
The base package still does not execute tenant membership workflows, invite flows, domain ownership validation, or product-specific backoffice/public-site orchestration. Those broader workflows are visible as boundary entries in the `tenant-governance-boundaries` surface so the current runtime stays honest: `Cephalon.MultiTenancy` owns resolution and ambient context, while `Cephalon.MultiTenancy.Governance` now owns the first concrete companion proof for tenant membership cataloging and membership evaluation. Invitations, domain ownership, approval/remediation workflows, and tenant administration remain future governance work until a package owns those paths explicitly.
