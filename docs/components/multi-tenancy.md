...
- projects the base-package versus future-companion boundary through the `tenant-governance-boundaries` runtime surface under `multi-tenancy`
...
The base package still does not execute tenant membership workflows, invite flows, domain ownership validation, or product-specific backoffice/public-site orchestration. Those broader workflows are now visible as taxonomy-only boundary entries in the `tenant-governance-boundaries` surface with `plannedOwnership = companion-planned` and suggested future package `Cephalon.MultiTenancy.Governance`. That keeps the current runtime honest: `Cephalon.MultiTenancy` owns resolution and ambient context today, while the future governance companion can own membership, invitations, domain ownership, and approval/remediation workflows when that package exists.
