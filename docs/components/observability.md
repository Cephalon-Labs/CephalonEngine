...
The same shared telemetry contract is also intentionally open to downstream packages. Teams can bind `ObservabilityOptions.FromConfiguration(builder.Configuration).Telemetry`, add provider-specific options under `Engine:Observability:Telemetry:{ProviderName}`, keep exporter/auth/resource logic in their own companion package, and optionally publish their own diagnostics convention plus startup summary through `IDiagnosticsConventionContributor` and `IHostedService`. That is the intended path for later Alibaba Cloud, Cloudflare, DigitalOcean, OpenShift, Tanzu, or internal gateway integrations without modifying `Cephalon.Engine`, `Cephalon.Abstractions`, or the shared `ILogger` pipeline.
