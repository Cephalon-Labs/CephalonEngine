...
- a sanitized `telemetry-export-guidance` runtime surface for `/engine/technology-surfaces` and `/engine/snapshot`
- the shared dependency-health configuration convention consumed by optional upstream probe companion packages
- the dependency-health probe infrastructure baseline shipped through `Cephalon.Observability.DependencyHealth.Core`
...
The baseline package now also projects the shared telemetry intent as an `ITechnologyRuntimeContributor` surface. The projection reports provider, protocol, enabled signals, and shared endpoint mode, but it deliberately redacts raw endpoints, headers, tokens, connection strings, and other deployment secrets.
