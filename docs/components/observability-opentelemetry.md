...
- a sanitized `telemetry-export-opentelemetry` runtime surface for `/engine/technology-surfaces` and `/engine/snapshot`

## Main surfaces

- `Hosting/OpenTelemetryHostApplicationBuilderExtensions.cs`
- `Hosting/OpenTelemetryTelemetryRuntimeContributor.cs`
...
When the package is activated, it contributes the `telemetry-export-opentelemetry` runtime surface so operators can see the active OTLP path without exposing raw collector endpoints or credentials.
