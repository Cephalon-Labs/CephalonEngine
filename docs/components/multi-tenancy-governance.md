...
- `tenancy.invitation.catalog`, `tenancy.invitation.store`, `tenancy.invitation.validation`, `tenancy.invitation.delivery-dispatch`, and `tenancy.invitation.delivery-status-reconciliation` capabilities when the `MultiTenancy` technology is active and the related runtime path is enabled
