...
`scripts/validate-saas-tenant-governance-audit-adoption.ps1` now proves the generated consumer-app path for this package. The script packages `Cephalon.MultiTenancy.Governance` with its ASP.NET Core adapter, `Cephalon.Audit`, JSON-RPC, gRPC, and HTTP dependency-health companions, scaffolds a modular-monolith tenant host outside the repository, registers a smoke `ITenantInvitationDeliverySender`, issues an invitation through the tenant-administration workflow, dispatches it, reconciles delivery status, reads normalized observations, and checks the `multi-tenancy` technology surface plus `/engine/snapshot` so tenant-governance execution is tied to a real running generated app.
