...
- one Cephalon-managed tenant-invitation delivery status reconciliation baseline that records provider or receiver status observations without owning webhook mapping or provider polling
...
`ITenantInvitationDeliveryStatusReconciler` owns the host-agnostic status-observation path after dispatch. It resolves the same merged invitation catalog, optionally requires the incoming provider message id to match the id recorded by dispatch, records `lastDeliveryStatus`, `lastDeliveryStatusObservedAtUtc`, status provider message id, sender id, channel, source, actor, correlation id, reason, reconciliation outcome, reconciliation ownership, and external status ownership through `ITenantInvitationStore`, and returns `provider-message-missing` or `provider-message-mismatch` instead of attaching a provider callback to the wrong invitation. It does not map HTTP webhook endpoints, poll delivery providers, verify provider signatures, or translate provider-specific status vocabularies beyond normalized metadata; those remain adapter/provider-pack responsibilities.
