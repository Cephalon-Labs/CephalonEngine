...
- `tenancy.domain-ownership.catalog`, `tenancy.domain-ownership.store`, `tenancy.domain-ownership.validation`, `tenancy.domain-ownership.workflow`, `tenancy.domain-ownership.proof-challenge`, and `tenancy.domain-ownership.proof-evaluation` capabilities when the `MultiTenancy` technology is active
...
- stable governance diagnostics for allowed and denied membership evaluation, invitation validation, domain-ownership validation, domain-ownership verification workflow transitions, domain-ownership proof challenge issuance, domain-ownership proof evaluation, domain-ownership store persistence, governance-action decisions, governance-action workflow transitions, and governance-action store persistence
