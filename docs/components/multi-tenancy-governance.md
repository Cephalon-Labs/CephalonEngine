...
- `tenancy.invitation.catalog`, `tenancy.invitation.store`, `tenancy.invitation.validation`, `tenancy.invitation.delivery-dispatch`, `tenancy.invitation.delivery-status-reconciliation`, and `tenancy.invitation.delivery-status-observation-store` capabilities when the `MultiTenancy` technology is active and the related runtime path is enabled
