...
- the redaction filter contract through `IRedactionFilter` and `RedactionContext` so consumer apps and observability companion packs can register synchronous filters that redact secrets, PII, authentication tokens, and other sensitive values before they leave the engine boundary; at M1 maturity two engine emission sites pipe values through registered filters via the `RedactionPipeline` resolved from DI: `Cephalon.AspNetCore`'s HTTP request/response logging middleware (HTTP request/response span tags) and `Cephalon.Engine`'s module-phase runtime activity tags (`runtime.{phase}` and `module.{phase}` spans during initialize/start/stop) — consumer-registered filters actually run on attribute values before exporter dispatch
