...
- the first package-scoped deployment-mode claim: this clean-baseline package declares `PublishSingleFile=true` and `EnableSingleFileAnalyzer=true`, and `scripts/deployment-mode-support.json` scopes `singleFile` support to `Cephalon.Diagnostics` without widening the global engine / host / generated-app single-file support contract
