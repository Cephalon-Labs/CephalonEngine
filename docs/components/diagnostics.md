...
- the redaction filter contract through `IRedactionFilter` and `RedactionContext` so consumer apps and observability companion packs can register synchronous filters that redact secrets, PII, authentication tokens, and other sensitive values before they leave the engine boundary; at M1 maturity the engine pipes the first emission site (`Cephalon.AspNetCore`'s HTTP request/response logging middleware) through registered filters via the `RedactionPipeline` resolved from DI, so consumer-registered filters actually run on attribute values before exporter dispatch
