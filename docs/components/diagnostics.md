...
Consumer apps register one or more `IRedactionFilter` implementations and call `AddRedactionPipeline()` once. The engine's M1-promoted emission sites (AspNetCore HTTP middleware, engine runtime module-phase tags, Cephalon.Eventing.Wolverine dispatch-time tags, Cephalon.Agentics in-process tool-dispatch tags, and Cephalon.Retrieval knowledge-indexing + knowledge-query tags) automatically route attribute values through the registered pipeline before exporter dispatch.
