...
- the redaction filter contract through `IRedactionFilter` and `RedactionContext` so consumer apps and observability companion packs can register synchronous filters that redact secrets, PII, authentication tokens, and other sensitive values before they leave the engine boundary; at M1 maturity three engine emission sites pipe values through registered filters via the `RedactionPipeline` resolved from DI: `Cephalon.AspNetCore`'s HTTP request/response logging middleware (HTTP request/response span tags), `Cephalon.Engine`'s module-phase runtime activity tags (`runtime.{phase}` and `module.{phase}` spans during initialize/start/stop), and `Cephalon.Eventing`'s in-process publication-dispatch activity tags (`eventing.publication.dispatch` span emitted under `CephalonActivitySources.Eventing`) — consumer-registered filters actually run on attribute values before exporter dispatch
...
Consumer apps register one or more `IRedactionFilter` implementations and call `AddRedactionPipeline()` once. The engine's M1-promoted emission sites (AspNetCore HTTP middleware, engine runtime module-phase tags, and Cephalon.Eventing in-process publication-dispatch tags) automatically route attribute values through the registered pipeline before exporter dispatch.
...
- maturity today: `M4` — `cephalon-managed`; the canonical name set is consumed by both the engine and an observability companion pack as a real subscription contract, not just a guidance pattern:
    ...
    - **`Cephalon.Observability.OpenTelemetry`** subscribes to all five canonical activity sources (`CephalonActivitySources.Engine` / `.AspNetCore` / `.Worker` / `.Eventing` / `.MultiTenancyGovernance`) and the matching five canonical meters (`CephalonMeters.Engine` / `.AspNetCore` / `.Worker` / `.Eventing` / `.MultiTenancyGovernance`) when telemetry export is enabled, so consumer apps that opt into the OTLP companion pack export every engine-emitted span and metric without further configuration
