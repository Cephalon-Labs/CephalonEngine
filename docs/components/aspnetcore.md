...
- an explicit trim/AOT boundary on `MapCephalon()`: the full operator route surface still uses ASP.NET Core Minimal API delegate binding, so the public method carries `RequiresUnreferencedCode` and `RequiresDynamicCode`; package-local analyzer builds are clean, and the deployment-mode harness audits that the manifest-backed route boundary still has both annotations, but this is not a Native AOT support claim. The opt-in `core` operator surface now maps its bounded route subset through prebuilt `RequestDelegate` handlers and `MapMethods(...)`; the harness emits `CoreRouteDelegateAuditStatus` and scorecard/release/doctor readback for that proof, but it remains route-subset proof, not a full-adapter trim/AOT claim
