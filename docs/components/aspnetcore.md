...
- an explicit trim/AOT boundary on `MapCephalon()`: the full operator route surface still contains ASP.NET Core Minimal API delegate binding, so the public method carries `RequiresUnreferencedCode` and `RequiresDynamicCode`; package-local analyzer builds are clean, and the deployment-mode harness audits that the manifest-backed route boundary still has both annotations, but this is not a Native AOT support claim. The opt-in `core` operator surface, the shared full/common routes, the full/default behavior-resilience, saga-choreography, durable-execution, foundation, and tail introspection route blocks now map their bounded route subsets through prebuilt `RequestDelegate` handlers and `MapMethods(...)`; the harness emits `CoreRouteDelegateAuditStatus` and `FullCommonRouteDelegateAuditStatus` plus scorecard/release/doctor readback for the generated core/full-common proofs, and Pester guards the full/default workflow/foundation/tail blocks, but they remain route-subset proofs, not a full-adapter trim/AOT claim
