...
requires ordered reporting, and refreshes `lastReportId` plus typed `observationFreshness` answers
on `/engine/cdc-captures/runtime*`, `/engine/cdc-capture-runtimes*`, and `snapshot` from the same
configured stale-window policy instead of a host-only timeout monitor.
