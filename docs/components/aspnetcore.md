...
By default the host registers the `v1` OpenAPI document and keeps `/scalar` as the selector-first entry point by redirecting to the slash-normalized Scalar page at `/scalar/` instead of pinning one version immediately. The long-term versioned config contract is `OpenApi:EnabledVersions` plus `OpenApi:DefaultVersion`, for example `EnabledVersions: [1, 2]` and `DefaultVersion: 2`, so Scalar can render a version selector with the configured default document preselected while endpoints mapped with `BehaviorRestEndpointGroup.ApiVersion(2)` continue to appear in `/openapi/v2.json`. Deep links such as `/scalar/v2` still work when a caller wants to pin one document explicitly. Legacy `OpenApi:Documents` and `OpenApi:DefaultDocument` string settings still work for backward compatibility or custom non-version document names. `OpenApi:Version` remains available as a global `info.version` override for single-document hosts, but multi-document hosts now keep each document version truthful to its own resolved document name such as `v1` or `v2`.
