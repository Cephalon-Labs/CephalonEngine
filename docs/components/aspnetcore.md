...
`ICellHealthIsolationCatalog` is active. `/engine/technology-surfaces/cell-based-architecture`
keeps the same topology, routing, and health-isolation posture visible through the existing
technology-surface projection. That keeps the phase 13 cell baseline operator-facing without
turning cell topology, traffic posture, or health-partition posture into an ASP.NET Core-only
concept.
