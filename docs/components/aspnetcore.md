...
message id, channel id, operation id, actor id, correlation id, reason, dispatch outcome, or command
outcome separately from the latest per-outbox dispatch state. A duplicate command id does not
...
`/engine/event-dispatch-remediation-commands/retention` before issuing a new command id.
