...
- an explicit trim/AOT boundary on `MapCephalon()`: the full operator route surface still uses ASP.NET Core Minimal API delegate binding, so the public method carries `RequiresUnreferencedCode` and `RequiresDynamicCode`; package-local analyzer builds are clean, but this is not a Native AOT support claim
