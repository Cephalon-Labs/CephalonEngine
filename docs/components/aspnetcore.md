...
- an explicit trim/AOT boundary on `MapCephalon()`: the full operator route surface still contains ASP.NET Core Minimal API delegate binding, so the public method carries `RequiresUnreferencedCode` and `RequiresDynamicCode`; package-local analyzer builds are clean, and the deployment-mode harness audits that the manifest-backed route boundary still has both annotations, but this is not a Native AOT support claim. The opt-in `core` operator surface, the shared full/common routes, and the full/default behavior-resilience, saga-choreography, and durable-execution route families now map their bounded route subsets through prebuilt `RequestDelegate` handlers and `MapMethods(...)`; the harness emits `CoreRouteDelegateAuditStatus` and `FullCommonRouteDelegateAuditStatus` plus scorecard/release/doctor readback for those two generated proofs, and Pester guards the full/default workflow route block, but they remain route-subset proofs, not a full-adapter trim/AOT claim
