...
- an explicit trim/AOT boundary on `MapCephalon()`: the public method still carries `RequiresUnreferencedCode` and `RequiresDynamicCode` because the full adapter support row is unclaimed, even though the operator route catalog now avoids ASP.NET Core Minimal API delegate binding through prebuilt result/async `RequestDelegate` handlers, POST request delegates, `MapMethods(...)`, and source-generated request-body JSON metadata; package-local analyzer builds are clean, the deployment-mode harness audits that the manifest-backed route boundary still has both annotations, `CoreRouteDelegateAuditStatus` and `FullCommonRouteDelegateAuditStatus` keep the generated core/full-common proofs fail-closed, and Pester now guards the full/default operator catalog against direct `engineGroup.MapGet(...)` / `engineGroup.MapPost(...)` drift while response/output JSON and non-operator endpoint proof remain unclaimed
