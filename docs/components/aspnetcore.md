...
- an explicit trim/AOT boundary on `MapCephalon()`: the public method still carries `RequiresUnreferencedCode` and `RequiresDynamicCode` because the full adapter support row is unclaimed, even though the read-only operator GET catalog now avoids ASP.NET Core Minimal API delegate binding through prebuilt result/async `RequestDelegate` handlers and `MapMethods(...)`; package-local analyzer builds are clean, the deployment-mode harness audits that the manifest-backed route boundary still has both annotations, `CoreRouteDelegateAuditStatus` and `FullCommonRouteDelegateAuditStatus` keep the generated core/full-common proofs fail-closed, and Pester now guards the full/default read-only GET catalog against `engineGroup.MapGet(...)` drift while the six POST action seams remain explicit and unclaimed
