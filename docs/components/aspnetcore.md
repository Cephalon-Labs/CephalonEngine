...
`reporterLeaseExpiresAtUtc`, `observedEdgeNodeIds`, plus typed `observationFreshness` answers on
`/engine/cdc-captures/runtime*`, `/engine/cdc-capture-runtimes*`, and `snapshot` from the same
configured stale-window, reporter-lease, and edge-node policy instead of a host-only timeout or
topology monitor.
