...
`/engine/cdc-capture-runtimes/dry-runs/operations/{operationId}` so operator flows can query the
...
managed-connector dry-run posture directly instead of rebuilding a second HTTP-local remediation,
governance, drift, action-planning, readiness, preflight, or dry-run index.
