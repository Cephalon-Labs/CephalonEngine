...
`/engine/cells/modules/{moduleId}` when the engine-owned `ICellBoundaryCatalog` is active, while
`/engine/technology-surfaces/cell-based-architecture` keeps the same topology visible through the
existing technology-surface projection. That keeps the phase 13 cell baseline operator-facing
without turning cell topology into an ASP.NET Core-only concept.
