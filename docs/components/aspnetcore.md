...
- `/engine/cdc-captures/runtime` when the shared CDC runtime-state catalog is active
...
counts, checkpoints, errors, and optional linked `OutboxDispatchState` derived from the shared
runtime catalog instead of inventing an ASP.NET Core-only live CDC monitor.
