...
- an explicit trim/AOT boundary on `MapCephalon()`: the public method still carries `RequiresUnreferencedCode` and `RequiresDynamicCode` because the full adapter support row is unclaimed, even though the operator route catalog now avoids ASP.NET Core Minimal API delegate binding through prebuilt result/async `RequestDelegate` handlers, POST request delegates, `MapMethods(...)`, source-generated request-body JSON metadata, and source-generated operator response JSON contracts registered into HTTP and MVC JSON options; Cephalon-owned non-operator host/documentation GET endpoints now use app-level request-delegate helpers instead of direct `app.MapGet(...)`, while framework-owned health/OpenAPI/Scalar endpoint extensions remain the explicit adapter boundary; package-local analyzer builds are clean, the deployment-mode harness audits that the manifest-backed route boundary still has both annotations, `CoreRouteDelegateAuditStatus`, `FullCommonRouteDelegateAuditStatus`, `FullOperatorRouteDelegateAuditStatus`, `OperatorResponseJsonContractAuditStatus`, `NonOperatorEndpointAuditStatus`, and `FrameworkEndpointBoundaryAuditStatus` keep the generated proofs fail-closed, and Pester now guards the full/default operator catalog against direct `engineGroup.MapGet(...)` / `engineGroup.MapPost(...)` drift
...
- OpenAPI and Scalar integration for REST APIs, including XML-comment enrichment that resolves generated `.xml` files from the publish/output base directory instead of `Assembly.Location` so single-file publish analysis stays clean
- REST result-envelope OpenAPI metadata through `ResultModelEnvelopeResponseMetadata`, allowing adapters to describe `ResultModel<T>` success envelopes without constructing closed generic envelope types at runtime
...
The default `MapCephalon()` surface remains the full operator route catalog so existing hosts keep every `/engine/*` route family they already depend on. Cold-start-sensitive hosts can set `Engine:AspNetCore:OperatorSurface:Mode=core` before `AddCephalon(...)` / `MapCephalon()` to keep the core introspection, diagnostics, health, documentation, and selected public transport routes without paying to map optional drill-down families such as behavior-resilience, CDC runtime, eventing, agent-tool, retrieval, BFF, strangler-fig, cell, and data-product routes. That mode is additive and host-adapter-local: it does not change the engine manifest, module composition, selected transports, or package runtime catalogs. Its bounded operator routes now use request delegates instead of Minimal API delegate binding so the deployment-mode remediation lane has real runtime proof; `CoreRouteDelegateAuditStatus` makes that proof generated and fail-closed. The shared full/common routes (`/`, `/manifest`, `/snapshot`, `/app-model`, and `/resilience`) use the same helper in full mode and are guarded by `FullCommonRouteDelegateAuditStatus`. The full/default read-only GET route catalog now uses result/async request delegates too, including the CDC runtime, eventing, agentics, audit-history, strangler-fig, backend-for-frontend, cell, technology, and knowledge-index read families. The six POST operator actions now also use request delegates with source-generated request-body JSON metadata, covering CDC runtime report and command execution, event publication, agent-tool execution, knowledge queries, and manual reindex. `AddCephalon(...)` registers `AspNetCoreJsonSerializerContext.Default` into both HTTP and MVC JSON resolver chains so the core/full-common operator response shapes (`RuntimeManifest`, `RuntimeIntrospectionSnapshot`, `AppProfile`, runtime catalogs, policy snapshots, diagnostics, health, localization, and reference-doc metadata) have source-generated JSON metadata; `OperatorResponseJsonContractAuditStatus` keeps that response proof generated and fail-closed. `FullOperatorRouteDelegateAuditStatus` now promotes the whole full/default operator route-catalog proof into generated hazard-inventory, scorecard, release-validation, and doctor readback. Cephalon-owned non-operator host/documentation GET endpoints now use app-level request-delegate helpers and `NonOperatorEndpointAuditStatus` keeps that proof generated and fail-closed. `FrameworkEndpointBoundaryAuditStatus` separately counts the framework-owned `MapHealthChecks`, `MapOpenApi`, and `MapScalarApiReference` extension endpoints so those boundaries cannot disappear silently. The default/full surface still carries the explicit trim/AOT boundary because those framework-owned extension endpoints remain a deliberate adapter support boundary until a later support-promotion slice widens the claim.
...
runtime, including duplicate command ids rejected under the native `unique-command-id` /
`reject-without-mutation` policy, and keeps broker-specific dead-letter commands outside the claim
until a companion package owns that path.
...
`/engine/event-dispatch-remediation-commands/outcomes/{outcome}/latest` so operators can inspect accepted
...
outcome, inclusive observed UTC window, observed-window summary, filter-specific summary, or filter-specific latest command separately from the latest per-outbox dispatch state. A
...
through `/engine/event-dispatch-remediation-commands/{commandId}`,
`/engine/event-dispatch-remediation-commands/latest`,
...
retained cutoff.
The filter-summary routes return `EventDispatchRemediationRuntimeSummary` for retained command
records matching one outbox, message, channel, operation, actor, correlation id, reason, dispatch
...
latest, retention, in-doubt-summary, oldest-in-doubt, filter-summary, filter-latest, or single-command reads keep their existing contracts.
...
`commandInDoubtRoute`, `commandInDoubtSummaryRoute`, `commandOldestInDoubtRoute`, and the filter
...
in-doubt summary, command-id, outbox drill-down, filtered summary, or filtered latest URL from route names or docs.
...
command result exists, with route, in-doubt route/cutoff, in-doubt summary, oldest in-doubt route/cutoff, filter-summary route/policy, filter-latest route/policy, retention, read-limit, observed-window, idempotency,
