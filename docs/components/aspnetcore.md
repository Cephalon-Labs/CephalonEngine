...
`/engine/technology-surfaces/cell-based-architecture` keeps the same topology and routing posture
visible through the existing technology-surface projection. That keeps the phase 13 cell baseline
operator-facing without turning cell topology or traffic posture into an ASP.NET Core-only concept.
