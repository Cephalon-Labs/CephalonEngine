...
- a sanitized `telemetry-export-gcp` runtime surface for `/engine/technology-surfaces` and `/engine/snapshot`
...
- `Hosting/GcpTelemetryRuntimeContributor.cs`
...
When the package is activated, it contributes the `telemetry-export-gcp` runtime surface so operators can see the GCP observability pack is active without exposing collector endpoints or credential material.
