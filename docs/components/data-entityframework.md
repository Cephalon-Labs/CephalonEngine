...
The current recommendation is still not to turn `DbContext` inheritance into the primary engine contract. That contract now lives one layer higher: the engine ships `Engine:Databases`, projects it into `AppProfile.Databases`, and exposes the resolved topology through `/engine/databases`. `Cephalon.Data.EntityFramework` now consumes the engine-owned `write` and `read` role shape directly and uses the same app-model selection to decide whether startup schema apply should run, while any future convenience `DbContext` base classes stay thin and optional. Dedicated `outbox` and `history` migration execution are still future follow-through. See [Database topology](../database-topology.md).
