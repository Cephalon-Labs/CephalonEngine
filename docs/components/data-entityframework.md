...
- contributes the resolved physical-target truth that lets the engine-owned migration playbook and topology posture surface physical-target execution groups, grouped command paths, and warnings when multiple EF-backed logical migration targets share one physical database and still need coordinated deploy-time execution
