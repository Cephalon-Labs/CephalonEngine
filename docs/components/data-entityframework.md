...
The current recommendation is still not to turn `DbContext` inheritance into the primary engine contract. That contract now lives one layer higher: the engine ships `Engine:Databases`, projects it into `AppProfile.Databases`, and exposes the resolved topology through `/engine/databases`. `Cephalon.Data.EntityFramework` should consume that engine-owned shape instead of inventing a second physical-topology model, while any future convenience `DbContext` base classes stay thin and optional. See [Database topology](../database-topology.md).
