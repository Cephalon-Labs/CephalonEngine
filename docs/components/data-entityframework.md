...
- exposes an opt-in Entity Framework-backed event-dispatch remediation command journal for durable operator command reservations, command results, and command-id duplicate detection
...
- `Modeling/EntityFrameworkEventDispatchRemediationCommandEntry.cs`
- `Modeling/IEntityFrameworkEventDispatchRemediationCommandJournalContext.cs`
...
- `Services/EntityFrameworkEventDispatchRemediationCommandJournal.cs`
...
The current slice is intentionally honest and narrow: it proves optional `DbContext` registration, CQRS-style read/write role separation, runtime capability metadata, topology-aware resolution from `Engine:Databases`, startup schema apply through a generic-host hosted service for the registered `write` and optional `read` `DbContext` roles, an opt-in inbox persistence path where the write-side `DbContext` explicitly implements `IEntityFrameworkInboxContext` and maps the shared processed-message entity with `ConfigureCephalonInbox()`, an opt-in outbox persistence path where the same write-side `DbContext` can implement `IEntityFrameworkOutboxContext` and map the shared outbox entity with `ConfigureCephalonOutbox()`, and an opt-in durable remediation command journal where that write-side `DbContext` implements `IEntityFrameworkEventDispatchRemediationCommandJournalContext` and maps the shared command-result entity with `ConfigureCephalonEventDispatchRemediationCommandJournal()`. When `RegisterOutbox` is active and the context implements that journal interface, the pack registers `IEventDispatchRemediationCommandJournal` without adding a second string-config toggle; the active eventing dispatcher reserves command ids in the EF journal before dispatch-store mutation, updates the same row to the final accepted/rejected result, and ASP.NET Core `/engine/event-dispatch-remediation-commands*` reads then use the EF journal for command-id duplicate detection, in-doubt `reserved` visibility, durable operator history, and aggregate `ReservedCount` / `HasInDoubtCommands` summary posture. Operator-facing inbox and outbox descriptors make staged-only idempotency and delivery truth visible through the runtime introspection surface, an `inbox-stores` technology surface entry describes application-managed idempotency stores when `EventDrivenIntegration` is active, an `outbox-producers` technology surface entry under the same technology selection describes staged-publication posture, and the `data.entity-framework.event-dispatch-remediation-command-journal` capability advertises `journalStorage = entity-framework-table`, `journalDurability = durable`, `journalScope = cross-node`, and `crossNodeCommandAudit = true`. The same baseline also supplies subscription metadata that can say an application-managed inbox store is available without pretending the eventing pack dispatches through it, a staged-only outbox-backed `IEventPublisher` handoff path, an adapter-neutral Entity Framework-backed `IEventDispatchStore` that can read pending staged rows and persist `dispatch_attempt_count`, `dispatched_at_utc`, and `next_attempt_at_utc` follow-through for later adapter-owned dispatch loops, and an outbox descriptor that moves truthfully between `disabled`, `consumer-managed`, and runtime-managed dispatch ownership as eventing packs are added. The same baseline now also understands the engine-owned `UseRole` contract for dependent targets, so `Outbox` and `History` can explicitly reuse the concrete `write` connection while still layering local schema or runtime overrides. Full subscription execution, arbitrary role graphs, durable replay cursors, and richer projection pipelines are still later slices.
...
The durable remediation journal also participates in the ENG-581 reserved-age readback: global and observed-window summaries expose `OldestReservedCommandId` and `OldestReservedObservedAtUtc` from retained `reserved` EF rows, and the `event-dispatch-remediation-commands` technology surface mirrors those values for snapshot-driven operator tooling. ENG-582 extends the same durable journal read model with `GetInDoubt()`, so `/engine/event-dispatch-remediation-commands/in-doubt` can read retained reserved EF rows newest-first with the same `limit` and `pageSize` caps used by the process-local fallback. ENG-583 adds `GetInDoubtBefore(...)` and `beforeUtc` support on that route, letting the EF journal query stale reserved rows by an inclusive observed-UTC cutoff instead of forcing an operator UI to fetch and filter the whole reserved set. ENG-584 adds `GetOldestInDoubtBefore(...)` and `/engine/event-dispatch-remediation-commands/in-doubt/oldest?beforeUtc={beforeUtc}`, pushing the single oldest retained stale-reservation query into the EF provider and returning `404` at the host boundary when no retained row matches.
