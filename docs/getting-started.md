...
- the generated host-project `PackageReference` baseline, including `Cephalon.Analyzers` and `Cephalon.Engine.SourceGen` as private analyzer packages, generated REST behavior modules including `Cephalon.Behaviors.SourceGen` as a private analyzer package, plus `Configurations/**/*.json` copy/publish settings
...
- a generated host that keeps the scaffolded host-project `PackageReference` set, including `Cephalon.Analyzers` and `Cephalon.Engine.SourceGen` as private analyzer packages, generated REST behavior modules including `Cephalon.Behaviors.SourceGen` as a private analyzer package, plus `Configurations/**/*.json` copy/publish behavior intact shows `[ok]` for the generated host project baseline before teams rely on split project configuration at build or publish time
...
For the microservice REST + JSON-RPC + gRPC replay, run:

```powershell
pwsh ./scripts/validate-microservice-multi-transport-adoption.ps1
```

That script publishes the same temporary feed including `Cephalon.AspNetCore.JsonRpc`, `Cephalon.AspNetCore.Grpc`, `Cephalon.Observability.HttpDependencies`, and `Cephalon.Observability.DependencyHealth.Core`, installs the CLI, scaffolds a `Microservice` host with `RestApi`, `JsonRpc`, and `Grpc`, validates the generated module implements REST behavior profiles plus `IJsonRpcModule` and `IGrpcModule`, runs the generated host over HTTP/1.1 and HTTP/2, calls `/api/v1/platform/status`, `/json-rpc/platform`, and `DiscoveryService.SayHello` through `/grpc`, and probes `/engine/transports`, `/engine/snapshot`, `/engine/dependencies`, `/engine/diagnostics`, `/engine/runtime-story`, `/engine/technology-surfaces/json-rpc`, `/engine/technology-surfaces/grpc`, and `/scalar`. It writes `artifacts/adoption-smoke/microservice-multi-transport-adoption.json` unless `-ReportPath` overrides it.

For the SaaS tenant governance, invitation delivery, and audit replay, run:

```powershell
pwsh ./scripts/validate-saas-tenant-governance-audit-adoption.ps1
```

That script publishes the same temporary feed including `Cephalon.MultiTenancy.Governance`, `Cephalon.MultiTenancy.Governance.AspNetCore`, `Cephalon.Audit`, `Cephalon.AspNetCore.JsonRpc`, `Cephalon.AspNetCore.Grpc`, `Cephalon.Observability.HttpDependencies`, and `Cephalon.Observability.DependencyHealth.Core`, installs the CLI, scaffolds a modular-monolith tenant host with `MultiTenancy`, `RestApi`, `JsonRpc`, and `Grpc`, wires a smoke invitation-delivery sender plus audit recording, runs the generated host over HTTP/1.1 and HTTP/2, issues a tenant-administration invitation command, dispatches the invitation, reconciles and reads delivery-status observations, records an audit proof event, and probes `/engine/tenant-administration/commands`, `/engine/tenant-invitations/delivery-dispatches`, `/engine/tenant-invitations/delivery-status`, `/engine/tenant-invitations/delivery-status/observations`, `/engine/audit-stores`, `/engine/technology-surfaces/multi-tenancy`, `/engine/dependencies`, `/engine/transports`, `/engine/snapshot`, `/scalar`, REST, JSON-RPC, and gRPC business endpoints. It writes `artifacts/adoption-smoke/saas-tenant-governance-audit.json` unless `-ReportPath` overrides it.
...
For a repo-native cold-start proof of that full template-pack path, use `pwsh ./scripts/validate-template-pack-adoption.ps1`; it publishes the template-pack feed with the generated app package closure including `Cephalon.Analyzers`, `Cephalon.Diagnostics`, and `Cephalon.Resilience`, then writes `artifacts/adoption-smoke/template-pack-adoption.json` unless `-ReportPath` overrides it.
For the matching microservice multi-transport replay, use `pwsh ./scripts/validate-microservice-multi-transport-adoption.ps1`. That script publishes the same temporary feed with the generated app package closure including `Cephalon.Analyzers`, `Cephalon.AspNetCore.JsonRpc`, `Cephalon.AspNetCore.Grpc`, `Cephalon.Observability.HttpDependencies`, and `Cephalon.Observability.DependencyHealth.Core`, installs `Cephalon.Cli`, scaffolds a `Microservice` with `RestApi`, `JsonRpc`, and `Grpc`, runs the generated host over HTTP/1.1 and HTTP/2, validates the generated REST, JSON-RPC, and gRPC endpoints, checks the HTTP dependency-health self-probe, and writes `artifacts/adoption-smoke/microservice-multi-transport-adoption.json` unless `-ReportPath` overrides it.
For the matching SaaS tenant governance and audit replay, use `pwsh ./scripts/validate-saas-tenant-governance-audit-adoption.ps1`. That script adds the governance, governance ASP.NET Core, audit, JSON-RPC, gRPC, and HTTP dependency-health companions to a generated modular-monolith tenant host, proves invitation issue/dispatch/status observation plus audit-store truth, and writes `artifacts/adoption-smoke/saas-tenant-governance-audit.json` unless `-ReportPath` overrides it.
For the matching out-of-tree package parity replay, use `pwsh ./scripts/validate-out-of-tree-package-adoption.ps1`. That script publishes the same temporary feed with the generated app package closure including `Cephalon.Analyzers`, `Cephalon.Behaviors.SourceGen`, `Cephalon.Diagnostics`, and `Cephalon.Resilience`, installs `Cephalon.Cli`, scaffolds a fresh app outside the repository, packs and stages `Cephalon.ReferenceModule.Operations` through `cephalon package stage`, patches `Engine:Discovery:PackageDirectories` plus `Engine:PackagePolicy` and `Engine:Trust`, reruns `cephalon doctor --app-root <path>`, restores, builds, runs, validates `/api/operations/status`, `/engine/packages`, `/engine/package-policy`, `/engine/trust-policy`, and `/engine/snapshot`, and writes `artifacts/adoption-smoke/out-of-tree-package-adoption.json` unless `-ReportPath` overrides it. The release-readiness scorecard validates generated-app foundation, template-pack parity, modular-monolith REST/Worker/data, vertical-slice Eventing/outbox, microservice multi-transport, SaaS tenant governance/audit, and out-of-tree package replay scripts, docs, reference-module project, assertions, runtime probes, per-use-case execution-report contracts, and the golden use-case map through `scripts/adoption-smoke-support.json`.
