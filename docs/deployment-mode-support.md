...
`ENG-476` then retires the broader `Cephalon.Engine` module-discovery blocker by moving assembly-based module discovery onto generated `ModuleDiscoveryDescriptor` metadata registered by `Cephalon.Engine.SourceGen`. `ENG-477` follows by moving the remaining MySQL SciSharp reflective transport path out of `Cephalon.Data.MySql` and into the optional `Cephalon.Data.MySql.SciSharpReplication` adapter package, so the core MySQL package no longer carries that third-party reflection dependency. The current manifest has 6 package entries, 2 packages with known hazards, 14 known hazard entries, 2 `high` + 0 `medium` + 3 `excluded-by-design` + 1 `clean-baseline` tier counts, zero active `low` entries, 1 package-scoped `singleFile` claim, and the same transitive-hazard hint counts. Remaining global deployment-mode gating work is now the optional `Cephalon.Data.MySql.SciSharpReplication` transport adapter posture, the by-design `Cephalon.ReferenceDocs` posture, transitive validation, and the deliberate decision to turn publish probes into a release gate.
