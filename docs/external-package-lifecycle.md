...
For the repo-native replay of this full out-of-tree path, use `pwsh ./scripts/validate-out-of-tree-package-adoption.ps1`. It publishes the generated app package closure, including `Cephalon.Behaviors.SourceGen`, `Cephalon.Diagnostics`, and `Cephalon.Resilience`, writes `artifacts/adoption-smoke/out-of-tree-package-adoption.json` by default, with `-ReportPath` available for release artifact folders. The scorecard read model validates that replay and execution-report contract through `scripts/adoption-smoke-support.json` and emits the package-stage, trust-policy, host-run, runtime-probe coverage, and execution-report metadata as `AdoptionSmokeEvidence`.
