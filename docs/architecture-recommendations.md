...
Current state: the contract-first baseline is now shipped through `Engine:Resilience:CircuitBreaker`, `AppProfile.Resilience`, and `/engine/resilience`, and the behavior pipeline now also enforces circuit-breaker policy through `Cephalon.Behaviors`, `/engine/behavior-resilience`, and `snapshot.BehaviorResiliencePolicies`. The runtime publishes live circuit metadata such as current state, last-open timestamps, retry-after timing, and the last exception type that opened the circuit, and `Cephalon.Behaviors.Http` now translates open-circuit rejections into truthful REST `503` responses plus per-route OpenAPI metadata while the generic GraphQL, JSON-RPC, SSE, and WebSocket behavior HTTP adapters keep the same open-circuit truth in protocol-native envelopes. Direct `IGrpcModule`, `IJsonRpcModule`, `IServerSentEventsModule`, and `IWebSocketModule` endpoints now also have adapter-owned open-circuit handling through `grpc-direct-module-resilience`, `json-rpc-direct-module-resilience`, `sse-direct-module-resilience`, and `websocket-direct-module-resilience`; built-in GraphQL query, mutation, and subscription root-field execution now has the same adapter-owned posture through `graphql-execution-resilience`, so Wolverine remains optional for those public transport lanes.
...
- Provider-specific non-REST transport adapters that need their own native fault envelopes beyond the ASP.NET Core direct gRPC, JSON-RPC, SSE, WebSocket, and built-in GraphQL execution baselines
...
- richer provider-native execution semantics and deeper source-specific posture beyond the shipped typed freshness/lag/publication baseline and the shipped SQL Server checkpoint / PostgreSQL slot-confirmed-flush / MongoDB resume-token / MySQL binlog checkpoint / Oracle LogMiner checkpoint live proofs plus Oracle LogMiner lifecycle/resume runtime proof
...
Effort: medium for additional provider-specific follow-through; the MongoDB / SQL Server / PostgreSQL / MySQL / Oracle live-provider evidence baseline is now shipped, while later work should focus on new providers, deeper operator automation, or alternate CDC execution topologies.
