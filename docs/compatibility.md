...
- for `Cephalon.MultiTenancy`, the base-pack managed proof is the configuration-driven tenant-resolution and ambient-context path around `ITenantResolver` and `ITenantContextAccessor`; `Cephalon.MultiTenancy.Governance` now owns companion proofs for membership cataloging/evaluation through `ITenantMembershipCatalog` and `ITenantMembershipEvaluator`, invitation cataloging/validation through `ITenantInvitationCatalog` and `ITenantInvitationValidator`, declared domain-ownership cataloging/validation through `ITenantDomainOwnershipCatalog` and `ITenantDomainOwnershipValidator`, and approval/remediation action cataloging/decision through `ITenantGovernanceActionCatalog` and `ITenantGovernanceActionDecider`, while DNS/HTTP verification execution, human approval workflow execution, remediation execution, durable governance storage, invitation delivery, identity-provider synchronization, and tenant administration remain outside the compatibility promise until a package owns them explicitly
