...
- for `Cephalon.Eventing`, the core pack owns channel and subscription descriptors, staged publication, application-managed runtime reporting, the public `IEventSubscriptionExecutionBindingCatalog` binding read seam, the public `IEventSubscriptionExecutionReadinessCatalog` readiness read seam, and stable `EventSubscriptionRuntimeMetadataKeys`; Wolverine remains an optional `provider-managed` proof over those seams, not an engine requirement or a generic broker/inbox ownership claim
