...
- for `Cephalon.MultiTenancy`, the current managed proof is the configuration-driven tenant-resolution and ambient-context path around `ITenantResolver` and `ITenantContextAccessor`; tenant membership, invitations, domain ownership, and governance/approval workflows are taxonomy-only companion-boundary entries until a package such as `Cephalon.MultiTenancy.Governance` owns them explicitly
