...
- for `Cephalon.Eventing`, the core pack owns channel and subscription descriptors, staged publication, application-managed runtime reporting, the public `IEventSubscriptionExecutionBindingCatalog` read seam, and stable `EventSubscriptionRuntimeMetadataKeys`; Wolverine remains an optional `provider-managed` proof over that seam, not an engine requirement or a generic broker/inbox ownership claim
