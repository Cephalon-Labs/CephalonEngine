...
- for `Cephalon.Agentics`, the current managed/operator proof is the dispatcher-plus-run-state path around registered `IAgentToolExecutor` services, including opt-in bounded process-local retry posture through `ExecutionMaxAttempts`, `retry-scheduled` observations, retry counters, and `/engine/agent-tool-runs/retry-pending`; `IAgentToolDispatcher`, `AgentToolExecutionRequest`, `AgentToolExecutionResult`, and `IAgentToolRunCatalog` now live in `Cephalon.Abstractions.Agentics` so hosts can expose `/engine/agent-tool-runs*`, `POST /engine/agent-tools/{toolId}/runs`, and `snapshot.AgentToolRuns` without depending on implementation types; broader autonomous planning, memory persistence, durable retry queues, distributed scheduling, or AI-provider orchestration are not part of that compatibility promise until a package owns them explicitly
