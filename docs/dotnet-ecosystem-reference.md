...
For eventing, that rule now has a runtime checkpoint: `Cephalon.Eventing` emits `eventing-superiority-profile` plus capability `eventing.superiority-profile` so the active runtime can say which MassTransit/NServiceBus/Wolverine/MediatR-inspired dimensions are `claimed`, `partial`, or `not-claimed` before docs or roadmap items describe them as complete, including durable remediation command audit plus provider-owned command-journal replay cursor posture from the active command-journal descriptor without claiming broker replay ownership.
