...
The component catalog now includes the observability baseline package plus the optional HTTP dependency-health, Postgres dependency-health, RabbitMQ dependency-health, Redis dependency-health, and OpenTelemetry exporter companions so operator-facing docs stay aligned with the shipped host integration paths.
