...
| [`sre-posture.md`](sre-posture.md) + [`benchmarking.md`](benchmarking.md) + [`../scripts/sre-posture-support.json`](../scripts/sre-posture-support.json) | Engine SLIs, SLOs, benchmark guardrails, error-budget rules | Checks operational reliability and performance evidence, including which SLI targets are declared, which SLIs are mapped to benchmark guardrail entries, and which baselines are still pending. |
...
The JSON artifact uses schema version `1.8.0`. Release validation publishes it by default through `scripts/validate-release.ps1` unless `-SkipEngineCompletionScorecard` is passed for a deliberately narrower local run. The release-validation scorecard step also prints deployment-mode global-claim, package-scoped-claim, known-hazard, transitive-audit, and publish-probe mode counts beside provider integration row/live/composition/external-gate/default-skip/runtime-contract counts plus dependency-health provider-manifest reference/schema/status/provider-count readback, SRE SLI count, target-declared count, pending stable-baseline count, stable-baseline count, SRE guardrail-mapped/pending/not-applicable coverage counts, supply-chain workflow/external-policy counts, and public API package/pending/addition/removal counts from the generated artifact. The script-level Pester suite now dot-sources release validation with `CEPHALON_VALIDATE_RELEASE_NO_RUN` and proves the dependency-health provider-manifest readback prints from the generated JSON shape and fails when `ProviderIntegrationEvidence.DependencyHealthProviderManifest` is missing. It then fails closed on scorecard hard blockers: blocked platform gates, supply-chain blocked evidence items, or public API removal entries. The later public-API delta summary step writes `artifacts/public-api-delta-release/public-api-delta.md` plus `artifacts/public-api-delta-release/public-api-delta.json` and passes `-FailOnRemovals`, so removal entries still leave reviewable Markdown and machine-readable JSON artifacts before they fail the release run.

For a local release-manager readback, `cephalon doctor --scorecard <path-to-engine-completion-scorecard.json>` summarizes that generated artifact's schema/source documents, platform-gate posture, validated evidence-reference count, per-package GA-readiness count, deployment-mode counts from `DeploymentModeEvidence` (global claim rows, global `not-claimed` rows, package-scoped claim packages, known hazard packages/entries, transitive audit entries, and publish-probe mode), provider integration counts from `ProviderIntegrationEvidence` (evidence rows, live proofs, composition-only rows, external-service gates, default-skipped rows, runtime contracts, and the `DependencyHealthProviderManifest` reference/schema/status/provider count), SRE posture counts from `SrePostureEvidence` (SLIs, target-declared SLIs, pending stable baselines, stable baselines, and guardrail coverage counts), supply-chain release evidence counts from `SupplyChainEvidence` (evidence items, workflow-ready items, external-policy-pending items, and blocked items), and public API compatibility counts from `PublicApiCompatibilityEvidence` (package baselines, pending packages, additions, and removals). The CLI consumes the generated JSON read model only; it does not parse this Markdown page, replace source documents, or promote support/GA claims by itself.
...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, benchmark guardrail coverage references, release-validation readback, and source docs; the current mapped set covers four SLIs including ASP.NET Core request allocation, and `pending-stable-baseline` entries should be promoted only after the dedicated stable-baseline pass. |
...
7. Keep SRE posture evidence aligned with `scripts/sre-posture-support.json`, `docs/sre-posture.md`, `docs/benchmarking.md`, benchmark guardrails, and release-validation readback so the current four guardrail-mapped SLIs stay machine-checkable and declared targets do not read as stable baselines before the baseline pass lands.
