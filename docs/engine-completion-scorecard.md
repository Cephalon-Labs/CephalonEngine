...
| [`deployment-mode-support.md`](deployment-mode-support.md) + `scripts/deployment-mode-support.json` | trim, Native AOT, single-file, publish-probe support posture, and manifest-backed hazard inventory | Controls deployment-mode promotion; `claim-truthful` is required before support claims widen, `PublishProbePolicy` explains which mode release validation publishes by default, `PublishProbeGate` records whether the gated probe passed, and `hazard-inventory.json` makes tier/scoped-claim/transitive-hazard counts plus the lock-file-backed transitive audit visible in validation artifacts. |
...
The JSON artifact uses schema version `1.9.0`. Release validation publishes it by default through `scripts/validate-release.ps1` unless `-SkipEngineCompletionScorecard` is passed for a deliberately narrower local run. The release-validation scorecard step also prints deployment-mode global-claim, package-scoped-claim, known-hazard, transitive-audit, and publish-probe mode counts beside provider integration row/live/composition/external-gate/default-skip/runtime-contract counts plus dependency-health provider-manifest reference/schema/status/provider-count readback, SRE SLI count, target-declared count, pending stable-baseline count, stable-baseline count, stable-baseline manifest/row/measurement counts, SRE guardrail-mapped/pending/not-applicable coverage counts, supply-chain workflow/external-policy counts, and public API package/pending/addition/removal counts from the generated artifact. The script-level Pester suite now dot-sources release validation with `CEPHALON_VALIDATE_RELEASE_NO_RUN` and proves the dependency-health provider-manifest readback plus SRE stable-baseline manifest readback print from the generated JSON shape and fail when required generated nodes are missing. It then fails closed on scorecard hard blockers: blocked platform gates, supply-chain blocked evidence items, or public API removal entries. The later public-API delta summary step writes `artifacts/public-api-delta-release/public-api-delta.md` plus `artifacts/public-api-delta-release/public-api-delta.json` and passes `-FailOnRemovals`, so removal entries still leave reviewable Markdown and machine-readable JSON artifacts before they fail the release run.

For a local release-manager readback, `cephalon doctor --scorecard <path-to-engine-completion-scorecard.json>` summarizes that generated artifact's schema/source documents, platform-gate posture, validated evidence-reference count, per-package GA-readiness count, deployment-mode counts from `DeploymentModeEvidence` (global claim rows, global `not-claimed` rows, package-scoped claim packages, known hazard packages/entries, transitive audit entries, and publish-probe mode), provider integration counts from `ProviderIntegrationEvidence` (evidence rows, live proofs, composition-only rows, external-service gates, default-skipped rows, runtime contracts, and the `DependencyHealthProviderManifest` reference/schema/status/provider count), SRE posture counts from `SrePostureEvidence` (SLIs, target-declared SLIs, pending stable baselines, stable baselines, stable-baseline manifest rows/measurements, and guardrail coverage counts), supply-chain release evidence counts from `SupplyChainEvidence` (evidence items, workflow-ready items, external-policy-pending items, and blocked items), and public API compatibility counts from `PublicApiCompatibilityEvidence` (package baselines, pending packages, additions, and removals). The CLI consumes the generated JSON read model only; it does not parse this Markdown page, replace source documents, or promote support/GA claims by itself.
...
10. Keep release validation fail-closed on machine-readable hard blockers from the generated scorecard: blocked platform gates, supply-chain blocked evidence items, and public API removals. Keep `partial`, `not-claimed`, remaining provider composition-only rows, SRE pending baselines, and external-policy-pending supply-chain items visible as readback/exception posture rather than accidental hard blockers.
