...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, benchmark guardrail coverage references, release-validation readback, and source docs; promote `pending-stable-baseline` entries only after the dedicated stable-baseline pass. |
...
7. Keep SRE posture evidence aligned with `scripts/sre-posture-support.json`, `docs/sre-posture.md`, `docs/benchmarking.md`, benchmark guardrails, and release-validation readback so guardrail-mapped SLIs are machine-checkable and declared targets do not read as stable baselines before the baseline pass lands.
