...
| [`sre-posture.md`](sre-posture.md) + [`benchmarking.md`](benchmarking.md) + [`../scripts/sre-posture-support.json`](../scripts/sre-posture-support.json) | Engine SLIs, SLOs, benchmark guardrails, error-budget rules | Checks operational reliability and performance evidence, including which SLI targets are declared and which baselines are still pending. |
...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, release-validation readback, and source docs; promote `pending-stable-baseline` entries only after the dedicated stable-baseline pass. |
...
7. Keep SRE posture evidence aligned with `scripts/sre-posture-support.json`, `docs/sre-posture.md`, `docs/benchmarking.md`, benchmark guardrails, and release-validation readback so declared targets do not read as stable baselines before the baseline pass lands.
