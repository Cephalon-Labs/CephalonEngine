...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, benchmark guardrail coverage references, stable-baseline manifest, release-validation readback, and source docs; the current stable subset covers four benchmark-backed SLIs, one deployment-mode claims-report SLI, plus restore locked-mode, reference-docs, and full validate-release wall-time SLIs, while three cold-start and flake-rate SLIs remain `pending-stable-baseline`. |
