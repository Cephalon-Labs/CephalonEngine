...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, benchmark guardrail coverage references, stable-baseline manifest, release-validation readback, and source docs; the current stable subset covers four benchmark-backed SLIs plus one deployment-mode claims-report SLI, while six cold-start, wall-time, and flake-rate SLIs remain `pending-stable-baseline`. |
...
7. Keep SRE posture evidence aligned with `scripts/sre-posture-support.json`, `scripts/sre-stable-baselines.json`, `docs/sre-posture.md`, `docs/benchmarking.md`, benchmark guardrails, deployment-mode claims-report readback, and release-validation readback so the four benchmark-backed stable baselines plus the deployment-mode claim-truth baseline stay machine-checkable while the six remaining non-evidenced SLIs remain pending until their own baseline lanes land.
