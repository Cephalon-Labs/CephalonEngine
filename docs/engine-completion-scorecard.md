...
| [`../scripts/provider-integration-support.json`](../scripts/provider-integration-support.json) + [`../scripts/observability-dependency-health-providers.json`](../scripts/observability-dependency-health-providers.json) | Provider live-proof, composition-only, external-service-gate, runtime-contract, and dependency-health invariant evidence | Checks whether provider claims have machine-readable proof rows before release readback; repo-owned disposable live lanes can run by default, externally managed live provider lanes remain opt-in, composition-only rows stay explicit, and the dependency-health provider family shape plus provider-integration rows are guarded by a source-derived manifest. |
