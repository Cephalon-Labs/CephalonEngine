...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, benchmark guardrail coverage references, stable-baseline manifest, release-validation readback, and source docs; the current stable subset covers four benchmark-backed SLIs, one deployment-mode claims-report SLI, plus restore locked-mode and reference-docs wall-time SLIs, while four cold-start, release-validation wall-time, and flake-rate SLIs remain `pending-stable-baseline`. |
...
7. Keep SRE posture evidence aligned with `scripts/sre-posture-support.json`, `scripts/sre-stable-baselines.json`, `docs/sre-posture.md`, `docs/benchmarking.md`, benchmark guardrails, deployment-mode claims-report readback, and release-validation readback so the four benchmark-backed stable baselines, deployment-mode claim-truth baseline, restore/reference-docs wall-time baselines, and cold-start guardrail mappings stay machine-checkable while the four remaining non-evidenced stable baselines remain pending until their own lanes land.
