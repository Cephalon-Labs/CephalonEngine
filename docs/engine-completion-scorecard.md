...
| [`../scripts/summarise-public-api-deltas.ps1`](../scripts/summarise-public-api-deltas.ps1) + `src/Cephalon.*/PublicAPI.*.txt` | Per-package public API delta files, shipped baselines, additive entries, removal entries, and the release-validation removal gate | Checks that the public API compatibility gate has machine-readable package counts and removal counts instead of prose-only review posture; release validation runs the delta summary with `-FailOnRemovals`. |
...
| Public API compatibility | `partial`; removal entries are now a default release-validation failure through `scripts/summarise-public-api-deltas.ps1 -FailOnRemovals`, while stable package-validation baselines remain future GA work | No | Yes | Keep generated `PublicApiCompatibilityEvidence` aligned with `PublicAPI.Unshipped.txt` / `PublicAPI.Shipped.txt`, keep the removal gate wired, and enable package validation baselines when stable packages ship. |
...
6. Keep public API delta publishing aligned with `scripts/summarise-public-api-deltas.ps1 -FailOnRemovals` and the per-package `PublicAPI.*.txt` files so the generated artifact and CLI readback keep reporting the same package/addition/removal counts while release validation fails closed on removals.
