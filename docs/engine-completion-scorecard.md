...
| Public API compatibility | `partial`; removal entries are now a default release-validation failure through `scripts/summarise-public-api-deltas.ps1 -JsonOutputPath <path> -FailOnRemovals`, while stable package-validation baselines remain future GA work | No | Yes | Keep generated `PublicApiCompatibilityEvidence` aligned with `PublicAPI.Unshipped.txt` / `PublicAPI.Shipped.txt`, keep the Markdown/JSON delta artifacts and removal gate wired, and enable package validation baselines when stable packages ship. |
...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, benchmark guardrail coverage references, stable-baseline manifest, pending-baseline blocker rows, release-validation readback, and source docs; the current stable subset covers four benchmark-backed hot-path SLIs, the Worker cold-start mean-baseline proxy, one deployment-mode claims-report SLI, plus restore locked-mode, reference-docs, and full validate-release wall-time SLIs, while ASP.NET Core cold-start and flake-rate remain `pending-stable-baseline` with machine-readable blockers. |
