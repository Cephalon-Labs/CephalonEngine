...
For a local release-manager readback, `cephalon doctor --scorecard <path-to-engine-completion-scorecard.json>` summarizes that generated artifact's schema/source documents, platform-gate posture, validated evidence-reference count, per-package GA-readiness count, deployment-mode counts from `DeploymentModeEvidence` (global claim rows, global `not-claimed` rows, package-scoped claim packages, known hazard packages/entries, transitive audit entries, and publish-probe mode), provider integration counts from `ProviderIntegrationEvidence` (evidence rows, live proofs, composition-only rows, external-service gates, default-skipped rows, and runtime contracts), SRE posture counts from `SrePostureEvidence` (SLIs, target-declared SLIs, pending stable baselines, stable baselines, and guardrail coverage counts), supply-chain release evidence counts from `SupplyChainEvidence` (evidence items, workflow-ready items, external-policy-pending items, and blocked items), and public API compatibility counts from `PublicApiCompatibilityEvidence` (package baselines, pending packages, additions, and removals). The CLI consumes the generated JSON read model only; it does not parse this Markdown page, replace source documents, or promote support/GA claims by itself.
...
4. Keep `cephalon doctor --scorecard <path>` aligned with the generated JSON schema, including `DeploymentModeEvidence`, `ProviderIntegrationEvidence`, `SrePostureEvidence`, `SupplyChainEvidence`, and `PublicApiCompatibilityEvidence`, so local summaries remain a readback of the scorecard artifact, not a second source of truth.
