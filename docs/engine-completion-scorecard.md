...
4. Keep `cephalon doctor --scorecard <path>` aligned with the generated JSON schema, including `DeploymentModeEvidence`, `SrePostureEvidence`, `SupplyChainEvidence`, and `PublicApiCompatibilityEvidence`, so local summaries remain a readback of the scorecard artifact, not a second source of truth.
