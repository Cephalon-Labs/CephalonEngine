...
10. Keep release validation fail-closed on machine-readable hard blockers from the generated scorecard: blocked platform gates, supply-chain blocked evidence items, and public API removals. Keep `partial`, `not-claimed`, provider composition-only rows, SRE pending baselines, and external-policy-pending supply-chain items visible as readback/exception posture rather than accidental hard blockers.
