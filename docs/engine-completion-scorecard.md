...
| [`deployment-mode-support.md`](deployment-mode-support.md) + `scripts/deployment-mode-support.json` | trim, Native AOT, single-file, publish-probe support posture, and manifest-backed hazard inventory | Controls deployment-mode promotion; `claim-truthful` is required before support claims widen, `PublishProbePolicy` explains whether release validation is still audit-only, and `hazard-inventory.json` makes tier/scoped-claim/transitive-hazard counts plus the lock-file-backed transitive audit visible in validation artifacts. |
