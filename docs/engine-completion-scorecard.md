...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, benchmark guardrail coverage references, stable-baseline manifest, release-validation readback, and source docs; the current stable subset covers four benchmark-backed SLIs, while seven cold-start, wall-time, flake-rate, and deployment-mode truth SLIs remain `pending-stable-baseline`. |
...
7. Keep SRE posture evidence aligned with `scripts/sre-posture-support.json`, `scripts/sre-stable-baselines.json`, `docs/sre-posture.md`, `docs/benchmarking.md`, benchmark guardrails, and release-validation readback so the four benchmark-backed stable baselines stay machine-checkable while the seven non-evidenced SLIs remain pending until their own baseline lanes land.
