...
The JSON artifact uses schema version `1.4.0`. Release validation publishes it by default through `scripts/validate-release.ps1` unless `-SkipEngineCompletionScorecard` is passed for a deliberately narrower local run. The release-validation scorecard step also prints the SRE SLI count, target-declared count, pending stable-baseline count, and stable-baseline count from the generated artifact.

For a local release-manager readback, `cephalon doctor --scorecard <path-to-engine-completion-scorecard.json>` summarizes that generated artifact's schema/source documents, platform-gate posture, validated evidence-reference count, per-package GA-readiness count, SRE posture counts from `SrePostureEvidence` (SLIs, target-declared SLIs, pending stable baselines, and stable baselines), and public API compatibility counts from `PublicApiCompatibilityEvidence` (package baselines, pending packages, additions, and removals). The CLI consumes the generated JSON read model only; it does not parse this Markdown page, replace source documents, or promote support/GA claims by itself.
...
4. Keep `cephalon doctor --scorecard <path>` aligned with the generated JSON schema, including `SrePostureEvidence` and `PublicApiCompatibilityEvidence`, so local summaries remain a readback of the scorecard artifact, not a second source of truth.
