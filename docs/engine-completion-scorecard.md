...
| Engine SRE posture | `partial` | No | Yes | Keep generated `SrePostureEvidence` aligned with declared SLI targets, benchmark guardrail coverage references, stable-baseline manifest, release-validation readback, and source docs; the current stable subset covers four benchmark-backed hot-path SLIs, the Worker cold-start mean-baseline proxy, one deployment-mode claims-report SLI, plus restore locked-mode, reference-docs, and full validate-release wall-time SLIs, while ASP.NET Core cold-start and flake-rate remain `pending-stable-baseline`. |
