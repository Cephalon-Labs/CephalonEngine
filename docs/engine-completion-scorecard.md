...
| [`../scripts/provider-integration-support.json`](../scripts/provider-integration-support.json) | Provider live-proof, composition-only, external-service-gate, runtime-contract, and dependency-health invariant evidence | Checks whether provider claims have machine-readable proof rows before release readback; live provider lanes remain opt-in and composition-only rows stay explicit. |
...
| Observability, diagnostics, and cloud/provider packs | `ready-for-preview` for current companion posture | Keep SLI emission, event-id registry, dependency-health provider invariants, provider docs, and operational validation aligned. |
