...
| [`../scripts/provider-integration-support.json`](../scripts/provider-integration-support.json) + [`../scripts/observability-dependency-health-providers.json`](../scripts/observability-dependency-health-providers.json) + [`../scripts/run-provider-live-testcontainers.ps1`](../scripts/run-provider-live-testcontainers.ps1) + [`../.github/workflows/provider-live-testcontainers.yml`](../.github/workflows/provider-live-testcontainers.yml) | Provider live-proof, composition-only, external-service-gate, runtime-contract, dependency-health invariant, and Docker-capable live-execution evidence | Checks whether provider claims have machine-readable proof rows before release readback; repo-owned disposable live lanes can run by default, externally managed live provider lanes remain opt-in, composition-only rows stay explicit, dependency-health provider family shape plus provider-integration rows are guarded by a source-derived manifest, and the heavy Testcontainers provider proof has a scheduled/manual CI lane outside default release validation. |
...
| Data, eventing, event-sourcing, CDC | `partial` because many provider lanes exist with different ownership levels | Do not collapse provider-managed, application-managed, and cephalon-managed ownership; keep durable/distributed claims explicit and keep `ProviderIntegrationEvidence` live-proof/composition-only posture aligned with the actual test lanes. |
...
9. Keep provider integration evidence aligned with `scripts/provider-integration-support.json`, `scripts/observability-dependency-health-providers.json`, `scripts/run-provider-live-testcontainers.ps1`, `.github/workflows/provider-live-testcontainers.yml`, provider live-test gates, data provider runtime-surface tests, CDC integration lanes, and dependency-health provider invariants so "works with every provider" remains machine-checkable instead of aspirational prose. The non-relational provider live lanes must continue documenting both supported runtime modes: pre-provisioned services and the explicit `CEPHALON_PROVIDER_TESTCONTAINERS=1` disposable-provider path, and the Docker-capable workflow must keep invoking the shared script instead of duplicating provider filters in YAML. Dependency-health provider-integration rows must continue failing closed when their provider name, generated row id, component doc, invariant test, run posture, or runtime contracts drift from the source-derived provider manifest, and release-validation script tests must keep proving stale artifacts cannot omit the manifest readback.
10. Keep release validation fail-closed on machine-readable hard blockers from the generated scorecard: blocked platform gates, supply-chain blocked evidence items, public API removals, and invalid deployment-mode claims-report gate readback. Keep `partial`, `not-claimed`, remaining provider composition-only rows, SRE pending baselines, and external-policy-pending supply-chain items visible as readback/exception posture rather than accidental hard blockers.
