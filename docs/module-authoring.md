...
10. Use `ICellBoundaryContributor` and `ICellRouteContributor` when the package owns explicit cell topology or governed cell-to-cell paths that operators should be able to inspect through `/engine/cells`, `/engine/cell-routes`, `/engine/technology-surfaces/cell-based-architecture`, and `/engine/snapshot`.
