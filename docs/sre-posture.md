...
- **`scripts/measure-ci-flake-rate.ps1`** output — queries GitHub Actions run and attempt metadata, writes `artifacts/sre-ci-flake-rate/ci-flake-rate.json`, and only marks the flake-rate lane promotable when a completed 7-day window is measured within target
