...
- current provider-native dispatch-store coverage now includes Entity Framework, MongoDB, Redis, Elasticsearch, OpenSearch, Neo4j, and Qdrant; keep Cassandra, ClickHouse, and NATS as explicit storage-model follow-through work instead of forcing parity claims before their durable query/update semantics are truthful
