...
- the showcase database-topology operator projection now layers five complementary sample-level answers on top of the raw engine routes: a readiness summary, an ordered operator action plan, derived operator insights, an ordered migration playbook, and a Markdown operator brief export at `/api/v1/showcase/system/database-topology/brief`, all backed by the same role-health, migration, and read-model-sync truth instead of sample-only hidden state
