...
- the shipped `RestApi:Overrides` baseline is also shorthand-only by design and currently supports only `ApiVersionMajor`: it can retarget `MapProfile<TBehavior>()` and `MapGeneratedProfiles(...)` candidates to another effective API major version, records the applied rule through `RestEndpointCandidateRuntimeDescriptor.AppliedOverrideId`, keeps the `/api/v{major}` route segment and OpenAPI document name aligned, and still does not override explicit module DSL, manual module-owned REST endpoints, or shorthand groups that already declare `.ApiVersion(...)` explicitly
...
- when REST governance retargets a shorthand candidate to another API major version, runtime truth should point at the governing rule id rather than hiding the decision inside the final endpoint shape; `RestEndpointCandidateRuntimeDescriptor.AppliedOverrideId` is now the canonical trace for that override
