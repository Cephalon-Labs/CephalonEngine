...
- when REST governance rewrites shorthand `RequiredCapabilityKey`, the same effective value should drive `ProjectedEndpoint.RequiredCapabilityKey`, actual ASP.NET Core endpoint metadata, `/engine/rest-endpoints`, and `snapshot.RestEndpoints`; `RequireCapability(...)` now treats the last declared capability boundary as authoritative so a later host override can supersede an earlier shorthand `configureEndpoint` guard instead of stacking both boundaries
