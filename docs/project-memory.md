...
- the showcase database-topology operator projection now layers six complementary sample-level answers on top of the raw engine routes: a readiness summary, an ordered operator action plan, derived operator insights, an ordered migration playbook, a Markdown operator brief export at `/api/v1/showcase/system/database-topology/brief`, and a downloadable self-describing handoff package at `/api/v1/showcase/system/database-topology/handoff` that now includes a package `README.md`, a machine-readable `handoff-manifest.json`, the Markdown brief, and the raw projection payload, all backed by the same role-health, migration, read-model-sync, typed migration role-runtime fields, and engine-published migration-order truth instead of sample-only hidden state
...
- the engine-owned database runtime contract now also includes `RoleProbeFreshnessSeconds`; `Cephalon.Data.EntityFramework` uses the merged shared-plus-role runtime selection to cache probe results per logical role, treats `0` as the explicit no-cache answer, defaults to a 30-second freshness window when none is configured, invalidates cached probes when migration runtime state changes, and keeps `probeSource`, `probeFreshnessOrigin`, `probeFreshUntilUtc`, and `probeAgeSeconds` visible through `/engine/database-roles`, `snapshot.DatabaseRoles`, and the showcase database-topology projection
