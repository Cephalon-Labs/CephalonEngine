...
- the next endpoint-strategy follow-through after the shipped suppression-only governance baseline is a richer config-override model only if runtime truth remains explicit and introspectable; any future route- or binding-rewrite model should extend the same candidate and suppression catalogs instead of bypassing them, and any future additional shorthand sources should land only when that runtime truth stays visible
