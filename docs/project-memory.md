...
- Phase 12 is planned around migration and advanced coordination, including strangler fig, saga choreography, BFF, feature flags, and durable execution foundations; the first descriptor-only taxonomy slice is now shipped through `strangler-fig` and `backend-for-frontend`, while router and client-binding follow-through remain later work
