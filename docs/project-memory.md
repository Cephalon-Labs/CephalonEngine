...
- the showcase database-topology operator projection now layers four complementary sample-level answers on top of the raw engine routes: a readiness summary, an ordered operator action plan, derived operator insights, and an ordered migration playbook, all backed by the same role-health, migration, and read-model-sync truth instead of sample-only hidden state
