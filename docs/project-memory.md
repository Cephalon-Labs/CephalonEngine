...
- the next endpoint-strategy follow-through after shipped generated module shorthand is a richer endpoint governance/config-override model only if runtime truth remains explicit and introspectable; any future additional shorthand sources should land only when the candidate catalog can keep precedence and suppression truth visible
