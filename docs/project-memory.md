...
- the shipped profile-consumption shorthand keeps runtime publication on the same `sourceKind = module-dsl` path, while `/engine/rest-endpoints` now distinguishes shorthand-authored endpoints through `metadata.authoringStyle = behavior-module-profile` and exposes explicit profile-driven binding plans through additive `metadata.bindingDescriptors`
