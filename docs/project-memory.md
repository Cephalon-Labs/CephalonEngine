...
- the current database-topology baseline now has three complementary operator answers: `/engine/databases` is the raw engine-owned topology projection, `/engine/database-roles` plus `snapshot.DatabaseRoles` are the resolved runtime catalog for requested versus resolved roles, `UseRole`, co-location, consumers, and audit-history metadata, and `/engine/database-migrations` plus `snapshot.DatabaseMigrations` are the resolved runtime catalog for logical migration targets and their current status
