...
- the shipped low-code inline module-owned REST helpers are `engine.AddRestBehaviorModule<TMarker>(...)` and `engine.AddGeneratedRestBehaviorModule<TMarker>(...)`: they materialize real modules without dedicated module classes, reuse the same normalized projection/materialization/candidate/publication-group/governance pipeline as `RestBehaviorModuleBase`, use `TMarker` as the source-assembly marker for generated profiles and as each helper's distinct module-type identity, expect one stable marker type per inline module, let the common generated inline path derive its behavior-id prefix directly from `ModuleDescriptor.Id` while reusing the same fail-fast deterministic route-group validation as `GroupFromBehaviorIdPrefix(...)`, keep the explicit `behaviorIdPrefix` overload available when inline module identity and generated ownership should differ, and still never publish public REST from `[AppBehavior]` alone
