...
- `Cephalon.Behaviors.Http` now also publishes stable startup diagnostics for shorthand REST governance through `/engine/diagnostics` source `Cephalon.Behaviors.Http` with event ids `5200-5205` for governance suppression, authoring-policy suppression, precedence suppression, applied overrides, no-op override matches, and preserved binding fallback; when the runtime candidate registry is active, published-endpoint logging should reconcile against post-materialization truth so no-op published endpoints do not claim they changed the runtime answer, event `5204` now follows whichever typed `BindingFallbackMode` was actually preserved, including both `PreserveSourceImplicitFallback` and `PreserveRemainingBodyFallback`, and event `5205` now records authoring-policy suppression through the stable kinds `disallowed-authoring-style`, `not-allowed-authoring-style`, and `preferred-authoring-style-selected`
