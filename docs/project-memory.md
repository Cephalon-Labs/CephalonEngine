...
- phase 11 resilience work now has a contract-first shipped baseline: `Engine:Resilience`, `AppProfile.Resilience`, and `/engine/resilience` are the public source of truth for requested `Retry`, `Timeout`, `CircuitBreaker`, `Bulkhead`, and `RateLimiting` policy intent, while actual behavior-pipeline and transport enforcement remains a later slice on top of that stable contract
