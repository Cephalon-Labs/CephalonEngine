...
- phase 11 resilience work now has a contract-first baseline plus two shipped ASP.NET Core follow-through slices: `Engine:Resilience`, `AppProfile.Resilience`, and `/engine/resilience` remain the public source of truth for requested `Retry`, `Timeout`, `CircuitBreaker`, `Bulkhead`, and `RateLimiting` intent, while ASP.NET Core now projects effective public-HTTP enforcement through `IRateLimitingRuntimeCatalog`, `/engine/rate-limiting`, and `snapshot.RateLimitingPolicies` with operator/docs route exclusions and endpoint-scoped override metadata derived from `Engine:Resilience:RateLimiting:Overrides`; broader behavior-pipeline resilience and non-HTTP transport-native semantics remain later slices
