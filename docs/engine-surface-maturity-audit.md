...
| `Cephalon.Eventing` remediation command-result catalog | Abstraction-level remediation command-result read model, duplicate command-id rejection, `IEventDispatchRemediationRuntimeCatalog`, `EventDispatchRemediationRuntimeState`, `EventDispatchRemediationRuntimeSummary`, `EventDispatchRemediationRuntimeRetention`, `event-dispatch-remediation-commands` catalog and command-result entries, and `/engine/event-dispatch-remediation-commands*` summary/latest/retention/observed-window plus observed-window-summary plus metadata-advertised list/result/outbox route discovery, inclusive observed-UTC window policy, and newest-first `limit` on list/filter drill-downs for accepted/rejected command results by command id, outbox id, message id, channel id, operation id, actor id, correlation id, reason, dispatch outcome, and command outcome | `cephalon-managed` bounded process-local operator/audit read model over the active remediation dispatcher | `M3` | Keep command-result history bounded, truthful, duplicate-safe, latest-readable, summary-readable with dropped-command and incomplete-warning evidence, retention/truncation-readable, observation-window-queryable, observation-window-summary-readable, catalog-discoverable before history exists, list/result/outbox-route-discoverable, message/channel/operation/actor/correlation/reason/dispatch-outcome/command-outcome-queryable with introspectable read-side limits and window policy, and separate from durable compliance storage or broker dead-letter/replay ownership |
