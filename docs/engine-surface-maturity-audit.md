...
| `Cephalon.Eventing` remediation command-result catalog | Abstraction-level remediation command-result read model, duplicate command-id rejection, `IEventDispatchRemediationRuntimeCatalog`, `EventDispatchRemediationRuntimeState`, `event-dispatch-remediation-commands`, and `/engine/event-dispatch-remediation-commands*` drill-downs for accepted/rejected command results | `cephalon-managed` bounded process-local operator/audit read model over the active remediation dispatcher | `M3` | Keep command-result history bounded, truthful, duplicate-safe, and separate from durable compliance storage or broker dead-letter/replay ownership |
