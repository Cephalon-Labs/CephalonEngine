...
| `Cephalon.MultiTenancy.Governance` | Tenant membership, invitation, delivery dispatch/retry/status reconciliation, delivery-status observation storage, tenant-administration, domain-ownership verification/proof collection/polling, governance action workflows, optional ASP.NET Core governance endpoints, optional HTTP/SMTP/SendGrid senders, and optional ASP.NET Core SendGrid callback translation plus signed-webhook verification, process-local replay protection, and observation-store-backed event-id idempotency | mixed: `cephalon-managed` governance core, ASP.NET Core adapter endpoints, provider-neutral callback signatures, bounded process-local replay protection, local durable stores, HTTP/SMTP/SendGrid sender companions, and SendGrid callback translation/signature verification/process-local replay/event-id-idempotency protection; external provider delivery/status remains `provider-managed` | `M2` | Actual DNS proof publication, provider-backed proof publication or mutation, remediation execution beyond state transitions, distributed or provider-backed membership/invitation/domain/action-store backends, Mailgun/SES/Microsoft Graph or other non-SendGrid provider-specific email API senders, SMS/chat/CRM/identity-provider invitation senders, distributed retry queues, cross-node retry leases, provider-specific or distributed callback inboxes, cross-node callback replay protection, distributed event-id ledgers, non-SendGrid provider-specific callback payload translation/signature verification, provider polling, identity-provider synchronization, public onboarding, and tenant-admin UI/backoffice flows only when the package truly owns those paths |
...
- keep `Cephalon.MultiTenancy` intentionally thin in the base package; the governance family now owns the concrete governance workflows plus optional ASP.NET Core routes, HTTP/SMTP/SendGrid sender companions, provider-neutral callback signatures/replay protection, and optional SendGrid callback translation plus signed-webhook verification, bounded process-local signed-callback replay protection, and observation-store-backed event-id idempotency. Actual DNS proof publication, provider-backed proof publication or mutation, remediation execution beyond state transitions, distributed or provider-backed membership/invitation/domain/action stores, Mailgun/SES/Microsoft Graph or other non-SendGrid provider-specific email API senders, SMS/chat/CRM/identity-provider senders, distributed retry queues, cross-node retry leases, provider-specific or distributed callback inboxes, cross-node callback replay protection, distributed event-id ledgers, non-SendGrid provider-specific callback payload translation/signature verification, provider polling, identity-provider synchronization, public onboarding, and tenant-admin UI/backoffice flows remain explicitly outside the current claim
...
- actual DNS proof publication, provider-backed proof publication or mutation, remediation execution beyond state transitions, distributed or provider-backed membership/invitation/domain/action-store backends, Mailgun/SES/Microsoft Graph or other non-SendGrid provider-specific email API senders, SMS/chat/CRM/identity-provider invitation senders, distributed retry queues, cross-node retry leases, provider-specific or distributed callback inboxes, cross-node callback replay protection, non-SendGrid provider-specific callback payload translation and signature verification, provider polling, identity-provider synchronization, public onboarding, and tenant-admin UI/backoffice flows when `Cephalon.MultiTenancy.Governance` or provider packs truly own those paths
