...
| `Cephalon.Eventing` event context policy catalog | Code-first `EventContextPolicyDescriptor` registrations through `EventingOptions.ContextPolicies` and `IEventContextPolicyContributor`, `IEventContextPolicyCatalog` lookup by policy id/header name, `event-context-policies` technology surface, `eventing.context-policies` capability, and `tenant-and-correlation-context-ownership` policy-declared partial evidence | `cephalon-managed` descriptor catalog over host/module-authored context policy metadata | `M2` | Keep tenant/correlation/causation/baggage/header policy metadata code-owned and Wolverine-free, keep duplicate policy id overrides deterministic, expose header and context-policy coverage to operators, and do not claim executable context propagation or validation until a provider or engine package owns those paths |
