...
- treat `Engine:Messaging:Channels` as configuration-owned channel descriptor discovery, while `Engine:Messaging:Subscriptions` and `Engine:Messaging:SubscriptionHandlers` stay unsupported for runtime behavior; subscriptions and executors are code-owned for performance and type safety
