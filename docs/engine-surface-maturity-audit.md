...
| `Cephalon.Eventing` core package | Config-driven channel/subscription descriptors, config-driven subscription handler binding, staged publication, subscription runtime truth, public managed-execution binding catalog, abstraction-level subscription execution-readiness catalog, abstraction-level publication dispatcher, abstraction-level publication runtime-state catalog, abstraction-level dispatch-state/summary terminal-failure posture, abstraction-level dispatch-remediation dispatcher, `event-dispatch-remediations` command-readiness posture, `/engine/event-subscription-readiness`, `POST /engine/event-publications`, `/engine/event-publications/runtime*`, `/engine/event-dispatches/terminal-failures`, `POST /engine/event-dispatches/{outboxId}/commands/{operationId}`, `snapshot.EventSubscriptionExecutionReadiness`, `snapshot.EventPublicationStates`, `snapshot.EventDispatchStates`, stable subscription runtime metadata vocabulary, opt-in direct in-process subscription execution, bounded process-local retry, bounded process-local duplicate-completed execution suppression, config-driven event-type-to-channel publication routing, and bounded process-local scheduled publication acceptance | mixed: `application-managed` descriptors/runtime reports plus `cephalon-managed` direct in-process execution, configuration-owned descriptor overrides and handler bindings, bounded process-local retry, bounded process-local idempotency, config-driven publication routing, bounded process-local scheduled publication acceptance, bounded publication action truth, publication runtime-state truth for direct execution, scheduled delayed handoff, routed/validated channel selection, or accepted outbox handoff, typed dispatch terminal-failure operator truth over reported dispatch state, and provider-neutral `retry-now`/`retry-later`/`skip`/`quarantine` remediation commands over supported dispatch stores | `M3` | Keep the in-process lane, handler bridge, retry loop, idempotency guard, route table, process-local scheduler, operator publication route, publication runtime-state catalog, dispatch terminal-failure posture, and remediation commands bounded to reported state plus mutable dispatch-store ownership; only claim arbitrary handler scanning, generated handler pipelines, generic broker/inbox/durable retry/downstream delivery ownership, broker topology materialization, provider partition ownership, durable/distributed scheduling, or broker dead-letter/replay ownership when a package truly owns those paths |
| `Cephalon.Eventing` remediation command-result catalog | Abstraction-level remediation command-result read model, `IEventDispatchRemediationRuntimeCatalog`, `EventDispatchRemediationRuntimeState`, `event-dispatch-remediation-commands`, and `/engine/event-dispatch-remediation-commands*` drill-downs for accepted/rejected command results | `cephalon-managed` bounded process-local operator/audit read model over the active remediation dispatcher | `M3` | Keep command-result history bounded, truthful, and separate from durable compliance storage or broker dead-letter/replay ownership |
