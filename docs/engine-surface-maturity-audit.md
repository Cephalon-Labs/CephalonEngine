...
| `Cephalon.Eventing` core package | Channel descriptors, staged publication, subscription runtime truth, public managed-execution binding catalog, abstraction-level subscription execution-readiness catalog, abstraction-level publication dispatcher, `/engine/event-subscription-readiness`, `POST /engine/event-publications`, `snapshot.EventSubscriptionExecutionReadiness`, stable subscription runtime metadata vocabulary, opt-in direct in-process subscription execution, and bounded process-local retry | mixed: `application-managed` descriptors/runtime reports plus `cephalon-managed` direct in-process execution, bounded process-local retry, and bounded publication action truth | `M3` | Keep the in-process lane, retry loop, and operator publication route bounded, non-durable, and non-brokered; only claim generic broker/inbox/durable retry ownership when a package truly owns those paths |
...
- treat the `Cephalon.Eventing` in-process direct subscription lane plus bounded process-local retry and `POST /engine/event-publications`, `Cephalon.Eventing.Wolverine`, the `Cephalon.Agentics` dispatcher/run-state lane plus direct operator read/action seams, and the `Cephalon.Retrieval` lexical index/query/freshness lane plus direct operator read, manual reindex, and opt-in background reindex seams as current managed vertical proofs instead of widening descriptor breadth before ownership is real
