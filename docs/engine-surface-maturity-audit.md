...
| `Cephalon.Retrieval` | Knowledge collection descriptors plus managed lexical indexing, query execution, freshness state, abstraction-level index-state catalog, `/engine/knowledge-indexes`, and `snapshot.KnowledgeIndexes` | mixed: `application-managed` source documents plus `cephalon-managed` index/query/operator-read baseline | `M2` | Provider-specific vector/search engines, durable or distributed indexes, reindex automation, and operator remediation only after a package truly owns those paths |
...
- treat `Cephalon.Eventing.Wolverine`, the `Cephalon.Agentics` dispatcher/run-state lane plus direct operator read seam, and the `Cephalon.Retrieval` lexical index/query/freshness lane plus direct operator read seam as current managed vertical proofs instead of widening descriptor breadth before ownership is real
