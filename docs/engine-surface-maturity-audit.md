...
- treat the ASP.NET Core invitation delivery dispatch endpoint as a bounded `M2` action seam over the host-agnostic dispatcher, and treat the delivery-status observation read endpoint as a bounded `M2` operator/audit projection over normalized store records; do not promote either endpoint into provider-specific sender ownership, durable retry queues, callback-inbox, provider-polling, distributed-replay, or exactly-once language until a package owns those paths
