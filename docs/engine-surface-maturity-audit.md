...
| `Cephalon.Data` shared CDC runtime plus provider-native pumps | Shared and provider-native data execution truth, including data-product introspection through `/engine/data-products` and `/engine/data-products/runtime` | `cephalon-managed` plus `provider-managed` | `M3` | More package-level external adoption proof and operator docs per provider family |
