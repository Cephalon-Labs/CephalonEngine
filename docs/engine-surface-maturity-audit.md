...
| `Cephalon.Eventing` core package | Channel descriptors, staged publication, subscription runtime truth, public managed-execution binding catalog, abstraction-level subscription execution-readiness catalog, `/engine/event-subscription-readiness`, `snapshot.EventSubscriptionExecutionReadiness`, stable subscription runtime metadata vocabulary, and opt-in direct in-process subscription execution | mixed: `application-managed` descriptors/runtime reports plus `cephalon-managed` direct in-process execution truth | `M2` | Keep the in-process lane non-durable and non-brokered; only claim generic broker/inbox/retry ownership when a package truly owns those paths |
...
- treat the `Cephalon.Eventing` in-process direct subscription lane, `Cephalon.Eventing.Wolverine`, the `Cephalon.Agentics` dispatcher/run-state lane plus direct operator read/action seams, and the `Cephalon.Retrieval` lexical index/query/freshness lane plus direct operator read, manual reindex, and opt-in background reindex seams as current managed vertical proofs instead of widening descriptor breadth before ownership is real
