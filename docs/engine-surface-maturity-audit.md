...
- treat `Cephalon.Eventing.Wolverine`, the `Cephalon.Agentics` dispatcher/run-state lane plus direct operator read/action seams, and the `Cephalon.Retrieval` lexical index/query/freshness lane plus direct operator read, manual reindex, and opt-in background reindex seams as current managed vertical proofs instead of widening descriptor breadth before ownership is real
