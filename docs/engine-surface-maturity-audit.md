...
- treat the `Cephalon.Eventing` in-process direct subscription lane plus bounded process-local retry, bounded process-local idempotency, `POST /engine/event-publications`, and `/engine/event-publications/runtime*`, the optional `Cephalon.Eventing.Wolverine` provider-managed dispatch/subscription lane plus bounded terminal dispatch and subscription retry proof, the `Cephalon.Agentics` dispatcher/run-state lane plus direct operator read/action seams, and the `Cephalon.Retrieval` lexical index/query/freshness lane plus direct operator read, manual reindex, and opt-in background reindex seams as current managed vertical proofs instead of widening descriptor breadth before ownership is real
