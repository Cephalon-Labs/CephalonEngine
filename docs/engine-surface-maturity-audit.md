...
| --- | --- | --- | --- | --- |
| `Cephalon.Engine` app model, manifest, runtime introspection, policy composition | Core runtime contract and composition, including transport introspection through `/engine/transports` and `/engine/transports/runtime`, localization introspection through `/engine/localization` and `/engine/localization/runtime`, and rate-limiting introspection through `/engine/rate-limiting` and `/engine/rate-limiting/runtime` | `cephalon-managed` | `M4` | Keep compatibility, docs, and generated surfaces aligned as new packs land |
...
| `Cephalon.Data` shared CDC runtime plus provider-native pumps | Shared and provider-native data execution truth, including data-product introspection through `/engine/data-products` and `/engine/data-products/runtime`, plus shared substrate introspection through `/engine/outboxes`, `/engine/outboxes/runtime`, `/engine/inboxes`, and `/engine/inboxes/runtime` | `cephalon-managed` plus `provider-managed` | `M3` | More package-level external adoption proof and operator docs per provider family |
...
| `Cephalon.Observability` core plus exporter and dependency-health companion family | Shared observability options, startup summary projection, telemetry-export configuration intent, optional OpenTelemetry/Serilog/Kubernetes/AWS/GCP/Azure Monitor/Alibaba Cloud/Huawei Cloud/Oracle Cloud/DigitalOcean/OpenShift/Tanzu/Grafana Cloud/New Relic configuration packs, optional dependency-health probe core plus per-provider probe packs (Cassandra, ClickHouse, Consul, Elasticsearch, Http, Kafka, Memcached, MongoDB, MQTT, MySQL, NATS, Neo4j, OpenSearch, Oracle, PostgreSQL, RabbitMQ, Redis, SqlServer) with probe freshness, duration, and failure-streak tracking | `cephalon-managed` for the observability core, configuration packs, and dependency-health probe baselines; `provider-managed` for per-provider dependency-health probe runtime execution | `Cephalon.Observability` core at `M2`; cloud configuration packs at `M1`; `Cephalon.Observability.DependencyHealth.Core` at `M3` with operator metadata (CheckedAtUtc, ProbeDurationMilliseconds, ConsecutiveFailureCount); per-provider dependency-health probe packs at `M3` | Probe runtime owns operator automation loops through metadata fields; telemetry collection remains host-owned via OpenTelemetry stack; config packs promote when they own runtime collection or managed export loops |
