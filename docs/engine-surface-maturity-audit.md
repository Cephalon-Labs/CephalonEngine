...
| `Cephalon.Data` non-relational provider packs (`Cephalon.Data.Redis`, `Cephalon.Data.Neo4j`, `Cephalon.Data.Cassandra`, `Cephalon.Data.ClickHouse`, `Cephalon.Data.Elasticsearch`, `Cephalon.Data.OpenSearch`, `Cephalon.Data.Qdrant`, `Cephalon.Data.Nats`, `Cephalon.Data.Debezium`) | Provider-specific store descriptors and persistence surfaces over Redis, Neo4j, Cassandra, ClickHouse, Elasticsearch, OpenSearch, Qdrant, and NATS now project provider outbox/inbox truth through `IOutboxCatalog`, `IInboxCatalog`, and the `outbox-producers` / `inbox-stores` runtime technology surfaces; dispatch-store bridges are shipped where the provider exposes `IEventDispatchStore`. URI-based providers redact inline URI user-info and query credentials in capability metadata. `Cephalon.Data.Debezium` contributes external managed-connector CDC descriptors and execution-runtime truth through `ICdcCaptureCatalog` and `ICdcCaptureExecutionRuntimeCatalog` while keeping connector ownership outside the engine | `provider-managed` | `M1` family-level | Per-pack managed-execution proof (read/write/projection lifecycle and provider-owned provisioning for store packs; managed connector orchestration for Debezium) before promoting any individual pack from `M1` provider-visible runtime truth to `M2` managed execution. `Cephalon.Data.MongoDB` remains covered by its separate `M2` provider row because it owns the provider-native change-stream runner in addition to the shared outbox/inbox surface; relational providers (`SqlServer`, `Postgres`, `MySql`, `Oracle`) remain covered by the existing `Cephalon.Data` shared CDC runtime row |
