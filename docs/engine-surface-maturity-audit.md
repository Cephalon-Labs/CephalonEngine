...
Surface maturity in this document reflects the repository state as of `September 16, 2026` (after the Sprint 125 Eventing operational-superiority evidence stream, scorecard schema `1.25.0`, the generated-app adoption report readback, the adoption golden-use-case readback, the component maturity-badge completion guard, the Entity Framework, MongoDB, Redis, and NATS EventSourcing durable-snapshot proofs, the Gate 0 machine-readable maturity reconciliation, and the September planning refresh; existing package maturity declarations are unchanged).
...
The [completion plan](framework-completion-plan.md#m0m4-acceptance-and-maintenance) adds role-appropriate promotion, evidence renewal and expiry rules as of September 16, 2026. It preserves the current 107-package distribution and does not promote a package by planning alone.
...
ENG-605 adds `subscription-concurrency-ownership`, keeping per-subscription concurrency limits, parallel handler execution, consumer prefetch, backpressure, provider concurrency, consumer leases, work stealing, and distributed work sharing outside declared subscriptions, direct in-process execution, code-first middleware, hosted binding metadata, and optional provider binding evidence until a provider/runtime supplies complete live proof. ENG-627 adds `EventSubscriptionConcurrencyMetadata`, so the dimension becomes `claimed` only when successful subscription reports carry the complete concurrency proof set.
ENG-606 adds `subscription-ordering-ownership`, keeping handler ordering, local fan-out ordering, per-key ordering, partition ordering, causal ordering, replay ordering, cross-node ordering, and provider ordering outside declared subscriptions, direct in-process execution, code-first middleware, hosted binding metadata, and optional provider binding evidence until a provider/runtime supplies complete live proof. ENG-628 adds `EventSubscriptionOrderingMetadata`, so the dimension becomes `claimed` only when successful subscription reports carry the complete ordering proof set.
ENG-607 adds `process-manager-state-ownership`, keeping durable process-manager state, saga persistence, saga correlation, timeout scheduling, compensation workflow, process-manager concurrency, and recovery outside declared subscriptions, direct in-process execution, code-first middleware, choreography bridge handoff, outbox publication, hosted binding metadata, and optional provider binding evidence until a provider/runtime supplies complete live proof. ENG-629 adds `EventSubscriptionProcessManagerStateMetadata`, so the dimension becomes `claimed` only when successful subscription reports carry the complete process-manager proof set.
...
ENG-630 adds `EventDispatchBrokerTopologyMetadata`, so `broker-topology-materialization-ownership` becomes `claimed` only when successful dispatch reports carry complete exchange provisioning, queue provisioning, topic provisioning, partition provisioning, topology verification, and provider topology proof. Routing, accepted outbox handoff, dispatch reports without topology proof, provider partition evidence, and optional Wolverine dispatch stay outside that claim.

ENG-631 adds `EventDispatchProviderPartitionMetadata`, so `provider-partition-ownership` becomes `claimed` only when successful dispatch reports carry complete partition assignment, partition affinity, partition rebalancing, partition ordering guarantee, and provider partitioning proof. Routing, accepted outbox handoff, broker topology proof, dispatch reports without partition proof, and optional Wolverine dispatch stay outside that claim.
...
| `Cephalon.Eventing` event context policy catalog and publisher enforcement | Code-first `EventContextPolicyDescriptor` registrations through `EventingOptions.ContextPolicies` and `IEventContextPolicyContributor`, `IEventContextPolicyCatalog` lookup by policy id/header name, stable `EventContextHeaderNames`, publisher-enforced required-header validation when `ValidatesMessageHeaders = true`, staged `EventContextHandoffMetadataKeys` outbox proof, dispatch-store-read `EventDispatchItem` header/metadata proof, dispatch-report metadata proof through `EventDispatchContextReportMetadata`, `event-context-policies` technology surface, `eventing.context-policies` capability, and `tenant-and-correlation-context-ownership` partial evidence | `cephalon-managed` descriptor catalog and native publisher enforcement over host/module-authored context policy metadata | `M2` | Keep tenant/correlation/causation/baggage/header policy metadata code-owned and Wolverine-free, keep duplicate policy id overrides deterministic, expose header coverage, validation failure, direct in-process forwarding, outbox-stage, dispatch-store-read, and dispatch-report metadata evidence to operators, and do not claim provider/broker headers, consumer extraction, delivery completion, or cross-node context handoff until a provider or engine package owns those paths |
| `Cephalon.Agentics` | Tool descriptors, managed tool dispatch, abstraction-level run-state catalog, bounded operator action route, bounded process-local executor retry posture, bounded process-local duplicate-completed suppression, opt-in active `IInbox`-backed completed-run markers for `toolId + runId`, approval-required and terminal-failure operator filters, `/engine/agent-tool-runs`, `/engine/agent-tool-runs/retry-pending`, `/engine/agent-tool-runs/idempotency-duplicates`, `/engine/agent-tool-runs/approval-required`, `/engine/agent-tool-runs/terminal-failures`, `POST /engine/agent-tools/{toolId}/runs`, `snapshot.AgentToolRuns`, and agent-workload runtime surface | mixed: `application-managed` descriptors plus `cephalon-managed` dispatcher/run-state/operator-action/bounded-process-local-retry/process-local-or-inbox-backed-idempotency/read-filter baseline over an explicitly registered `IInbox` provider when inbox durability is selected | `M3` | Broader agent operator workflows, durable approval workflows, durable retry queues, provider-owned durable inbox command processing beyond the completed-run marker, memory persistence, dead-letter systems, cross-node exactly-once delivery, distributed scheduling, and provider-specific AI orchestration only after a package truly owns those paths |
...
| `Cephalon.Analyzers` | Curated consumer-facing analyzer meta-package bundling `Microsoft.CodeAnalysis.BannedApiAnalyzers`, `Microsoft.CodeAnalysis.PublicApiAnalyzers`, `Roslynator.Analyzers`, `Meziantou.Analyzer`, and `Microsoft.VisualStudio.Threading.Analyzers`; ships curated `BannedSymbols.txt` (auto-wired through `buildTransitive/Cephalon.Analyzers.props`) and curated `cephalon-analyzers.editorconfig`; `Cephalon.Abstractions` enforces its API baseline with a direct PublicApiAnalyzers reference; meta-package adoption proof belongs to NuGet consumers, `cephalon new` emits it into generated non-test projects as a private analyzer package, and every app/module template-pack starter references it by default | `cephalon-managed` | `M2` | Promote beyond `M2` only when analyzer enforcement is wired into release scorecard or operator adoption gates with stable remediation guidance instead of relying on package-reference presence alone |
...
| `Cephalon.EventSourcing` core | Event-sourced aggregate contracts, stable event-type registry, merged `IEventStoreCatalog`, on-demand `IEventStreamReplayWorker`, snapshot lifecycle over `ISnapshotStore` with process-local fallback or provider-durable store detection, registered `IProjection<IDomainEvent>` rebuild during replay, and `event-sourcing-managed-replay-worker` runtime-surface evidence with latest replay status/counts plus `providerDurableSnapshotProviders` readback | mixed: `application-managed` aggregate logic plus `cephalon-managed` on-demand replay/snapshot/projection lifecycle | `M2` | Keep this at `M2` until Cephalon owns durable or distributed replay execution, named projection orchestration, retention/archival, and hosted background replay/projection runners. The core worker can consume provider-durable snapshots when a provider supplies `ISnapshotStore`, but provider persistence remains a provider-specific claim |
| `Cephalon.EventSourcing.EntityFramework` | Entity Framework append/read event store, stable event-type registry persistence, `CephalonEvents` mapping, provider-durable `CephalonEventSnapshots` mapping and `ISnapshotStore` implementation, sanitized `IEventStoreContributor` descriptor metadata, and `event-sourcing` runtime-surface entries that report `snapshotLifecycle = provider-durable` / `snapshotStorage = CephalonEventSnapshots` | `provider-managed` append/read store plus durable latest-snapshot persistence | `M2` | Keep it at `M2` until it owns projection/rebuild orchestration, retention, archival, distributed replay, or hosted replay/projection runners beyond the shared core worker |
| `Cephalon.EventSourcing.MongoDB` | MongoDB append/read event store, stable event-type registry persistence, compound stream-version index, provider-durable MongoDB latest-snapshot collection, `ISnapshotStore` implementation, sanitized `IEventStoreContributor` descriptor metadata, and `event-sourcing` runtime-surface entries that report `snapshotLifecycle = provider-durable` / `snapshotStorage = {collectionName}_snapshots` | `provider-managed` append/read store plus durable latest-snapshot persistence | `M2` | Keep it at `M2` until it owns projection/rebuild orchestration, retention, archival, distributed replay, change-stream subscription orchestration, or hosted replay/projection runners beyond the shared core worker |
| `Cephalon.EventSourcing.Redis` | Redis Streams append/read event store, stable event-type registry persistence, provider-durable Redis Hash latest-snapshot store, `ISnapshotStore` implementation, sanitized `IEventStoreContributor` descriptor metadata, and `event-sourcing` runtime-surface entries that report `snapshotLifecycle = provider-durable` / `snapshotStorage = Redis Hash latest-snapshot` | `provider-managed` append/read store plus durable latest-snapshot persistence | `M2` | Keep it at `M2` until it owns projection/rebuild orchestration, retention, archival, distributed replay, Redis Stream consumer-group subscription orchestration, atomic stream-append hardening, or hosted replay/projection runners beyond the shared core worker |
| `Cephalon.EventSourcing.Nats` | NATS JetStream KV append/read event store, stable event-type registry persistence, provider-durable NATS JetStream KV latest-snapshot store, `ISnapshotStore` implementation, sanitized `IEventStoreContributor` descriptor metadata, and `event-sourcing` runtime-surface entries that report `snapshotLifecycle = provider-durable` / `snapshotStorage = NATS JetStream KV latest-snapshot` / `snapshotConcurrency = revision-compare-and-set` | `provider-managed` append/read store plus durable latest-snapshot persistence | `M2` | Keep it at `M2` until it owns projection/rebuild orchestration, retention, archival, distributed replay, NATS consumer-group subscription orchestration, JetStream stream publishing beyond KV, or hosted replay/projection runners beyond the shared core worker |
| Remaining EventSourcing provider packs (`Cephalon.EventSourcing.Neo4j`, `Cephalon.EventSourcing.Cassandra`, `Cephalon.EventSourcing.ClickHouse`, `Cephalon.EventSourcing.Elasticsearch`, `Cephalon.EventSourcing.OpenSearch`, `Cephalon.EventSourcing.Qdrant`) | Provider-specific append/read event stores, sanitized `IEventStoreContributor` descriptors, and `event-sourcing` runtime-surface entries for the six remaining provider stores | `provider-managed` per-pack append/read store contracts and catalog/runtime-surface contributions | `M1` family-level | Keep these provider packs at `M1` until an individual provider owns managed execution beyond append/read, such as durable snapshot persistence, projection/rebuild lifecycle, retention, archival, or hosted replay workers. Secret redaction for connection strings, passwords, URI user-info, and provider credential values remains required runtime truth |
