...
| `Cephalon.Eventing` core package | Channel descriptors, staged publication, subscription runtime truth, public managed-execution binding catalog, abstraction-level subscription execution-readiness catalog, `/engine/event-subscription-readiness`, `snapshot.EventSubscriptionExecutionReadiness`, and stable subscription runtime metadata vocabulary | mixed: `application-managed` baseline plus companion-bound execution truth | `M1` | Keep the adapter-neutral execution and readiness seams narrow and truthful without claiming generic broker/inbox ownership in the core pack |
...
| `Cephalon.Retrieval` | Knowledge collection descriptors plus managed lexical indexing, query execution, freshness state, abstraction-level index-state catalog, manual reindex command seam, `/engine/knowledge-indexes`, `POST /engine/knowledge-indexes/{collectionId}/reindex`, and `snapshot.KnowledgeIndexes` | mixed: `application-managed` source documents plus `cephalon-managed` index/query/operator-read/manual-remediation baseline | `M3` | Provider-specific vector/search engines, durable or distributed indexes, and background reindex scheduling or automation only after a package truly owns those paths |
...
- treat `Cephalon.Eventing.Wolverine`, the `Cephalon.Agentics` dispatcher/run-state lane plus direct operator read/action seams, and the `Cephalon.Retrieval` lexical index/query/freshness lane plus direct operator read and manual reindex seams as current managed vertical proofs instead of widening descriptor breadth before ownership is real
