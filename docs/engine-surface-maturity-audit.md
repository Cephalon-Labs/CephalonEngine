...
| `Cephalon.EventSourcing.EntityFramework` | Entity Framework append/read event store, stable event-type registry persistence, `CephalonEvents` mapping, provider-durable `CephalonEventSnapshots` mapping and `ISnapshotStore` implementation, sanitized `IEventStoreContributor` descriptor metadata, and `event-sourcing` runtime-surface entries that report `snapshotLifecycle = provider-durable` / `snapshotStorage = CephalonEventSnapshots` | `provider-managed` append/read store plus durable latest-snapshot persistence | `M2` | This is the first EventSourcing provider promoted above the family floor. Keep it at `M2` until it owns projection/rebuild orchestration, retention, archival, distributed replay, or hosted replay/projection runners beyond the shared core worker |
| Remaining EventSourcing provider packs (`Cephalon.EventSourcing.MongoDB`, `Cephalon.EventSourcing.Redis`, `Cephalon.EventSourcing.Neo4j`, `Cephalon.EventSourcing.Cassandra`, `Cephalon.EventSourcing.ClickHouse`, `Cephalon.EventSourcing.Elasticsearch`, `Cephalon.EventSourcing.OpenSearch`, `Cephalon.EventSourcing.Qdrant`, `Cephalon.EventSourcing.Nats`) | Provider-specific append/read event stores, sanitized `IEventStoreContributor` descriptors, and `event-sourcing` runtime-surface entries for the nine remaining provider stores. Redis has opt-in live-provider evidence in `tests/Cephalon.Tests.ProviderIntegration` for stream append/read/version/concurrency behavior against a real Redis runtime | `provider-managed` per-pack append/read store contracts and catalog/runtime-surface contributions | `M1` family-level | Keep these provider packs at `M1` until an individual provider owns managed execution beyond append/read, such as durable snapshot persistence, projection/rebuild lifecycle, retention, archival, or hosted replay workers. Secret redaction for connection strings, passwords, URI user-info, and provider credential values remains required runtime truth |
