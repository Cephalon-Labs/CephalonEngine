...
- `Cephalon.MultiTenancy.Governance.AmazonSesDelivery.AspNetCore` is the optional ASP.NET Core Amazon SES over SNS callback translation, SNS signature-verification, process-local SNS replay-protection, observation-store-backed SNS message-id idempotency, and verified SNS subscription-confirmation companion package for the multi-tenancy governance delivery-status reconciler
