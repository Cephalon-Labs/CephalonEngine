...
public static class OpenShiftHostApplicationBuilderExtensions
{
    ...
    /// </para>
    /// </remarks>
    public static TBuilder AddCephalonOpenShift<TBuilder>(
        this TBuilder builder,
        Action<OpenShiftTelemetryExportOptions>? configure = null)
        where TBuilder : IHostApplicationBuilder
    {
        ArgumentNullException.ThrowIfNull(builder);

        var telemetry = ObservabilityOptions.FromConfiguration(builder.Configuration).Telemetry;
        var options = OpenShiftTelemetryExportOptions.FromConfiguration(builder.Configuration);
        ...
        builder.Services.AddSingleton<IDiagnosticsConventionContributor, OpenShiftDiagnosticsConventionContributor>();
        builder.Services.AddHostedService<OpenShiftSummaryHostedService>();
        builder.Services.TryAddEnumerable(ServiceDescriptor.Singleton<ITechnologyRuntimeContributor, OpenShiftTelemetryRuntimeContributor>());
        ...
        var openTelemetry = builder.Services
            ...
            .ConfigureResource(resource =>
            {
                ...
                ConfigureOpenShiftResource(resource, platformAttributes);
            });
    ...
    internal static string? NormalizeHostedPlatform(string hostedPlatform)
    {
        ...
        return hostedPlatform.Trim().ToLowerInvariant() switch
        {
            ...
            "aro" => "aro",
            "azureredhatopenshift" => "aro",
            "azure-red-hat-openshift" => "aro",
            "rosa" => "rosa",
            "redhatopenshiftserviceonaws" => "rosa",
    ...
    internal static OtlpExportProtocol ResolveExporterProtocol(string protocol)
    {
        ...
        return protocol.Trim().ToLowerInvariant() switch
        {
            ...
            _ => throw new InvalidOperationException(
                $"Telemetry protocol '{protocol}' is not supported by Cephalon OpenShift observability integration. Use 'otlp', 'otlp/grpc', or 'otlp/http'.")
        };
    }
    ...
    internal static string ResolveCollectorScheme(string? scheme)
    {
        ...
        return scheme.Trim().ToLowerInvariant() switch
        {
            ...
            "https" => "https",
            _ => throw new InvalidOperationException(
                $"Cephalon OpenShift observability integration only supports collector scheme 'http' or 'https'. Configured value '{scheme}' is not supported.")
        };
    }

    internal static string ResolveCollectorServiceName(OpenShiftTelemetryExportOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);

        return string.IsNullOrWhiteSpace(options.CollectorServiceName)
            ? throw new InvalidOperationException(
                "Cephalon OpenShift observability integration requires Engine:Observability:Telemetry:OpenShift:CollectorServiceName to be configured when UseInClusterCollectorService is enabled.")
            : options.CollectorServiceName.Trim();
    }

    internal static string? ResolveWorkloadNamespace(OpenShiftTelemetryExportOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);
    ...
    internal static string ResolveCollectorNamespace(OpenShiftTelemetryExportOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);
        ...
        return Normalize(options.CollectorNamespace)
            ...
            ?? throw new InvalidOperationException(
                "Cephalon OpenShift observability integration requires Engine:Observability:Telemetry:OpenShift:CollectorNamespace, Engine:Observability:Telemetry:OpenShift:Namespace, or POD_NAMESPACE to be available when UseInClusterCollectorService is enabled.");
    }

    private static bool ShouldRegister(TelemetryExportOptions telemetry, OpenShiftEndpointMode endpointMode)
    {
        ArgumentNullException.ThrowIfNull(telemetry);

        return endpointMode != OpenShiftEndpointMode.None &&
            (telemetry.ExportLogs || telemetry.ExportMetrics || telemetry.ExportTraces);
    }

    private static OpenShiftEndpointMode ResolveEndpointMode(
        ...
        OpenShiftTelemetryExportOptions options)
    {
        ...
        {
            return OpenShiftEndpointMode.ConfiguredEndpoint;
        }

        if (telemetry.UseSelfHostedDefaults)
        {
            return OpenShiftEndpointMode.SelfHostedDefaults;
        }

        return options.UseInClusterCollectorService
            ? OpenShiftEndpointMode.InClusterCollectorService
            : OpenShiftEndpointMode.None;
    }
    ...
    private static void EnsureProviderIsSupported(string provider)
    {
        ...
        throw new InvalidOperationException(
            $"Cephalon OpenShift observability integration only supports telemetry provider 'OpenTelemetry'. Configured provider '{provider}' is not supported.");
    }
    ...
    private static void ConfigureExporter(
        ...
        OpenShiftEndpointMode endpointMode,
        ...
        Uri endpoint)
    {
        ...
        {
            throw new InvalidOperationException(
                "Cephalon OpenShift observability integration currently only supports TrustedCaCertificatePath for HTTPS OTLP/HTTP traces and metrics. OTLP/gRPC custom CA handling should stay explicit in the collector or host trust store.");
        }

        if (signal == TelemetrySignal.Logs)
        {
            throw new InvalidOperationException(
                "Cephalon OpenShift observability integration cannot apply TrustedCaCertificatePath to OTLP/HTTP logs because the current OpenTelemetry logging exporter does not support custom HttpClientFactory wiring. Disable log export for that path or rely on system trust.");
        }

        var trustedCaCertificatePath = options.TrustedCaCertificatePath.Trim();
        exporter.HttpClientFactory = () => OpenShiftTrustedCaHttpClientHandler.CreateHttpClient(trustedCaCertificatePath);
    }
        ...
        OpenShiftEndpointMode endpointMode)
    {
        ...
        {
            OpenShiftEndpointMode.ConfiguredEndpoint => ResolveAbsoluteEndpoint(
                telemetry.Endpoint,
                "Telemetry endpoint"),
            OpenShiftEndpointMode.SelfHostedDefaults => exporterProtocol switch
            {
            ...
            OpenShiftEndpointMode.InClusterCollectorService => BuildInClusterCollectorEndpoint(options, exporterProtocol),
            _ => null
        };
    }
    ...
    private static Uri ResolveAbsoluteEndpoint(string? value, string description)
    {
        ...
        {
            throw new InvalidOperationException(
                $"Cephalon OpenShift observability integration requires {description.ToLowerInvariant()} to be configured.");
        }
        ...
        IReadOnlyList<KeyValuePair<string, object>> platformAttributes)
    {
        ...
        ConfigureOpenShiftResource(resourceBuilder, platformAttributes);
        return resourceBuilder;
    }

    private static void ConfigureOpenShiftResource(
        ...
        string? environmentName)
    {
        ...
        if (string.Equals(normalizedHostedPlatform, "aro", StringComparison.Ordinal))
        {
            attributes.Add(new KeyValuePair<string, object>("cloud.provider", "azure"));
        }
        else if (string.Equals(normalizedHostedPlatform, "rosa", StringComparison.Ordinal))
        {
            attributes.Add(new KeyValuePair<string, object>("cloud.provider", "aws"));
        }
...
internal enum OpenShiftEndpointMode
{
    None,
    ConfiguredEndpoint,
    SelfHostedDefaults,
