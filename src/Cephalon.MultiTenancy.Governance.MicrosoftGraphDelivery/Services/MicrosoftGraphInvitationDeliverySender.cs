...
internal sealed class MicrosoftGraphInvitationDeliverySender(
    MicrosoftGraphInvitationDeliveryOptions options,
    IMicrosoftGraphInvitationDeliveryClient client,
    ILogger<MicrosoftGraphInvitationDeliverySender> logger) : ITenantInvitationDeliverySender
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var reason = $"Microsoft Graph invitation delivery sender '{SenderId}' does not support channel '{context.Channel}'.";
            MicrosoftGraphInvitationDeliveryLogs.Failed(logger, SenderId, context.TenantId, context.InvitationId, reason, null);
        ...
        {
            var reason = $"Microsoft Graph invitation delivery could not resolve a recipient email address from metadata key '{GetRecipientMetadataKey()}'.";
            MicrosoftGraphInvitationDeliveryLogs.Failed(logger, SenderId, context.TenantId, context.InvitationId, reason, null);
        ...
        try
        {
            ...
            {
                metadata["microsoftGraphStatusCode"] = result.StatusCode.Value.ToString(CultureInfo.InvariantCulture);
            }

            if (result.Accepted)
            {
                MicrosoftGraphInvitationDeliveryLogs.Accepted(logger, SenderId, context.TenantId, context.InvitationId, result.StatusCode ?? 0);
                ...
                return new TenantInvitationDeliverySenderResult(
                    ...
                    reason: string.IsNullOrWhiteSpace(result.Reason)
                        ? "Microsoft Graph sendMail accepted the invitation delivery request."
                        : result.Reason,
                    dispatchedAtUtc: context.DispatchedAtUtc,
                    metadata: metadata);
            }
            ...
            var reason = string.IsNullOrWhiteSpace(result.Reason)
                ? "Microsoft Graph sendMail did not accept the invitation delivery request."
                : result.Reason;
            MicrosoftGraphInvitationDeliveryLogs.Failed(logger, SenderId, context.TenantId, context.InvitationId, reason, null);
        ...
        {
            var reason = $"Microsoft Graph invitation delivery timed out after {Math.Clamp(options.TimeoutSeconds, 1, 300)} seconds.";
            MicrosoftGraphInvitationDeliveryLogs.Failed(logger, SenderId, context.TenantId, context.InvitationId, reason, exception);
        ...
        {
            var reason = "Microsoft Graph invitation delivery failed before sendMail accepted the request.";
            MicrosoftGraphInvitationDeliveryLogs.Failed(logger, SenderId, context.TenantId, context.InvitationId, reason, exception);

            var metadata = BuildBaseMetadata(context, null, null, reason);
            metadata["microsoftGraphExceptionType"] = exception.GetType().Name;
    ...
    private MicrosoftGraphInvitationDeliveryMessage CreateMessage(TenantInvitationDeliveryContext context, string recipient)
    {
        var messageId = CreateMessageId(context);
        _ = MicrosoftGraphInvitationDeliveryAddress.TryCreate(recipient, context.DisplayName, out var toAddress);

        return new MicrosoftGraphInvitationDeliveryMessage(
            messageId,
            options.GetSenderMailboxScope(),
            toAddress!.Address,
    ...
    {
        return MicrosoftGraphInvitationDeliveryAddress.TryCreate(address, null, out _);
    }
    ...
    private static bool IsSafeHeader(string? name, string? value)
    {
        return !string.IsNullOrWhiteSpace(name) &&
            ...
            name.Trim().StartsWith("x-", StringComparison.OrdinalIgnoreCase) &&
    ...
    private string CreateMessageId(TenantInvitationDeliveryContext context)
    {
        var material = string.Join(
            '\n',
            "cephalon-microsoft-graph-invitation-delivery",
            "v1",
        ...
        string? reason)
    {
        ...
        {
            ["microsoftGraphSenderId"] = SenderId,
            ["microsoftGraphEndpointHost"] = options.TryGetBaseUrl()?.Host ?? string.Empty,
            ["microsoftGraphApiVersion"] = options.GetApiVersion(),
            ["microsoftGraphSenderUserId"] = options.GetSenderMailboxScope(),
            ["microsoftGraphRecipientMetadataKey"] = GetRecipientMetadataKey(),
            ["microsoftGraphSaveToSentItems"] = options.SaveToSentItems ? "true" : "false",
            ["microsoftGraphAcceptedStatusCodes"] = string.Join(",", options.GetAcceptedStatusCodes()),
            ["microsoftGraphExternalDeliveryOwnership"] = "provider-managed"
        };

        if (message is not null)
        {
            metadata["microsoftGraphCephalonMessageId"] = message.MessageId;
            metadata["microsoftGraphRecipientEmail"] = message.ToEmail;
            metadata["microsoftGraphBodyContentType"] = message.HasHtmlBody ? "HTML" : "Text";
            metadata["microsoftGraphCategoryCount"] = message.Categories.Count.ToString(CultureInfo.InvariantCulture);
            metadata["microsoftGraphHeaderCount"] = message.Headers.Count.ToString(CultureInfo.InvariantCulture);
        }

        if (!string.IsNullOrWhiteSpace(reason))
        {
            metadata["microsoftGraphReason"] = reason.Trim();
        }
...
internal static class MicrosoftGraphInvitationDeliveryLogs
{
    private static readonly Action<ILogger, string, string, string, int, Exception?> AcceptedMessage =
        LoggerMessage.Define<string, string, string, int>(
            ...
            new EventId(
                MicrosoftGraphInvitationDeliveryDiagnosticsConventions.MicrosoftGraphInvitationDeliveryAccepted.Id,
                MicrosoftGraphInvitationDeliveryDiagnosticsConventions.MicrosoftGraphInvitationDeliveryAccepted.Name),
            "Microsoft Graph invitation delivery sender '{SenderId}' accepted invitation '{InvitationId}' for tenant '{TenantId}' with status code {StatusCode}.");

    private static readonly Action<ILogger, string, string, string, string, Exception?> FailedMessage =
        LoggerMessage.Define<string, string, string, string>(
            LogLevel.Warning,
            new EventId(
                MicrosoftGraphInvitationDeliveryDiagnosticsConventions.MicrosoftGraphInvitationDeliveryFailed.Id,
                MicrosoftGraphInvitationDeliveryDiagnosticsConventions.MicrosoftGraphInvitationDeliveryFailed.Name),
            "Microsoft Graph invitation delivery sender '{SenderId}' failed invitation '{InvitationId}' for tenant '{TenantId}'. Reason: {Reason}.");
