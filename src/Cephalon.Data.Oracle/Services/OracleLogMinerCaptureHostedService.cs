    ...
    ILogger<OracleLogMinerCaptureHostedService> logger) : BackgroundService
{
    ...
    {
        if (exception is OracleLogMinerCaptureException direct)
        {
            failure = new OracleLogMinerCaptureFailure(direct.FailureKind, direct.Metadata);
            return true;
        }
        ...
        failure = new OracleLogMinerCaptureFailure(failureKind.Trim(), metadata);
        return true;
    }
