...
/// <remarks>
/// <para>
/// At M1 maturity the engine routes seven emission sites through registered filters:
...
/// <c>CephalonActivitySources.Retrieval</c>), <c>Cephalon.Worker</c>'s lifecycle activity
...
/// <c>cephalon.blueprint</c> / <c>cephalon.module.count</c> tags), and <c>Cephalon.Eventing</c>'s
/// in-process publication-dispatch activity tags (the <c>eventing.publication.dispatch</c> span
/// emitted under <c>CephalonActivitySources.Eventing</c> with publisher / publication / channel /
/// event-type / outcome / matched-subscription-count identifiers). All seven sites pipe values
/// through the registered <see cref="RedactionPipeline"/> resolved from DI before the value
/// reaches an exporter. Additional emission sites (multi-tenancy governance once its OTel
/// adapter lands) will adopt the same pattern as the surface continues to promote. The contract
/// guarantees the runtime honors are:
/// </para>
/// <list type="bullet">
