...
internal sealed class AmazonSesInvitationDeliveryStatusCallbackReplayGuard
{
    private readonly Lock gate = new();
    private readonly Dictionary<string, DateTimeOffset> acceptedCallbacks = new(StringComparer.Ordinal);

    public AmazonSesInvitationDeliveryStatusCallbackReplayDecision TryRecord(
        string replayKey,
        ...
        int cacheLimit)
    {
        ...
        lock (gate)
        {
            ...
            {
                return new AmazonSesInvitationDeliveryStatusCallbackReplayDecision(
                    Accepted: false,
...
internal sealed record AmazonSesInvitationDeliveryStatusCallbackReplayDecision(
    bool Accepted,
    string Outcome,
    int CacheSize);
