...
internal sealed class SendGridInvitationDeliveryStatusCallbackReplayGuard
{
    private readonly Lock gate = new();
    private readonly Dictionary<string, DateTimeOffset> acceptedCallbacks = new(StringComparer.Ordinal);

    public SendGridInvitationDeliveryStatusCallbackReplayDecision TryRecord(
        string replayKey,
        ...
        int cacheLimit)
    {
        ...
        lock (gate)
        {
            ...
            {
                return new SendGridInvitationDeliveryStatusCallbackReplayDecision(
                    Accepted: false,
...
internal sealed record SendGridInvitationDeliveryStatusCallbackReplayDecision(
    bool Accepted,
    string Outcome,
    int CacheSize);
