...
namespace Cephalon.EventSourcing.Elasticsearch.Services;

internal sealed class ElasticsearchEventStoreContributor(string uri, string indexName) : IEventStoreContributor
{
    public IReadOnlyList<EventStreamDescriptor> Contribute()
    {
        ...
        [
            new EventStreamDescriptor(
                id: "elasticsearch-event-store",
                displayName: "Elasticsearch Event Store",
                description: "Appends and replays domain events through an Elasticsearch index.",
                sourceModuleId: "elasticsearch-event-sourcing",
                provider: "elasticsearch",
                mode: "append-only",
                tags: ["event-sourcing", "elasticsearch", "search"],
