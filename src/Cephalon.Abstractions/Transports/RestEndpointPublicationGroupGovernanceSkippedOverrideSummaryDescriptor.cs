...
public sealed class RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryDescriptor
{
    ...
    /// </param>
    public RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryDescriptor(
        string ruleId,
        IReadOnlyList<string>? candidateIds = null)
    {
        ...
        if (normalizedCandidateIds.Length == 0)
        {
            throw new ArgumentException(
                "At least one candidate id is required for a grouped skipped override-rule summary.",
                nameof(candidateIds));
        }
...
internal static class RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryBuilder
{
    internal static RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryDescriptor[] BuildFromCandidates(
        IReadOnlyList<RestEndpointCandidateRuntimeDescriptor> candidates)
    {
        ...
        return orderedRuleIds
            .Select(ruleId =>
                new RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryDescriptor(
                    ruleId,
                    candidateIdsByRuleId[ruleId]))
            .ToArray();
    }

    internal static RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryDescriptor[] Normalize(
        IReadOnlyList<RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryDescriptor>? summaries,
        ...
        string parameterName)
    {
        ...
        var result = new List<RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryDescriptor>(summaries.Count);
        ...
        foreach (var summary in summaries.Where(static summary => summary is not null))
        {
            ...
            {
                throw new ArgumentException(
                    "A grouped skipped override-rule summary can only declare one entry per rule id.",
                    parameterName);
            }
            ...
            {
                throw new ArgumentException(
                    "Grouped skipped override-rule summary candidate ids must refer to candidates in the grouped publication answer.",
                    parameterName);
            }

            result.Add(new RestEndpointPublicationGroupGovernanceSkippedOverrideSummaryDescriptor(
                summary.RuleId,
