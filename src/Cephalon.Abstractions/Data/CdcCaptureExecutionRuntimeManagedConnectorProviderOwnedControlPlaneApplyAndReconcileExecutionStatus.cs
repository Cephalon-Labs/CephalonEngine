...
/// </summary>
public sealed record CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStatus
{
    /// <summary>
    /// Creates a new managed-connector provider-owned control-plane apply-and-reconcile execution answer.
    /// </summary>
    /// <param name="state">
    /// The stable provider-owned control-plane apply-and-reconcile execution state, such as <c>not-applicable</c>, <c>operator-only</c>, <c>apply-and-reconcile-ready</c>, <c>apply-and-reconcile-blocked</c>, <c>apply-and-reconcile-executing</c>, <c>apply-and-reconcile-completed</c>, or <c>apply-and-reconcile-risk</c>.
    /// </param>
    /// <param name="description">An optional operator-facing provider-owned control-plane apply-and-reconcile execution summary.</param>
    public CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStatus(
        string state,
        string? description = null)
    {
        if (string.IsNullOrWhiteSpace(state))
        {
            throw new ArgumentException("Managed-connector provider-owned control-plane apply-and-reconcile execution state is required.", nameof(state));
        }
    ...
    /// </summary>
    public string SourceId { get; init; } = CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionSources.Unknown;
    ...
    public bool AppliesToManagedConnector =>
        !string.Equals(State, CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStates.NotApplicable, StringComparison.OrdinalIgnoreCase);
    ...
    /// </summary>
    public bool IsOperatorOnly => string.Equals(State, CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStates.OperatorOnly, StringComparison.OrdinalIgnoreCase);

    /// <summary>
    /// Gets a value indicating whether provider-owned control-plane apply-and-reconcile execution is currently ready.
    /// </summary>
    public bool IsApplyAndReconcileReady => string.Equals(State, CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStates.ApplyAndReconcileReady, StringComparison.OrdinalIgnoreCase);

    /// <summary>
    /// Gets a value indicating whether provider-owned control-plane apply-and-reconcile execution currently remains blocked.
    /// </summary>
    public bool IsApplyAndReconcileBlocked => string.Equals(State, CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStates.ApplyAndReconcileBlocked, StringComparison.OrdinalIgnoreCase);
    ...
    /// </summary>
    public bool IsApplyAndReconcileExecuting => string.Equals(State, CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStates.ApplyAndReconcileExecuting, StringComparison.OrdinalIgnoreCase);
    ...
    /// </summary>
    public bool IsApplyAndReconcileCompleted => string.Equals(State, CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStates.ApplyAndReconcileCompleted, StringComparison.OrdinalIgnoreCase);

    /// <summary>
    /// Gets a value indicating whether provider-owned control-plane apply-and-reconcile execution currently remains risky.
    /// </summary>
    public bool IsApplyAndReconcileRisk => string.Equals(State, CdcCaptureExecutionRuntimeManagedConnectorProviderOwnedControlPlaneApplyAndReconcileExecutionStates.ApplyAndReconcileRisk, StringComparison.OrdinalIgnoreCase);
}
