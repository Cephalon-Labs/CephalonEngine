...
public sealed class AuditEntry
{
        ...
        IReadOnlyDictionary<string, string>? metadata = null)
    {
        ...
        {
            throw new ArgumentException("Audit entry id is required.", nameof(id));
        }

        if (string.IsNullOrWhiteSpace(category))
        {
            throw new ArgumentException("Audit entry category is required.", nameof(category));
        }

        if (string.IsNullOrWhiteSpace(action))
        {
            throw new ArgumentException("Audit entry action is required.", nameof(action));
        }

        if (string.IsNullOrWhiteSpace(summary))
        {
            throw new ArgumentException("Audit entry summary is required.", nameof(summary));
        }

        if (string.IsNullOrWhiteSpace(subjectType))
        {
            throw new ArgumentException("Audit entry subject type is required.", nameof(subjectType));
        }

        if (occurredAtUtc == default)
        {
            throw new ArgumentException("Audit entry occurrence time is required.", nameof(occurredAtUtc));
        }
