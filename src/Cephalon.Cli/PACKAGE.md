...
If you pass `cephalon doctor --scorecard <path>`, doctor reads an existing generated `engine-completion-scorecard.json` artifact and summarizes schema/source truth, platform gate posture, validated evidence-reference counts, per-package GA readiness counts, deployment-mode global/package-scoped/hazard/publish-probe counts from `DeploymentModeEvidence`, SRE SLI/target/baseline counts from `SrePostureEvidence`, supply-chain workflow/external-policy counts from `SupplyChainEvidence`, and public API package/pending/addition/removal counts from `PublicApiCompatibilityEvidence`. The CLI only reports the generated read model; it does not parse scorecard Markdown, duplicate source truth, or promote GA/support claims.
