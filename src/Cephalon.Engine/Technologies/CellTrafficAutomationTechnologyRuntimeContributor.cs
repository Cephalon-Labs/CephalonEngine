    ...
    ICellTrafficAutomationRuntimeCatalog catalog) : ITechnologyRuntimeContributor
{
    ...
    private static TechnologyRuntimeEntry CreateEntry(CellTrafficAutomationRuntimeDescriptor automation)
    {
        ...
        if (!string.IsNullOrWhiteSpace(automation.MaterializationState))
        {
            metadata["materializationState"] = automation.MaterializationState!;
        }

        if (automation.MaterializationObservedAtUtc is not null)
        {
            metadata["materializationObservedAtUtc"] = automation.MaterializationObservedAtUtc.Value.ToString("O", CultureInfo.InvariantCulture);
        }

        if (!string.IsNullOrWhiteSpace(automation.MaterializationError))
        {
            metadata["materializationError"] = automation.MaterializationError!;
        }
        ...
        if (!string.IsNullOrWhiteSpace(automation.ProviderMaterializationState))
        {
            metadata["providerMaterializationState"] = automation.ProviderMaterializationState!;
        }

        if (automation.ProviderMaterializationObservedAtUtc is not null)
        {
            metadata["providerMaterializationObservedAtUtc"] = automation.ProviderMaterializationObservedAtUtc.Value.ToString("O", CultureInfo.InvariantCulture);
        }
