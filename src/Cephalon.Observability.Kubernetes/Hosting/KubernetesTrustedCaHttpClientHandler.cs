...
internal sealed class KubernetesTrustedCaHttpClientHandler : HttpClientHandler
{
    private readonly X509Certificate2Collection trustedCertificates;

    public KubernetesTrustedCaHttpClientHandler(string trustedCaCertificatePath)
    {
    ...
    {
        return new HttpClient(new KubernetesTrustedCaHttpClientHandler(trustedCaCertificatePath), disposeHandler: true);
    }
    ...
    private static X509Certificate2Collection LoadTrustedCertificates(string trustedCaCertificatePath)
    {
        if (string.IsNullOrWhiteSpace(trustedCaCertificatePath))
        {
            throw new InvalidOperationException(
                "Cephalon Kubernetes observability integration requires Engine:Observability:Telemetry:Kubernetes:TrustedCaCertificatePath to be configured when a custom CA bundle is requested.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Kubernetes trusted CA bundle '{normalizedPath}' was not found.");
        }
        ...
        else
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Kubernetes trusted CA bundle '{normalizedPath}' could not be parsed as a PEM or DER certificate file.",
                    exception);
            }
        }
        ...
        {
            throw new InvalidOperationException(
                $"Kubernetes trusted CA bundle '{normalizedPath}' did not contain any certificates.");
        }
