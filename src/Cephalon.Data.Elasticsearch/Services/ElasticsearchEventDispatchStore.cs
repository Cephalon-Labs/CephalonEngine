...
internal sealed class ElasticsearchEventDispatchStore(
    Elastic.Clients.Elasticsearch.ElasticsearchClient client,
    string indexName) : IEventDispatchStore
{
    private const string OutboxId = "elasticsearch-outbox";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await client.SearchAsync<ElasticsearchOutboxEntry>(search => search
                .Index(indexName)
                .Size(Math.Min(maximumCount * 4, 256))
                .Query(query => query.Bool(boolean => boolean.MustNot(
                    mustNot => mustNot.Exists(exists => exists.Field(field => field.DispatchedAtUtc)))))
                .Sort(sort => sort.Field(field => field.CreatedAtUtc, descriptor => descriptor.Order(Elastic.Clients.Elasticsearch.SortOrder.Asc))),
            cancellationToken)
            .ConfigureAwait(false);

        if (!response.IsSuccess())
        {
            throw new InvalidOperationException($"Elasticsearch outbox dispatch-store read failed for index '{indexName}': {response.DebugInformation}");
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException($"Outbox '{report.OutboxId}' is not owned by the active Elasticsearch event dispatch store.");
        }

        if (string.IsNullOrWhiteSpace(report.MessageId))
        {
            throw new InvalidOperationException("Dispatch reports must include a message id when applied to the durable Elasticsearch event dispatch store.");
        }

        var getResponse = await client.GetAsync<ElasticsearchOutboxEntry>(
                report.MessageId,
                descriptor => descriptor.Index(indexName),
                cancellationToken)
            .ConfigureAwait(false);

        if (!getResponse.IsSuccess() || !getResponse.Found || getResponse.Source is null)
        {
            throw new InvalidOperationException($"Outbox message '{report.MessageId}' is not staged in the active Elasticsearch outbox.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"Elasticsearch outbox dispatch-store update failed for message '{report.MessageId}': {indexResponse.DebugInformation}");
        }
    }

    private static EventDispatchItem CreateDispatchItem(ElasticsearchOutboxEntry entry)
    {
    ...
    private static string NormalizeOutcome(string outcome)
    {
        ...
        return normalized switch
        {
            ...
            _ => throw new InvalidOperationException($"Dispatch outcome '{outcome}' is not supported by the Elasticsearch event dispatch store.")
        };
    }
