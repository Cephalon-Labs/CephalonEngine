...
internal sealed class MailgunInvitationDeliveryStatusCallbackReplayGuard
{
    private readonly Lock gate = new();
    private readonly Dictionary<string, DateTimeOffset> acceptedCallbacks = new(StringComparer.Ordinal);

    public MailgunInvitationDeliveryStatusCallbackReplayDecision TryRecord(
        string replayKey,
        ...
        int cacheLimit)
    {
        ...
        lock (gate)
        {
            ...
            {
                return new MailgunInvitationDeliveryStatusCallbackReplayDecision(
                    Accepted: false,
...
internal sealed record MailgunInvitationDeliveryStatusCallbackReplayDecision(
    bool Accepted,
    string Outcome,
    int CacheSize);
